...
return Math.min(base + (A - threshold) * bonus, cap);`;
    ...
    async function renderClassesPage(container) {
        ...
        const updateClassesTable = () => {
            ...
            // --- FIX: Prioritize AppState for branch selection ---
            let selectedBranchId = 'all';
            if (branchFilter) {
                selectedBranchId = branchFilter.value; // If filter visible, use its current value
            } else if (appState.selectedFilters.classesBranchId) {
                selectedBranchId = appState.selectedFilters.classesBranchId; // Use saved state
            } else if (effectiveBranches.length === 1) {
                selectedBranchId = effectiveBranches[0].id; // Force single allowed
            }
