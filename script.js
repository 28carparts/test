...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let appState = { 
        ...
        users: [],
        branches: [], // <--- ADD THIS
        selectedScheduleBranch: null, // <--- ADD THIS
        activePage: 'schedule', 
        currentUser: null,
        currentLanguage: 'en',
        studioSettings: {
        ...
        incomeSort: { // Add this new object
            key: 'date',
            direction: 'asc'
        },
...
${_('whatsapp_closing')}
    ...
    function openClsModal(dateIso, clsToEdit = null) {
                ...
                <form id="clsForm">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <select id="clsCreditType" name="costCreditTypeId" class="form-select"></select>
                        </div>
        ...
        let ctOptions = `<option value="general">${_('label_general_credits_default')}</option>`;
    ...
    function createClsElement(cls) {
        ...
        // --- END: TIER RESTRICTION CHECK ---

        // --- START: HYBRID FALLBACK CHECK ---
        // Even if the plan is restricted, can they pay with credits?
        ...
        // A user is "Truly Blocked" only if:
        // 1. Their Plan is restricted (NotForMonthly OR TierRestricted)
        // 2. AND they do NOT have enough credits to pay for it
        // 3. AND they haven't already booked it
        ...
        // Logic: If blocked, show error on click. 
            ...
            mainAction = () => {
                ...
                // Specific error message depending on why they are blocked
            ...
            // 1. Plan covers it
            // 2. OR Plan doesn't cover it, but Credits cover it (Hybrid Success)
            el.classList.add('cursor-pointer');
        ...
        if (isBookedByCurrentUser) {
            ...
            // If they can't pay via Plan AND can't pay via Credits, show Not Available
            ...
            // Standard state (Plan OK or Credits OK)
    ...
    function _renderScheduleCarousel(container, startDate, endDate, datesArray, initialScrollIndex, showAddButton, filteredClasses = null) {
        ...
        const branches = appState.branches || [];
        // Identify the current view's branch ID (fallback to first branch for legacy)
        const currentBranchId = appState.selectedScheduleBranch || (branches.length > 0 ? branches[0].id : 'legacy');
        const mainBranchId = branches.length > 0 ? branches[0].id : 'legacy';
        ...
        datesArray.forEach(dateIso => {
            ...
            // --- NEW: Independent Publish Status Logic ---
            let isPublished = false;
            const statusRaw = scheduleStatus[dateIso];

            if (typeof statusRaw === 'boolean') {
                // Legacy Data (Boolean): It only applies to the Main Studio
                if (currentBranchId === mainBranchId) {
                    isPublished = statusRaw;
                } else {
                    isPublished = false; // New studios default to Draft until explicitly published
                }
            } else if (statusRaw && typeof statusRaw === 'object') {
                // New Data (Object): Lookup specific branch status
    ...
    function performCopy(type, sourceData, targetDate) {
        ...
        const finalizeCopy = () => {
            ...
            if (appState.selectedScheduleBranch !== targetBranchId) {
                ...
                // CRITICAL FIX: Set the scroll target so the schedule jumps to the
                // specific day we just pasted into, ignoring previous scroll state.
                ...
                // If we are already on the correct branch, still scroll to the date
                // for better UX (in case the target date is off-screen)
                appState.scrollToDateOnNextLoad = targetDate;
                if(appState.activePage === 'schedule') {
    ...
    function renderAccountPage(container) {
        ...
        const getProfileContent = () => {
            // FIX: Changed 'justify-between' to 'gap-2' to keep date next to label
            ...
            if (member.monthlyPlan) {
                ...
                const nextDue = member.paymentDueDate ? formatShortDateWithYear(member.paymentDueDate) : 'N/A';
                
                // FIX: Changed 'justify-between' to 'gap-2' to keep pill next to label
                content += `
                <div class="mt-4 pt-4 border-t border-slate-100">
                    ...
                    <div class="mt-2"><p data-lang-key="label_next_payment_due" class="text-sm text-slate-500"></p><p class="font-bold text-sm text-slate-800">${nextDue}</p></div>
                </div>`;
            }
    ...
    function renderMembersPage(container) {
        ...
        const updateTable = (searchTerm = '') => {
            ...
            tableBody.innerHTML = paginatedUsers.map(member => {
                ...
                // --- 1. Prepare Data Rows ---
                // We create an array of "rows" to ensure Plan and Expiry align 1:1 vertically
                const displayRows = [];
                ...
                if (member.monthlyPlan) {
                    ...
                    // CHANGE: Updated text-slate-700 to text-slate-900
                ...
                if (activeWalletEntries.length > 0) {
                    ...
                    // Fallback for completely empty users
                ...
                // --- 2. Construct Columns with Fixed Height Wrappers ---
                // h-7 (1.75rem) ensures each "row" inside the cell is exactly the same height
        ...
        if (isOwner) {
            // Export handlers are mostly unchanged in logic, just re-attached
    ...
    function renderAdminPage(container) {
        ...
        container.innerHTML = `
            <div class="space-y-8">
                ...
                <!-- Announcement Card -->
                <div class="card p-6 md:p-8">
                    <h3 data-lang-key="header_announcements" class="text-2xl font-bold text-slate-800 mb-4"></h3>
                    <p data-lang-key="desc_announcements" class="text-slate-500 mb-6"></p>
    ...
    function renderColorPicker(container, colorInput) {
        const STORAGE_KEY = 'studioPulseCustomColors';
        const DEFAULT_GRAY = '#f1f5f9';
        
        // 1. Load or Init Storage
        let customColors = [];
        try {
            const stored = localStorage.getItem(STORAGE_KEY);
            customColors = stored ? JSON.parse(stored) : Array(9).fill(DEFAULT_GRAY);
        } catch (e) {
            customColors = Array(9).fill(DEFAULT_GRAY);
        }

        // 2. Predefined
        const predefinedColors = CLS_COLORS.slice(0, 18);
        const initialColor = colorInput.value || CLS_COLORS[0];
        
        let html = '';

        // A. Render Predefined Colors
        predefinedColors.forEach((color, index) => {
            const isChecked = color === initialColor ? 'checked' : '';
            html += `
                <input type="radio" name="color" id="color-${index}" value="${color}" class="color-swatch-radio" ${isChecked}>
                <label for="color-${index}" class="color-swatch-label" style="background-color: ${color};"></label>
            `;
        });

        // B. Render Custom Slots
        customColors.forEach((color, index) => {
            ...
            const isDefault = color === DEFAULT_GRAY;
            // Icon style: if default, dark gray. If colored, semi-transparent white.
            const iconClass = isDefault ? 'text-slate-400' : 'text-white/50 hover:text-white';
            
            // NOTE: We place the radio BEFORE the label so CSS adjacency works.
            // The label itself handles the "Select" click.
            // The input[type=color] is now small and positioned over the ICON only.
            html += `
                <div class="relative inline-block custom-color-wrapper">
                    <input type="radio" name="color" id="${id}" value="${color}" class="color-swatch-radio" ${isChecked}>
                    <label for="${id}" class="color-swatch-label relative flex items-center justify-center" style="background-color: ${color}; cursor: pointer;">
                        
                        <!-- The Edit Trigger (Icon Wrapper) -->
                        <div class="relative z-10 w-5 h-5 flex items-center justify-center rounded-full transition-colors hover:bg-black/10">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 ${iconClass} pointer-events-none" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4" />
                            </svg>
                            
                            <!-- Actual Color Input: Only covers the center icon area -->
                            <input type="color" 
                                   class="absolute inset-0 w-full h-full opacity-0 cursor-pointer" 
                                   value="${isDefault ? '#555555' : color}" 
                                   data-index="${index}" 
                                   data-target-id="${id}" 
        ...
        // Event 1: Handle Selection (Clicking the circle body)
        // This works for both predefined and custom slots because we use standard radios
        container.onclick = e => {
        ...
        // Event 2: Handle Editing (Clicking the center icon/input)
        container.addEventListener('input', (e) => {
            if (e.target.type === 'color') {
                // Stop the click from selecting the radio immediately (optional, but cleaner)
                e.stopPropagation();

                const newColor = e.target.value;
                const index = e.target.dataset.index;
                const radioId = e.target.dataset.targetId;
                ...
                if(icon) {
                    icon.className = `h-4 w-4 text-white/50 hover:text-white pointer-events-none transition-colors duration-200`;
                }
                
                // 2. Update Logic
                radio.value = newColor;
                radio.checked = true;
