    ...
    function openCheckInModal() {
        ...
        const renderScanner = (isSelfCheckIn) => {
            ...
            contentArea.innerHTML = `
                ...
                <div id="checkInResult" class="w-full mt-4 min-h-[3rem]"></div>
                <!-- NEW: Upcoming Classes List (Visible for Members scanning Studio) -->
                ${isSelfCheckIn ? `<div id="scannerUpcomingList" class="w-full mt-2 space-y-2"></div>` : ''}
            `;

            // Render List for Members
            if (isSelfCheckIn) {
        ...
        const renderStationMode = () => {
            ...
            document.getElementById('modalPrintStaticBtn').onclick = () => {
                cleanup(); 
                closeModal(modal); 
                setTimeout(() => openBranchQrModal(currentStationBranchId), 300);
            };
        };
