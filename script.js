...
${_('whatsapp_closing')}
    ...
    async function handleMemberSelfCheckIn(decodedText, html5QrCodeInstance) {
        ...
        // Security Level 3: Check Timestamp for Dynamic Codes
        if (qrData.type === 'studio_dynamic') {
            const now = Date.now();
            const qrTime = qrData.ts || 0;
            // Allow 60 seconds validity window
            if (now - qrTime > 60000) { 
        ...
        // Find bookings for today that are NOT checked in
        ...
        // Filter by Branch (if QR has specific branch)
        // If branchId in QR is missing or 'legacy', we assume it's valid for all or main.
        const validBookings = bookingsToday.filter(cls => {
            if (!qrData.branchId) return true; // Legacy/Global QR
        ...
        if (bookingsToday.length > 0 && validBookings.length === 0) {
            // Member has bookings today, but at a DIFFERENT branch
            if (resultEl) resultEl.innerHTML = `<div class="check-in-result-banner check-in-error">${_('error_qr_wrong_branch')}</div>`;
            return;
        }

        if (validBookings.length === 0) {
            // UX Improvement: No Booking -> Redirect to Schedule
            if (resultEl) resultEl.innerHTML = `<div class="check-in-result-banner check-in-notice">${_('error_no_booking_redirect')}</div>`;
            
            setTimeout(() => {
                if (html5QrCodeInstance) html5QrCodeInstance.stop().catch(console.error);
                // Set logic to scroll to "Now"
                const now = new Date();
                const hours = String(now.getHours()).padStart(2, '0');
                const minutes = String(now.getMinutes()).padStart(2, '0');
                
                // We use a custom property to tell the schedule renderer to filter/scroll
                appState.scrollToDateOnNextLoad = today;
        ...
        if (html5QrCodeInstance) html5QrCodeInstance.pause();

        if (validBookings.length === 1) {
            // Single Booking - Auto Check-in
            const cls = validBookings[0];
            await performCheckIn(cls.id, member.id, html5QrCodeInstance);
        } else {
    ...
    function openCheckInModal() {
        ...
        // **FIX:** Define State Here (Outside the render function) so it persists
        const branches = appState.branches || [];
        // Default to user's home branch, or the first available branch
        ...
        const renderStationMode = () => {
            ...
            if (hasMultipleBranches) {
                const buttonsHtml = branches.map(b => {
                    // **FIX:** Compare against the persisted state variable
            ...
            const renderQR = () => {
                ...
                if (!inner) return;
                inner.innerHTML = '';
                // **FIX:** Use the persisted state variable
            ...
            if (hasMultipleBranches) {
                contentArea.querySelectorAll('.station-branch-btn').forEach(btn => {
                    btn.onclick = () => {
                        // **FIX:** Update the state variable outside this function scope
                        currentStationBranchId = btn.dataset.branchId;
                        renderStationMode(); // Re-render logic to update UI
                    };
                });
            }
