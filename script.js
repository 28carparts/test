/**
 * BookPass - Class Management System
 * 
 * @version 2.4.5
 * @copyright 2026 28CarParts. All rights reserved.
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let appState = { 
        ...
        currentLanguage: 'en',
        studioProfile: {
            name: 'StudioPulse', // Default fallback
            fullName: 'StudioPulse Fitness',
            slug: 'studiopulse'
        },
        studioSettings: {
            clsDefaults: {
    ...
    const _ = (key) => {
        let text = LANG_PACK[appState.currentLanguage]?.[key] || LANG_PACK['en'][key] || key;
        
        // Inject Studio Name dynamically into translations
        if (typeof text === 'string' && appState.studioProfile?.name) {
            text = text.replace(/{studioName}/g, appState.studioProfile.name);
        }
    ...
    const updateStudioIdentity = () => {
        // 1. Destructure with defaults
        const { name, fullName, themeColor, backgroundColor } = appState.studioProfile;
        const safeThemeColor = themeColor || '#ffffff';
        const safeBgColor = backgroundColor || '#ffffff';

        // 2. Update Browser Tab Title
        const titleEl = document.getElementById('pageTitle');
        if (titleEl) titleEl.innerText = `${name} - Class Dashboard`;

        // 3. Update Image Alt Text
        document.querySelectorAll('.studio-logo-img').forEach(img => {
            img.alt = fullName || name;
        });

        // 4. Update Text Spans (Footers)
        document.querySelectorAll('.studio-name-text').forEach(el => {
            el.innerText = name;
        });

        // 5. Update Browser Address Bar Color
        let metaThemeTag = document.querySelector('meta[name="theme-color"]');
        if (!metaThemeTag) {
            metaThemeTag = document.createElement('meta');
            metaThemeTag.name = "theme-color";
            document.head.appendChild(metaThemeTag);
        }
        metaThemeTag.content = safeThemeColor;

        // --- FIX START: Generate Absolute URLs for Icons ---
        // This takes your current website address and attaches the filename to it
        // ensuring the manifest knows exactly where to find the images.
        const icon192 = new URL("android-chrome-192x192.png", window.location.href).href;
        const icon512 = new URL("android-chrome-512x512.png", window.location.href).href;
        // --- FIX END ---
        ...
        const dynamicManifest = {
            ...
            "icons": [
                {
                    "src": icon192, // Use the absolute URL
                    "sizes": "192x192",
                    "type": "image/png"
                },
                {
                    "src": icon512, // Use the absolute URL
                    "sizes": "512x512",
                    "type": "image/png"
                }
            ],
            "theme_color": safeThemeColor,
            "background_color": safeBgColor,
            "display": "standalone"
        };
    ...
    const canManageBranch = (user, targetBranchId) => {
        // 1. Only Owners & SuperAdmins have unrestricted access
    ...
    const getEffectiveBranches = (user, allBranches) => {
        // 1. Owners & SuperAdmins see everything
        if (user.role === 'owner' || user.role === 'superadmin') return allBranches;
    ...
    const formatBookingAuditText = (bookingInfo) => {
        ...
        const formattedDate = formatShortDateWithYear(bookingInfo.bookedAt) + ' - ' + formatTime(new Date(bookingInfo.bookedAt));
    ...
    const setupTimeInputMask = (inputElement) => {
        if (!inputElement) return;
        inputElement.addEventListener('input', (e) => {
            // Allow backspace/delete without interference
            if (e.inputType === 'deleteContentBackward') return;

            let val = e.target.value;

            // Scenario 1: User typed a colon (e.g. "9:")
            // We respect it and just ensure they don't type multiple colons or letters
            if (val.includes(':')) {
                // Allow digits and exactly one colon
                const parts = val.split(':');
                const hours = parts[0].replace(/\D/g, '').slice(0, 2); // Max 2 digits for hours
                let minutes = parts[1] ? parts[1].replace(/\D/g, '') : '';
                
                if (minutes.length > 2) minutes = minutes.slice(0, 2); // Max 2 digits for minutes
            ...
            // Scenario 2: User is typing pure digits (e.g. "0900")
            // Apply standard HH:MM auto-masking
            val = val.replace(/\D/g, ''); // Remove non-digits
            
            if (val.length > 4) val = val.slice(0, 4); // Max 4 digits total
     ...
     * Includes a fallback parser for environments where CSP blocks 'eval'.
    ...
    const executeSalaryScript = (scriptBody, context) => {
        // 1. Attempt standard dynamic execution (Most flexible)
        try {
            ...
            // 2. Fallback Mode: If CSP blocks 'eval' or syntax is invalid
            // We manually parse the standard formula pattern (Base + Bonus over Threshold)
            // This allows the default formula to work without 'unsafe-eval' permissions.
            console.warn("Dynamic execution blocked by CSP. Switching to Safe Fallback Parser.");

            try {
                // Helper to extract numeric variable values from the script string
                // Matches: const/var/let name = number;
                ...
                // Extract standard variables used in the default template
                // Note: 'cap' is optional, defaults to Infinity if not found
                const base = extractVar('base');
                const bonus = extractVar('bonus');
                const threshold = extractVar('threshold');
                
                // Check for 'cap' specifically
                const capMatch = scriptBody.match(/(?:const|var|let)\s+cap\s*=\s*([\d\.]+)/);
                const cap = capMatch ? parseFloat(capMatch[1]) : Infinity;

                // Determine which context variable is driving the logic (Attendees vs Revenue)
                let actualValue = 0;
                if (scriptBody.includes('context.attendees')) {
                    actualValue = context.attendees || 0;
                } else if (scriptBody.includes('context.revenue')) {
                    actualValue = context.revenue || 0;
                } else if (scriptBody.includes('context.credits')) {
                    actualValue = context.credits || 0;
                }

                // Perform the standard calculation logic:
                // If value <= threshold, pay base.
                if (actualValue <= threshold) {
                    return base;
                }

                // Otherwise: Base + ((Value - Threshold) * Bonus), limited by Cap
                const calculatedTotal = base + ((actualValue - threshold) * bonus);
                return Math.min(calculatedTotal, cap);

            } catch (fallbackError) {
                // If both dynamic execution AND fallback parsing fail, log error and return 0
                console.error("Custom Formula Fallback Error:", fallbackError);
                return 0; 
            }
        }
    };
    ...
    const handleLogout = () => {
        // --- FIX: Cleanup Camera on Logout ---
        if (window.memberScanner) {
            // We use .catch and .finally to ensure logout proceeds even if the scanner errors out
            window.memberScanner.stop()
                .catch(err => console.warn("Scanner stop error on logout:", err))
                .finally(() => {
    ...
    const renderNav = () => {
        ...
        const isAdmin = isOwner || isSuperAdmin || isManager || isStaff;
        // Helper for "Owner-level" access
        const isOwnerOrManager = isOwner || isSuperAdmin || isManager;
    ...
    const renderFooterInfo = () => {
        const container = document.getElementById('studioFooterInfo');
        if (!container) return;
        ...
        const isMulti = sortedBranches.length > 1;

        const parts = sortedBranches.map(b => {
            const name = (appState.currentLanguage === 'zh-TW' && b.name_zh) ? b.name_zh : b.name;
            const addr = (appState.currentLanguage === 'zh-TW' && b.address_zh) ? b.address_zh : b.address;
            const phone = b.phone ? formatDisplayPhoneNumber(b.phone) : '';

            if (!addr && !phone) return null;

            const details = [addr, phone].filter(Boolean).join(' ');
            
            let content = '';
            if (isMulti) {
                // Uses new CSS class
                content = `<strong class="footer-branch-name">${name}:</strong> ${details}`;
            } else {
                content = details;
            }

            return `<span class="block sm:inline">${content}</span>`;
        }).filter(Boolean);

        // Uses new CSS class
        container.innerHTML = parts.join('<span class="footer-separator">|</span>');
    };

    const renderSubscriptionBadge = () => {
        // 1. Find the Logo container
        const headerLink = DOMElements.appWrapper.querySelector('header a');
        if (!headerLink) return;

        // Redirect to Schedule on Click
        headerLink.onclick = (e) => {
            e.preventDefault(); 
            switchPage('schedule');
        };

        // 2. Clean up any existing badge
        const existingBadge = headerLink.querySelector('.plan-badge');
        if (existingBadge) existingBadge.remove();

        // 3. Permissions Check
        const role = appState.currentUser?.role;
        const allowedRoles = ['owner', 'manager', 'superadmin', 'staff'];
        
        if (!allowedRoles.includes(role)) return;

        // 4. Create the Badge Element
        const badge = document.createElement('span');
        badge.className = 'plan-badge ml-2 px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider align-middle select-none shadow-sm';
        
        // 5. Logic Branching
        
        // --- A. SUPER ADMIN (Clean) ---
        if (role === 'superadmin') {
            // No more style injection!
        ...
        // --- D. OWNER (Clean) ---
            ...
            if (plan === 'premium') {
                // No more style injection!
                badge.classList.add('badge-premium-shimmer');
                badge.textContent = 'PREMIUM';

            } else if (plan === 'pro') {
                badge.classList.add('badge-pro', 'shadow-sm');
    ...
    const updateUIVisibility = () => {
        ...
        // Expanded isAdmin definition to include Manager and SuperAdmin
        const role = appState.currentUser?.role;
        const isAdmin = role === 'owner' || role === 'superadmin' || role === 'manager' || role === 'staff';
    ...
    const renderCurrentPage = () => {
        ...
        const isOwner = appState.currentUser?.role === 'owner';
        const isSuperAdmin = appState.currentUser?.role === 'superadmin'; // NEW
        const isManager = appState.currentUser?.role === 'manager';
        const isStaff = appState.currentUser?.role === 'staff';
        
        // Helper: Treat SuperAdmin as Owner for routing purposes
        const isOwnerLevel = isOwner || isSuperAdmin; 
        ...
        // (Owners/SuperAdmins can always access to upgrade/manage)
        if (plan === 'basic' && !isOwnerLevel) {
        ...
        // If a Staff member tries to access an owner-only page (Manager & SuperAdmin are EXEMPT)
        if (isStaff && !isManager && !isSuperAdmin && ownerOnlyPages.includes(pageIdToRender)) {
            pageIdToRender = 'schedule';
        }
        
        // Rule C: Non-Admin Guard
        if (!isOwnerLevel && !isManager && !isStaff && (ownerOnlyPages.includes(pageIdToRender) || adminPages.includes(pageIdToRender))) {
            pageIdToRender = 'schedule';
        }
        
        // Rule D: Member Guard
        if ((isOwnerLevel || isManager || isStaff) && memberOnlyPages.includes(pageIdToRender)) {
            pageIdToRender = 'schedule';
        }
        ...
        if (pageIdToRender === 'schedule') {
            if (isOwnerLevel || isManager || isStaff) {
    ...
    function createWhatsAppMessage(member, cls) {
        ...
        const bookingLink = `${dynamicOrigin}/${appState.studioProfile.slug}`;
...
${_('whatsapp_closing')}
    ...
    function createUpdateWhatsAppMessage(member, originalCls, newClsData) {
        ...
        const bookingLink = `${window.location.origin}/${appState.studioProfile.slug}`;
...
${_('whatsapp_closing')}
    ...
    function handleClsUpdateRequest(originalCls, newClsData, onCancel = null) {
        ...
        const performUpdate = () => {
            ...
            database.ref(`/classes/${originalCls.id}`).set(finalClsData) // Use set() to ensure a clean update
                .then(() => {
                    ...
                    // --- FIX: Refresh Classes Page Table ---
                    if (appState.activePage === 'classes') {
        ...
        if (bookedMemberIds.length === 0 || !hasRelevantChanges) {
            ...
            openUpdateNotifyModal(originalCls, newClsData, performUpdate, onCancel); 
        }
    }
    ...
    async function openJoinedMembersModal(cls) {
        ...
        // --- UPDATED: Allow SuperAdmin & Manager to see Revenue ---
        const isOwner = appState.currentUser?.role === 'owner';
        const isSuperAdmin = appState.currentUser?.role === 'superadmin'; // NEW
        const isManager = appState.currentUser?.role === 'manager';
        
        const canViewRevenue = isOwner || isSuperAdmin || isManager;
        // ----------------------------------------------------------
        ...
        const translatedSubtitle = _('template_class_on_date')
            ...
            .replace('{time}', getTimeRange(currentCls.time, currentCls.duration));
        ...
        // --- NEW: Conditionally generate Check-In Note & Header Spacing ---
        const hasBookings = bookedMemberIds.length > 0;
        
        // Use 'mb-1' if note is shown, otherwise restore original 'mb-4'
        const headerMarginClass = hasBookings ? 'mb-1' : 'mb-4';

        const checkInNoteHTML = hasBookings ? `
            <p class="text-xs text-slate-500 mb-4 font-medium flex items-center gap-1.5">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-indigo-600" viewBox="0 0 20 20" fill="currentColor">
                    <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd" />
                </svg>
                ${_('note_checkin_indicator')}
            </p>` : '';
        // ------------------------------------------------------------------
        ...
        DOMElements.joinedMembersModal.innerHTML = `
            <div class="bg-white p-8 rounded-2xl shadow-2xl w-full max-w-2xl transform transition-all duration-300 scale-95 opacity-0 modal-content relative">
                ...
                <!-- FIX: Use dynamic margin class here -->
                <h3 class="text-xl font-bold text-slate-700 ${headerMarginClass}">${_('header_booked_members')}</h3>
        ...
        if (bookedMembers.length === 0) {
            ...
            // --- NEW: Safe Window Calculation ---
            const classStartDateTime = new Date(`${currentCls.date}T${currentCls.time}`);
            const now = new Date();

            // 1. Get Settings
            const settings = appState.studioSettings.clsDefaults || {};
            const cutoffHours = settings.cancellationCutoff || 0;
            const physicalWindowHours = 3; // Hardcoded 3 hours rule

            // 2. Calculate Thresholds
            const cancelCutoffTime = new Date(classStartDateTime.getTime() - (cutoffHours * 60 * 60 * 1000));
            const physicalWindowTime = new Date(classStartDateTime.getTime() - (physicalWindowHours * 60 * 60 * 1000));

            // 3. Determine Safe Start Time (The LATER of the two)
            // Logic: Window opens only when BOTH conditions are met.
            const safeCheckInTime = (cancelCutoffTime > physicalWindowTime) ? cancelCutoffTime : physicalWindowTime;
            
            const isSafeWindowOpen = now >= safeCheckInTime;
            
            // CHANGED: Format now includes Date + Time (e.g., "Nov 15 - 14:00")
            const formattedOpenTime = formatShortDateWithYear(safeCheckInTime.toISOString()) + ' ' + formatTime(safeCheckInTime);

            listEl.innerHTML = bookedMembers.map(member => {
                const isAttended = currentCls.attendedBy && currentCls.attendedBy[member.id];
                
                // Determine if this specific checkbox should be locked
                // It is locked if: The window is NOT open AND they are not already checked in
                const isLocked = !isSafeWindowOpen && !isAttended;
                
                const checkboxClass = isLocked ? 'locked-checkbox' : 'attendance-checkbox';
                // If locked, we don't use 'checked' attribute here, we manage visually via CSS
                const checkedState = isAttended ? 'checked' : '';

                return `<div class="bg-slate-100 p-3 rounded-lg flex justify-between items-center mb-2">
                    <div class="flex items-center min-w-0 flex-1 mr-2">
                       <input type="checkbox" 
                              data-member-id="${member.id}" 
                              class="h-5 w-5 rounded text-indigo-600 mr-4 flex-shrink-0 ${checkboxClass}" 
                              ${checkedState}
                              ${isLocked ? 'readonly' : ''} 
                       >
                       <div class="min-w-0">
                    ...
                    <p class="text-sm text-slate-600 whitespace-nowrap flex-shrink-0">${formatDisplayPhoneNumber(member.phone)}</p>
                </div>`;
            }).join('');

            // --- Handler 1: Standard Checkboxes (Normal behavior) ---
            ...
            // --- Handler 2: Locked Checkboxes (5-Tap Secret Logic) ---
            listEl.querySelectorAll('.locked-checkbox').forEach(checkbox => {
                let tapCount = 0;
                let tapResetTimer;

                checkbox.addEventListener('click', (e) => {
                    e.preventDefault(); // Always block the native toggle
                    
                    tapCount++;
                    clearTimeout(tapResetTimer);

                    // Reset the counter if the user stops clicking for 500ms
                    ...
                    if (tapCount >= 5) {
                        ...
                        // FIX: Rely on the CLASS state, not the DOM 'checked' property
                        // If 'locked-checkbox' is gone, it means we successfully checked them in previously (Undo Mode)
                        const isAlreadyForcedIn = !checkbox.classList.contains('locked-checkbox');

                        if (isAlreadyForcedIn) {
                            // --- CASE A: ALREADY FORCED IN -> UNDO (Revert) ---
                            showConfirmation(
                                _('title_cancel_booking'), 
                                _('confirm_undo_force_checkin'),
                                () => {
                                    if (currentCls.attendedBy) delete currentCls.attendedBy[memberId];
                                    attendedRef.remove().then(() => {
                                        // Update UI: Uncheck and re-lock styling
                                        checkbox.checked = false;
                                        checkbox.classList.add('locked-checkbox');
                                        checkbox.classList.remove('attendance-checkbox');
                                        ...
                                        showMessageBox(_('info_checkin_reverted'), 'info');
                                    });
                                }
                            );
                        } else {
                            // --- CASE B: STILL LOCKED -> FORCE CHECK-IN ---
                            showConfirmation(
                                _('title_confirm_spot'),
                                _('warning_force_checkin'),
                                () => {
                                    if (!currentCls.attendedBy) currentCls.attendedBy = {};
                                    currentCls.attendedBy[memberId] = true;
                                    attendedRef.set(true).then(() => {
                                        // Update UI: Check and visually unlock
                    ...
                    // FEEDBACK: 1st Click Only (Only show if still locked)
                    else if (tapCount === 1 && checkbox.classList.contains('locked-checkbox')) {
                        showMessageBox(_('info_checkin_opens_at').replace('{date}', formattedOpenTime), 'info');
                    }
                });
            });
        }
        ...
        addMemberSearchInput.oninput = () => {
            ...
            searchDebounce = setTimeout(async () => {
                ...
                const unbookedMembers = appState.users.filter(u => 
                    // FIX: Added 'superadmin' to exclusion list
                    u.role !== 'owner' && u.role !== 'superadmin' && u.role !== 'manager' && u.role !== 'staff' && !u.isDeleted &&
        ...
        addMemberSearchResults.onclick = (e) => {
            ...
            if (target) {
                ...
                if (memberToAdd.monthlyPlan) {
                    ...
                    // A2. Check Start Date (BUG FIX)
                    const startDate = memberToAdd.planStartDate ? new Date(memberToAdd.planStartDate) : null;
                    if(startDate) startDate.setHours(0,0,0,0);
                    const isPlanNotStarted = startDate && startDate > today;

                    if (isPlanOverdue) {
                        failureReason = 'plan_expired';
                        // Fall through to try credits
                    } else if (isPlanNotStarted) {
                        failureReason = 'plan_not_started';
                        // Fall through to try credits
                    } else {
                ...
                if (paymentMethod === 'none') {
                    ...
                    } else if (failureReason === 'plan_not_started') {
                        // Custom Message for Not Started
                        const startStr = memberToAdd.planStartDate || '';
                        const dateMsg = _('info_plan_not_started').replace('{date}', startStr);
                        showMessageBox(`${dateMsg} & ${_('error_member_insufficient_credits')} (${typeName})`, 'error');
    ...
    function openBookingModal(cls) {
        ...
        if (currentUser.monthlyPlan) {
            ...
            // --- BUG FIX: Start Date Check ---
            const planStartDate = currentUser.planStartDate ? new Date(currentUser.planStartDate) : null;
            if (planStartDate) planStartDate.setHours(0,0,0,0);
            const isPlanNotStarted = planStartDate && planStartDate > today;

            if (isPlanExpired) {
                planIneligibilityReason = 'plan_expired';
            } else if (isPlanNotStarted) {
                planIneligibilityReason = 'plan_not_started';
            } else {
            ...
            if (currentUser.monthlyPlan && planIneligibilityReason) {
                ...
                if (planIneligibilityReason === 'plan_expired') {
                    if (paymentMethod === 'credit') {
                        ...
                        noteColorClass = 'text-blue-600 bg-blue-50 border-blue-100';
                    } else {
                        reasonText = _('error_plan_overdue_no_credits');
                        noteColorClass = 'text-red-600 bg-red-50 border-red-100';
                    }
                }
                // --- NEW CASE ---
                else if (planIneligibilityReason === 'plan_not_started') {
                    const startStr = currentUser.planStartDate ? formatShortDateWithYear(currentUser.planStartDate) : '';
                    const dateMsg = _('info_plan_not_started').replace('{date}', startStr);

                    if (paymentMethod === 'credit') {
                        reasonText = `${dateMsg}. ${_('label_using_credits')}.`;
                    } else {
                        reasonText = `${dateMsg}. ${_('status_no_credits')}`;
                    }
                }
            ...
            <div class="bg-white p-0 rounded-2xl shadow-2xl w-full max-w-md transform transition-all duration-300 scale-95 opacity-0 modal-content overflow-hidden">
                ...
                <div class="p-8 space-y-4">
                    ...
                    <div class="flex justify-between items-center">
                        ...
                        <div class="text-right whitespace-nowrap">${paymentMethodHtml}</div>
                    </div>
                    ${noteHtml}
        ...
        confirmBtn.onclick = () => {
            ...
            if (cls.bookedBy && cls.bookedBy[memberId]) {
                ...
                // 1. Monthly Plan Expired
                if (currentUser.monthlyPlan && planIneligibilityReason === 'plan_expired') {
                    showMessageBox(_('error_plan_overdue_no_credits'), 'error');
                }
                // 1.5 Monthly Plan Not Started (NEW)
                else if (currentUser.monthlyPlan && planIneligibilityReason === 'plan_not_started') {
                    const startStr = currentUser.planStartDate ? formatShortDateWithYear(currentUser.planStartDate) : '';
                    const dateMsg = _('info_plan_not_started').replace('{date}', startStr);
                    showMessageBox(`${dateMsg}. ${_('status_no_credits')}`, 'error');
                }
                // 2. Credits Expired
    ...
    function openClsModal(dateIso, clsToEdit = null) {
        ...
        // --- NEW: Use Helper for Auto-colon ---
        setupTimeInputMask(form.querySelector('#clsTime'));
        // --------------------------------------
        ...
        if (clsToEdit) {
            ...
            // --- FIX: Store full class object for submit handler fallback ---
            form.dataset.originalClsString = JSON.stringify(clsToEdit);
            // ----------------------------------------------------------------
            ...
            // --- FIX: Clean up dataset ---
            delete form.dataset.originalClsString;
            // -----------------------------
    ...
    function handleClsFormSubmit(e) {
        ...
        const branchId = form.querySelector('#clsBranchId').value;
        ...
        // --- NEW: Use Helper for Normalization ---
        const time = normalizeTimeInput(form.querySelector('#clsTime').value);
        // ----------------------------------------
        ...
        const proceedWithSave = () => {
            ...
            if (clsId) {
                // --- FIX: Retrieve original class with fallback to dataset ---
                let originalCls = appState.classes.find(c => c.id === clsId);
                
                if (!originalCls && form.dataset.originalClsString) {
                    try {
                        originalCls = JSON.parse(form.dataset.originalClsString);
                    } catch(e) {
                        console.error("Failed to parse original class data from form", e);
                    }
                }
                // -------------------------------------------------------------
    ...
    function createClsElement(cls) {
        ...
        // SuperAdmin & Owner always have write permission. Check restricts others.
    ...
    function _reSortDayColumn(dateIso) {
        ...
        if (dailyClasses.length > 0) {
            ...
            // --- NEW: Check if we are in a "View Only" context to show placeholder ---
            // If the "Add Class" button is NOT present for this day, assume Member/Restricted view
            const columnWrapper = dayContainer.closest('.day-column');
            const hasAddButton = columnWrapper ? columnWrapper.querySelector('.add-cls-button') : false;

            if (!hasAddButton) {
                dayContainer.innerHTML = `
                    <div class="no-class-placeholder">
                        <span class="text-sm font-bold text-slate-400 select-none">${_('info_no_class_today')}</span>
                    </div>`;
            }
        }
    }
    ...
    function _renderScheduleCarousel(container, startDate, endDate, datesArray, initialScrollIndex, showAddButton, filteredClasses = null) {
        ...
        container.querySelectorAll('.classes-container').forEach(cont => {
            ...
            if (dailyClasses.length > 0) {
                ...
                // --- NEW: No Class Placeholder (Clean CSS) ---
                cont.innerHTML = `
                    <div class="no-class-placeholder">
                        <span class="text-sm font-bold text-slate-400 select-none">${_('info_no_class_today')}</span>
                    </div>`;
        ...
        // Include SuperAdmin in Admin check
        const isAdmin = isOwner || isSuperAdmin || isManager || isStaff; 
    ...
    function renderOwnerSchedule(container) {
        ...
        // Only apply restrictions if NOT Owner AND NOT SuperAdmin
    ...
    function openFilterModal() {
        ...
        // --- FIXED: Waterfall Logic ---
        const updateFilterDependencies = () => {
            const selectedDay = dayFilter.value;
            // FIX: If value is empty string (initial state), treat as 'all'
            const selectedSport = sportTypeFilter.value || 'all';
            const selectedTutor = tutorFilter.value || 'all';

            // 1. Base Filter: Filter All Classes by Selected Day
            ...
            // 2. Determine Available Sports (Waterfall from Tutor)
            // If a specific Tutor is selected, only show Sports they are teaching on this day.
            let classesForSports = dayFilteredClasses;
            if (selectedTutor !== 'all') {
                classesForSports = classesForSports.filter(c => c.tutorId === selectedTutor);
            }
            const relevantSportTypeIds = new Set(classesForSports.map(c => c.sportTypeId));
            const availableSportTypes = appState.sportTypes.filter(st => relevantSportTypeIds.has(st.id));
            
            // 3. Determine Available Tutors (Waterfall from Sport)
            // If a specific Sport is selected, only show Tutors teaching that Sport on this day.
            let classesForTutors = dayFilteredClasses;
            if (selectedSport !== 'all') {
                classesForTutors = classesForTutors.filter(c => c.sportTypeId === selectedSport);
            }
            const relevantTutorIds = new Set(classesForTutors.map(c => c.tutorId));
            ...
            // Populate Tutors
            // Note: We pass 'all' as the 2nd arg to bypass internal skill checks, 
            // relying purely on the schedule-based 'availableTutors' list we just calculated.
            populateTutorFilter(tutorFilter, 'all', availableTutors); 
            if (selectedTutor !== 'all' && relevantTutorIds.has(selectedTutor)) {
        ...
        // --- FIX: Initialize lists based on current state with explicit options ---
        // We set the innerHTML to a temporary option matching the saved state.
        // This ensures sportTypeFilter.value returns the correct ID (or 'all') when 
        // updateFilterDependencies runs its initial pass, preventing it from reading "".
        sportTypeFilter.innerHTML = `<option value="${memberSportType}"></option>`;
        sportTypeFilter.value = memberSportType;
        
        tutorFilter.innerHTML = `<option value="${memberTutor}"></option>`;
        ...
        tutorFilter.onchange = updateFilterDependencies;
    ...
    function openGoToDateModal() {
        ...
        form.onsubmit = (e) => {
            ...
            // Use UTC components to calculate exact calendar day difference
            const today = new Date();
            const todayUtc = Date.UTC(today.getFullYear(), today.getMonth(), today.getDate());
            
            // Parse target components explicitly to avoid browser parsing inconsistencies
            // targetDateIso is YYYY-MM-DD
            const [tYear, tMonth, tDay] = targetDateIso.split('-').map(Number);
            const targetUtc = Date.UTC(tYear, tMonth - 1, tDay);
            
            // Calculate difference in days (always integer)
            const daysAgo = Math.floor((todayUtc - targetUtc) / (1000 * 60 * 60 * 24));

            // Logic: If target is older than or equal to current visibility limit, extend range.
            // >= ensures we trigger even if the target is exactly at the edge (e.g., 1 day ago when 0 are visible).
            if (daysAgo > 0 && daysAgo >= appState.ownerPastDaysVisible) {
        ...
        const renderUpcomingList = (targetContainerId) => {
            ...
            if (todaysUnattendedBookings.length > 0) {
                ...
                const preSelectMsg = todaysUnattendedBookings.length > 1 
                    ? `<p class="text-center text-sm text-indigo-600 font-bold mb-3 px-2">${_(targetContainerId === 'upcomingCheckInSectionScan' ? 'check_in_preselect_prompt_scan' : 'check_in_preselect_prompt')}</p>` 
                    : '';
    ...
    /**
     * Generates the HTML for the "Consistency Heatmap" (GitHub style).
     * Updated: Includes Localized Day Labels (Sun-Sat) on the left side.
     */
    function renderConsistencyHeatmap(bookings, memberId) {
        // 1. Calculate Attendance Data
        const attendanceMap = {};
        let totalAttendedCount = 0;

        bookings.forEach(cls => {
        ...
        // 2. Setup Date Range (Last 24 Weeks)
        const today = new Date();
        ...
        // Adjust to previous Sunday (Start of grid)
        startDate.setDate(startDate.getDate() - startDate.getDay());

        // 3. Generate Month Labels (Smart Logic)
        let monthLabelsHTML = '';
        const monthsShort = _('months_short'); 
        ...
        for (let w = 0; w <= weeksToShow; w++) {
            ...
            const nextMonthIndex = nextWeek.getMonth();
            const prevWeek = new Date(weekStart);
            prevWeek.setDate(weekStart.getDate() - 7);
            const prevMonthIndex = prevWeek.getMonth();

            let label = '';
            
            // Show label if month changed, but skip first column if it changes immediately
            if (w === 0 && currentMonthIndex !== nextMonthIndex) {
                // Skip
            } else if (w === 0 || currentMonthIndex !== prevMonthIndex) {
                label = monthsShort[currentMonthIndex];
            }

            monthLabelsHTML += `<div class="heatmap-day-label">${label}</div>`;
        }

        // 4. Generate Grid Items (Dots)
        let gridHTML = '';
        ...
        while (loopDate <= today) {
            const isoDate = getIsoDate(loopDate);
            const count = attendanceMap[isoDate] || 0;
            const displayDate = formatShortDateWithYear(isoDate);
            
            // NEW: Use CSS Classes instead of Hex Codes
            let colorClass = 'heatmap-0';
            if (count === 1) colorClass = 'heatmap-1';
            else if (count === 2) colorClass = 'heatmap-2';
            else if (count === 3) colorClass = 'heatmap-3';
            else if (count >= 4) colorClass = 'heatmap-4';

            // Visual Separator for Month End
            const separatorStyle = isLastDayOfMonth(loopDate) 
                ? 'border-bottom: 2px solid #94a3b8; margin-bottom: -2px; z-index: 10;' 
                : '';

            const tooltipText = _('label_heatmap_tooltip').replace('{count}', count).replace('{date}', displayDate);

            // Clean HTML
            gridHTML += `<div class="heatmap-dot ${colorClass}" style="${separatorStyle}" title="${tooltipText}"></div>`;

            loopDate.setDate(loopDate.getDate() + 1);
        }

        // --- NEW: Generate Day Labels (Left Column) ---
        const dayLabels = _('days_short'); 
        
        const dayLabelsHTML = dayLabels.map(day => 
            // h-3 matches the dot height (12px) for perfect alignment
            `<div class="h-3 flex items-center justify-end text-[10px] text-slate-400 font-medium leading-none select-none">${day}</div>`
        ).join('');

        // --- NEW: Define Legend HTML here using CSS classes ---
        const legendHTML = `
            <div class="flex items-center justify-end gap-1.5 mt-3 text-xs text-slate-400 font-medium">
                <span class="mr-1">${_('label_less')}</span>
                <div class="heatmap-dot heatmap-0" title="0"></div>
                <div class="heatmap-dot heatmap-1" title="1"></div>
                <div class="heatmap-dot heatmap-2" title="2"></div>
                <div class="heatmap-dot heatmap-3" title="3"></div>
                <div class="heatmap-dot heatmap-4" title="4+"></div>
                <span class="ml-1">${_('label_more')}</span>
            </div>`;

        // 5. Build Container HTML
        return `
            <div class="card p-6 mb-8 border border-slate-200 shadow-sm">
                <div class="flex justify-between items-end mb-4">
                    <h3 class="text-xl font-bold text-slate-800 flex items-center gap-2">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-emerald-600" viewBox="0 0 20 20" fill="currentColor">
                            <path fill-rule="evenodd" d="M6 2a1 1 0 00-1 1v1H4a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V6a2 2 0 00-2-2h-1V3a1 1 0 10-2 0v1H7V3a1 1 0 00-1-1zm0 5a1 1 0 000 2h8a1 1 0 100-2H6z" clip-rule="evenodd" />
                        </svg>
                        ${_('header_consistency')}
                    </h3>
                    <span class="text-xs font-bold text-slate-500 bg-slate-100 px-2 py-1 rounded-full whitespace-nowrap">
                        ${_('label_classes_attended')}: ${totalAttendedCount}
                    </span>
                </div>
                ...
                <div class="flex">
                    ...
                    <div class="flex flex-col shrink-0 mr-2">
                        <div class="text-[10px] mb-1 invisible" aria-hidden="true">&nbsp;</div>
                        <div class="grid grid-rows-7 gap-[4px]">
                            ${dayLabelsHTML}
                        </div>
                    </div>

                    <!-- Right Column: Heatmap Scroll Container -->
                    <div id="heatmap-scroll-container" class="overflow-x-auto pb-2 scroll-smooth">
    ...
    async function renderAccountPage(container) {
        ...
        // --- START: Member Journey Calculation Logic ---
        const calculateJourneyStats = (bookings, userId) => {
            let attendedCount = 0;
            const sportCounts = {};
            const tutorCounts = {};
            
            // 1. Tally Totals & Favorites based on ATTENDED classes only
            bookings.forEach(cls => {
                if (cls.attendedBy && cls.attendedBy[userId]) {
                    attendedCount++;
                    
                    // Count Sport Types
                    sportCounts[cls.sportTypeId] = (sportCounts[cls.sportTypeId] || 0) + 1;
                    
                    // Count Tutors
                    tutorCounts[cls.tutorId] = (tutorCounts[cls.tutorId] || 0) + 1;
                }
            });

            // 2. Find Favorites (Mode)
            const getTopKey = (obj) => Object.keys(obj).reduce((a, b) => obj[a] > obj[b] ? a : b, null);
            
            const favSportId = getTopKey(sportCounts);
            const favTutorId = getTopKey(tutorCounts);
            
            const favSport = favSportId ? appState.sportTypes.find(s => s.id === favSportId) : null;
            const favTutor = favTutorId ? appState.tutors.find(t => t.id === favTutorId) : null;

            // 3. Define Badges
            const badges = [
                { threshold: 1, icon: '🌱', label: _('badge_newbie'), color: 'bg-green-100 text-green-700' },
                { threshold: 10, icon: '🔥', label: _('badge_regular'), color: 'bg-orange-100 text-orange-700' },
                { threshold: 25, icon: '🥉', label: _('badge_bronze'), color: 'bg-slate-100 text-slate-700' },
                { threshold: 50, icon: '🥈', label: _('badge_silver'), color: 'bg-gray-200 text-gray-800' },
                { threshold: 100, icon: '🥇', label: _('badge_gold'), color: 'bg-yellow-100 text-yellow-700' },
                { threshold: 250, icon: '👑', label: _('badge_legend'), color: 'bg-purple-100 text-purple-700' }
            ];

            // 4. Determine Status
            // Reverse to find the highest achieved first
            const currentBadge = badges.slice().reverse().find(b => attendedCount >= b.threshold) || null;
            // Find the first badge that is strictly higher than current count
            const nextBadge = badges.find(b => attendedCount < b.threshold); 

            return { attendedCount, favSport, favTutor, currentBadge, nextBadge };
        };

        const journeyStats = calculateJourneyStats(memberBookings, member.id);
        
        // --- Generate Journey HTML ---
        let statsCardHTML = '';
        if (journeyStats.attendedCount > 0) {
            const badgeIcon = journeyStats.currentBadge ? journeyStats.currentBadge.icon : '👋';
            const badgeLabel = journeyStats.currentBadge ? journeyStats.currentBadge.label : _('label_na');
            const badgeColor = journeyStats.currentBadge ? journeyStats.currentBadge.color : 'bg-slate-100 text-slate-600';
            
            // Calculate progress to next badge
            let progressHTML = '';
            if (journeyStats.nextBadge) {
                const nextThreshold = journeyStats.nextBadge.threshold;
                const prevThreshold = journeyStats.currentBadge ? journeyStats.currentBadge.threshold : 0;
                
                // Calculate logic for bar width
                const range = nextThreshold - prevThreshold;
                const progressInTier = journeyStats.attendedCount - prevThreshold;
                const percent = Math.min(100, Math.max(5, (progressInTier / range) * 100));
                
                const classesLeft = nextThreshold - journeyStats.attendedCount;
                
                progressHTML = `
                    <div class="mt-4 pt-4 border-t border-slate-100">
                        <!-- Updated: Increased margin-bottom from mb-1.5 to mb-3 for spacing -->
                        <div class="flex justify-between text-xs font-bold text-slate-500 mb-3 uppercase tracking-wide">
                            <span>${_('label_next_goal')}: ${journeyStats.nextBadge.label}</span>
                            <span>${_('label_more_classes').replace('{count}', classesLeft)}</span>
                        </div>
                        <div class="w-full bg-slate-100 rounded-full h-2.5 overflow-hidden">
                            <div class="bg-indigo-500 h-2.5 rounded-full transition-all duration-1000 ease-out" style="width: ${percent}%"></div>
                        </div>
                    </div>`;
            } else {
                // Max level reached
                progressHTML = `
                    <div class="mt-4 pt-4 border-t border-slate-100 text-center">
                        <span class="text-xs font-bold text-indigo-600 uppercase tracking-wide">✨ ${_('badge_legend')} ✨</span>
                    </div>`;
            }

            statsCardHTML = `
                <div class="card p-6 mb-8 border border-slate-200 shadow-sm relative overflow-hidden group">
                    <!-- Decorative Background Blob -->
                    <div class="absolute top-0 right-0 -mt-8 -mr-8 w-32 h-32 bg-indigo-50 rounded-full blur-2xl opacity-60 pointer-events-none group-hover:scale-110 transition-transform duration-700"></div>

                    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center relative z-10 gap-4">
                        <!-- Left: Badge & Status -->
                        <div class="flex items-center gap-4">
                            <div class="h-14 w-14 rounded-full flex items-center justify-center text-3xl shadow-sm border border-white ${badgeColor}">
                                ${badgeIcon}
                            </div>
                            <div>
                                <p class="text-xs text-slate-400 font-bold uppercase tracking-wider mb-0.5">${_('label_current_status')}</p>
                                <h3 class="text-xl font-extrabold text-slate-800 leading-none">${badgeLabel}</h3>
                                <p class="text-xs text-slate-500 mt-1 font-medium">${journeyStats.attendedCount} ${_('label_classes_attended')}</p>
                            </div>
                        </div>
                        
                        <!-- Right: Favorites -->
                        ${(journeyStats.favSport || journeyStats.favTutor) ? `
                        <div class="text-left sm:text-right bg-slate-50 sm:bg-transparent p-3 sm:p-0 rounded-lg w-full sm:w-auto mt-2 sm:mt-0">
                            <div class="text-[10px] text-slate-400 font-bold uppercase tracking-wider mb-1">${_('label_favorites')}</div>
                            <div class="text-sm text-slate-700">
                                ${journeyStats.favSport ? `
                                <span class="inline-block mr-3">
                                    <span class="text-[10px] text-slate-400 font-bold uppercase tracking-wide">${_('label_class')}:</span>
                                    <span class="font-bold">${getSportTypeName(journeyStats.favSport)}</span>
                                </span>` : ''}
                                
                                ${journeyStats.favTutor ? `
                                <span class="inline-block">
                                    <span class="text-[10px] text-slate-400 font-bold uppercase tracking-wide">${_('label_tutor')}:</span>
                                    <span class="font-bold">${journeyStats.favTutor.name}</span>
                                </span>` : ''}
                            </div>
                        </div>` : ''}
                    </div>
        ...
        const renderBookingList = () => {
            ...
            listContainer.innerHTML = paginatedBookings.map(cls => {
                ...
                return `<div class="${isHighlighted ? 'booking-highlight' : 'bg-slate-100'} p-4 rounded-lg flex justify-between items-center" data-cls-id="${cls.id}">
                    ...
                    ${(() => {
                        if (isAttended) return `<span class="text-sm font-semibold text-green-600 whitespace-nowrap">${_('status_completed')}</span>`;
        ...
        const heatmapHTML = renderConsistencyHeatmap(memberBookings, member.id);
        ...
        container.innerHTML = `
            <div class="w-full max-w-screen-lg mx-auto grid grid-cols-1 md:grid-cols-3 gap-8">
                ...
                <!-- Right Column: Heatmap & Bookings -->
                <div class="md:col-span-2">
                    <!-- 1. Journey Stats (Gamification) -->
                    ${statsCardHTML}

                    <!-- 2. Heatmap Card (Injected Here) -->
                    ${heatmapHTML}
        ...
        // --- NEW: Scroll Heatmap to End (Recent Dates) ---
        const heatmapScroll = container.querySelector('#heatmap-scroll-container');
        if (heatmapScroll) {
            // Using setTimeout ensures the DOM is fully rendered and layout is calculated
            setTimeout(() => {
                ...
                if (purchaseList.length > 0) {
                    purchContent.innerHTML = purchaseList.map(p => {
                        ...
                        if (p.lastModifiedBy) {
                            ...
                            auditMessage = `<span class="text-xs text-slate-500 mt-1 block">${_(actionKey).replace('{name}', p.lastModifiedBy).replace('{date}', formatShortDateWithYear(p.lastModifiedAt) + ' - ' + formatTime(new Date(p.lastModifiedAt)))}</span>`;
                        }
    ...
    function renderMembersPage(container) {
        // FIX: Exclude SuperAdmin from the count
        ...
        const updateTable = (searchTerm = '') => {
            ...
            const filteredUsers = appState.users.filter(u => {
                // FIX: Added 'superadmin' to exclusion list
                if (u.role === 'owner' || u.role === 'superadmin' || u.role === 'staff' || u.role === 'manager' || u.isDeleted) return false;

                // ... [Branch & Search checks remain same] ...
            ...
            const sortedUsers = filteredUsers.sort((a, b) => {
                ...
                if (key === 'credits') {
                    // Formula: (Plan Active? 100k : 0) + (Plan Cost) + (Total Credits)
                    const getScore = (u) => (u.monthlyPlan ? 100000 : 0) + (parseFloat(u.monthlyPlanAmount || 0)) + getWalletStatus(u).totalBalance;
            ...
            tableBody.innerHTML = paginatedUsers.map(member => {
                ...
                if (member.monthlyPlan) {
                    ...
                    displayRows.push({
                        planHtml: `<span class="text-xs font-bold px-2 py-1 rounded-full inline-block whitespace-nowrap" style="${pillStyle}">${formatCurrency(member.monthlyPlanAmount)}${_('label_mo')}${tierLabel}</span>`,
                        expiryHtml: dateHtml
                    });
                }
                ...
                if (entriesToShow.length > 0) {
                    entriesToShow.forEach(([typeId, data]) => {
                        ...
                        displayRows.push({
                            planHtml: `<span class="font-bold text-xs px-2.5 py-1 rounded-full inline-block whitespace-nowrap ${pillTextClass}" style="${pillStyle}" title="${name}">${formatCredits(currentBalance)} / ${formatCredits(initial)} ${name}</span>`,
                            expiryHtml: dateHtml
                        });
                    });
                    ...
                    displayRows.push({
                        planHtml: `<span class="font-bold text-xs px-2.5 py-1 rounded-full inline-block whitespace-nowrap" style="background-color: ${neutralColor}26; color: ${neutralColor};">0 ${_('label_credits')}</span>`,
                ...
                if (hasMultipleBranches) {
                    ...
                    branchRowHtml = `<td class="p-2 whitespace-nowrap text-sm">${bName}</td>`;
                }
                        ...
                        allClasses.forEach(cls => {
                            if (cls.bookedBy && cls.bookedBy[member.id]) {
                                ...
                                exportData.push({
                                    ...
                                    // UPDATED: Use i18n key for 'Member' label
                                    [_('export_header_booked_by')]: bookingInfo.bookedBy === 'member' ? _('label_member') : (bookingInfo.bookedBy || _('export_value_unknown'))
                                });
                            }
                        });
                    });
    ...
    function handleMemberDeletion(member) {
        ...
        // Archive Email
        updates[`/users/${memberId}/email`] = `deleted.${memberId}@${appState.studioProfile.slug}.app`;
        ...
        updates[`/users/${memberId}/wallet`] = null; // Clears all wallets (Credits/Monthly)
    ...
    function _renderMemberPurchaseHistory(member, container, historyIdInput, purchaseAmountInput, creditsInput, onEditStart, creditTypeSelect, expiryDateInput, editLabel) {
        container.innerHTML = ''; 
        ...
        if (purchaseHistory.length > 0) {
            ...
            container.innerHTML = `<div class="text-xs text-slate-400 mb-2 text-right font-medium">${fullText}</div>` + sortedHistory.map(p => {
                ...
                if (p.lastModifiedBy) {
                    ...
                    auditMessage = `<div class="text-[10px] text-slate-400 mt-1">${_(actionKey).replace('{name}', p.lastModifiedBy).replace('{date}', formatShortDateWithYear(p.lastModifiedAt) + ' - ' + formatTime(new Date(p.lastModifiedAt)))}</div>`;
                }

                return `
                    <div class="flex items-center gap-1 bg-slate-50 p-2 rounded-md transition ${isDeleted ? 'opacity-50' : ''}" data-history-item-id="${p.id}">
                        <div class="flex-grow cursor-pointer hover:bg-slate-200 p-1 rounded-md transition-colors">
                        ...
                        <button type="button" class="edit-history-btn text-indigo-400 hover:text-indigo-600 hover:bg-indigo-50 p-1.5 rounded-md transition ${isDeleted ? 'hidden' : ''}" title="${_('btn_edit')}">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                            </svg>
                        </button>

                        <button type="button" class="remove-history-btn text-red-400 hover:text-red-600 hover:bg-red-50 p-1.5 rounded-md font-bold text-lg leading-none transition ${isDeleted ? 'hidden' : ''}" data-history-id="${p.id}" title="${_('btn_delete')}">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" />
                            </svg>
                        </button>
        ...
        container.onclick = (e) => {
            ...
            const editTarget = e.target.closest('.edit-history-btn') || e.target.closest('[data-history-item-id] .flex-grow');
            ...
            if (removeTarget) {
                    ...
                    database.ref().update(updates)
                        .then(() => {
                            database.ref(`/users/${memberId}`).once('value', snapshot => {
                                ...
                                _renderMemberPurchaseHistory(updatedMember, container, historyIdInput, purchaseAmountInput, creditsInput, onEditStart, creditTypeSelect, expiryDateInput, editLabel);
                ...
                if (historyEntry) {
                    ...
                    if (editLabel) {
                        editLabel.innerHTML = _('info_editing_purchase_from').replace('{date}', formatShortDateWithYear(historyEntry.date));
    ...
    function _renderMemberPaymentHistory(member, container, historyIdInput, monthsPaidInput, paymentAmountInput, onEditStart, monthlyTierSelect, onPlanReset, editLabel) {
        container.innerHTML = ''; 
        ...
        if (paymentHistory.length > 0) {
            ...
            container.innerHTML = `<div class="text-xs text-slate-400 mb-2 text-right font-medium">${fullText}</div>` + sortedHistory.map(p => {
                ...
                if (p.lastModifiedBy) {
                    ...
                    auditMessage = `<span class="text-xs text-slate-500 mt-1">${_(actionKey).replace('{name}', p.lastModifiedBy).replace('{date}', formatShortDateWithYear(p.lastModifiedAt) + ' - ' + formatTime(new Date(p.lastModifiedAt)))}</span>`;
                }
        ...
        container.onclick = (e) => {
            ...
            if (removeTarget) {
                ...
                showConfirmation(
                    ...
                    updates[`/users/${memberId}/paymentHistory/${historyId}/status`] = 'deleted';
                    updates[`/users/${memberId}/paymentHistory/${historyId}/lastModifiedBy`] = appState.currentUser.name;
                    updates[`/users/${memberId}/paymentHistory/${historyId}/lastModifiedAt`] = new Date().toISOString();

                    updates[`/users/${memberId}/monthlyPlan`] = false;
                    updates[`/users/${memberId}/monthlyPlanAmount`] = null;
                    updates[`/users/${memberId}/planStartDate`] = null;
                    updates[`/users/${memberId}/paymentDueDate`] = null;
                    updates[`/users/${memberId}/monthlyPlanEstimatedAttendance`] = null;
                    updates[`/users/${memberId}/monthlyCreditValue`] = null;
                    updates[`/users/${memberId}/monthlyPlanTierId`] = null;

                    database.ref().update(updates)
                        .then(() => {
                            database.ref(`/users/${memberId}`).once('value', snapshot => {
                                const updatedMember = { id: snapshot.key, ...snapshot.val() };
                                _renderMemberPaymentHistory(updatedMember, container, historyIdInput, monthsPaidInput, paymentAmountInput, onEditStart, monthlyTierSelect, onPlanReset, editLabel);
                                showMessageBox(_('info_payment_entry_deleted'), 'info');
                                if (onPlanReset) onPlanReset(); 
                            });
                        })
                        .catch(error => {
                            showMessageBox(_('error_deleting_entry').replace('{error}', error.message), 'error');
                ...
                if (historyEntry) {
                    ...
                    if (editLabel) {
                        editLabel.innerHTML = _('info_editing_payment_from').replace('{date}', formatShortDateWithYear(historyEntry.date));
                        editLabel.classList.remove('hidden');
                    }
    ...
    function openMemberModal(memberToEdit) {
                ...
                <form id="memberForm" class="space-y-4">
                    ...
                    <div class="pt-4 border-t">
                        ...
                        <div id="creditFields" class="space-y-4">
                            <!-- NEW: Edit Mode Label for Credits -->
                            <div id="creditEditModeLabel" class="hidden text-sm font-bold text-orange-600 mb-2 p-2 bg-orange-50 rounded border border-orange-200 text-center"></div>
                            ...
                            <div>
                                ...
                                <div class="flex gap-2 mb-2">
                                    <button type="button" data-months="3" class="expiry-quick-select-btn flex-1 bg-slate-200 hover:bg-slate-300 text-slate-700 font-semibold py-1 px-2 rounded-md text-sm transition">${_('label_quick_select_3_months')}</button>
                                    <button type="button" data-months="6" class="expiry-quick-select-btn flex-1 bg-slate-200 hover:bg-slate-300 text-slate-700 font-semibold py-1 px-2 rounded-md text-sm transition">${_('label_quick_select_6_months')}</button>
                                    <button type="button" data-months="12" class="expiry-quick-select-btn flex-1 bg-slate-200 hover:bg-slate-300 text-slate-700 font-semibold py-1 px-2 rounded-md text-sm transition">${_('label_quick_select_1_year')}</button>
                                </div>
                        ...
                        <div id="monthlyPlanFields" class="hidden space-y-4">
                            ...
                            <div id="monthlyInputsContainer" class="space-y-4 transition-opacity duration-300">
                                <!-- NEW: Edit Mode Label for Monthly -->
                                <div id="paymentEditModeLabel" class="hidden text-sm font-bold text-orange-600 mb-2 p-2 bg-orange-50 rounded border border-orange-200 text-center"></div>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        ...
                                        <input type="number" id="monthlyPlanAmount" class="form-input" min="0" placeholder="${_('placeholder_eg_1500')}">
                                    </div>
                                </div>
        ...
        if (monthlyTiers.length > 0) { 
            ...
            tierOptionsHTML = `<option value="" disabled selected>${_('info_no_monthly_tiers_found')}</option>`;
        }
        
        monthlyTierSelect.innerHTML = tierOptionsHTML;
        monthlyTierSelect.parentElement.classList.remove('hidden');
        monthlyTierSelect.required = true;
        ...
        const memberHasGeneral = memberToEdit.credits !== undefined || (memberToEdit.wallet && memberToEdit.wallet.general);
        const showGeneralOption = hasGeneralCredits || memberHasGeneral;

        const placeholderText = (creditTypes.length === 0 && !showGeneralOption)
            ? _('info_no_credit_types_found')
            : _('placeholder_select_credit_type');

        let optionsHTML = `<option value="" disabled selected>${placeholderText}</option>`;
        
        if (showGeneralOption) { 
            optionsHTML += `<option value="general">${_('label_general_credits_default')}</option>`; 
        }
        ...
        const creditEditModeLabel = form.querySelector('#creditEditModeLabel');
        const paymentEditModeLabel = form.querySelector('#paymentEditModeLabel');
        ...
        const setCreditButtonToAddMode = () => {
            ...
            purchaseAmountInput.value = ''; creditsInput.value = ''; historyIdInput.value = ''; expiryDateInput.value = '';
            ...
            creditEditModeLabel.classList.add('hidden');
            creditEditModeLabel.textContent = '';
        };
        ...
        const refreshMemberData = () => {
            ...
            database.ref(`/users/${memberToEdit.id}`).once('value', snapshot => {
                ...
                _renderMemberPurchaseHistory(memberToEdit, historyContainer, historyIdInput, purchaseAmountInput, creditsInput, setCreditButtonToEditMode, creditTypeSelect, expiryDateInput, creditEditModeLabel);
        ...
        creditActionBtn.onclick = async () => {
             ...
             if (!selectedTypeId) { showMessageBox(_('error_select_credit_type'), 'error'); return; }
             if (isNaN(amount) || isNaN(credits) || amount < 0 || credits < 0) { showMessageBox(_('error_invalid_amount_and_credits'), 'error'); return; }
             if (!expiryDate) { showMessageBox(_('error_expiry_date_required'), 'error'); return; }
             ...
             try {
                 ...
                 if(historyId) {
                     ...
                     if (!originalEntry || originalEntry.status === 'deleted') throw new Error("Entry invalid."); 
                     ...
                     if (oldTypeId === newTypeId) {
                        ...
                        if (finalOldBal === 0 && finalOldInit === 0) updates[`/users/${memberId}/wallet/${oldTypeId}`] = null;
                        else {
        ...
        const setPaymentButtonToAddMode = () => {
            ...
            paymentEditModeLabel.classList.add('hidden');
            paymentEditModeLabel.textContent = '';
        };
        ...
        paymentActionBtn.onclick = () => {
            ...
            if (!monthlyTierSelect.checkValidity()) {
                monthlyTierSelect.reportValidity(); 
                return;
            }
            ...
            // --- START: NEW DUAL UPDATE LOGIC ---
            
            // 1. Gather Profile Data (To activate plan immediately)
            const planAmountVal = parseFloat(monthlyPlanAmountInput.value) || 0;
            const estAttendanceVal = parseInt(estimatedAttendanceInput.value) || 0;
            const calculatedCreditVal = (planAmountVal > 0 && estAttendanceVal > 0) ? (planAmountVal / estAttendanceVal) : 0;
            const planStart = planStartDateInput.value;
            const paymentDue = paymentDueDateInput.value;

            // 2. Prepare Updates Object
            const updates = {};

            // A. Profile Updates (Force Monthly Plan = Active)
            updates[`/users/${memberId}/monthlyPlan`] = true;
            updates[`/users/${memberId}/monthlyPlanAmount`] = planAmountVal;
            updates[`/users/${memberId}/planStartDate`] = planStart;
            updates[`/users/${memberId}/paymentDueDate`] = paymentDue;
            updates[`/users/${memberId}/monthlyPlanEstimatedAttendance`] = estAttendanceVal;
            updates[`/users/${memberId}/monthlyCreditValue`] = calculatedCreditVal;
            updates[`/users/${memberId}/monthlyPlanTierId`] = currentTierId;

            // B. History Entry Data
            const entryData = { 
                monthsPaid, 
                amount, 
            ...
            if (historyId) {
                // Edit existing entry specific fields
                updates[`/users/${memberId}/paymentHistory/${historyId}/monthsPaid`] = monthsPaid;
                updates[`/users/${memberId}/paymentHistory/${historyId}/amount`] = amount;
                updates[`/users/${memberId}/paymentHistory/${historyId}/monthlyPlanTierId`] = currentTierId;
                updates[`/users/${memberId}/paymentHistory/${historyId}/lastModifiedBy`] = appState.currentUser.name;
                updates[`/users/${memberId}/paymentHistory/${historyId}/lastModifiedAt`] = new Date().toISOString();
            } else {
                // Add new entry
                const newKey = database.ref(`/users/${memberId}/paymentHistory`).push().key;
                updates[`/users/${memberId}/paymentHistory/${newKey}`] = {
                    ...entryData, 
            ...
            database.ref().update(updates).then(() => {
                const msg = historyId ? _('success_payment_entry_updated') : _('success_payment_entry_added');
                showMessageBox(msg, 'success');
                
                // Visual feedback: ensure toggle looks on
                enableMonthlyCheckbox.checked = true;
                monthlyInputsContainer.classList.remove('opacity-50', 'pointer-events-none');

                // Refresh Data
                database.ref(`/users/${memberId}`).once('value', s => { 
                    memberToEdit = {id:s.key,...s.val()}; 
                    _renderMemberPaymentHistory(memberToEdit, paymentHistoryContainer, paymentHistoryIdInput, monthsPaidInput, paymentAmountInput, setPaymentButtonToEditMode, monthlyTierSelect, resetMonthlyFormUI, paymentEditModeLabel); 
                    setPaymentButtonToAddMode(); 
                });
            }).catch(e => showMessageBox(e.message, 'error'));
            // --- END: NEW DUAL UPDATE LOGIC ---
        };
        ...
        creditTypeSelect.onchange = () => {
            ...
            if (wallet[typeId] && wallet[typeId].expiryDate) { expiryDateInput.value = wallet[typeId].expiryDate; } else { expiryDateInput.value = ""; }
            if (typeId) { submitBtn.disabled = true; submitBtn.classList.add('opacity-50', 'cursor-not-allowed'); cancelCreditEditBtn.classList.remove('hidden'); } 
            else { setCreditButtonToAddMode(); }
        };

        enableMonthlyCheckbox.checked = !!memberToEdit.monthlyPlan;
        
        // --- START: Active Plan Lock Logic ---
        
        // 1. Calculate Active State
        const today = new Date();
        today.setHours(0,0,0,0);
        const currentDueDate = memberToEdit.paymentDueDate ? new Date(memberToEdit.paymentDueDate) : null;
        const isPlanActive = currentDueDate && currentDueDate >= today;

        // 2. Helper to show guidance modal
        const showLockedPlanModal = (dueDate) => {
            const dateStr = formatShortDateWithYear(dueDate.toISOString());
            const title = _('title_subscription_locked');
            const msgBody = _('desc_subscription_locked');
            
            const message = msgBody.replace('{date}', `<strong>${dateStr}</strong>`);
            ...
            DOMElements.confirmationModal.innerHTML = `
                <div class="bg-white p-8 rounded-2xl shadow-2xl w-full max-w-sm transform transition-all duration-300 scale-95 opacity-0 modal-content text-center">
                    <button class="modal-close-btn"><svg class="h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" /></svg></button>
                    <div class="mx-auto flex items-center justify-center h-12 w-12 rounded-full bg-orange-100 mb-4">
                        <svg class="h-6 w-6 text-orange-600" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
                        </svg>
                    </div>
                    <h2 class="text-xl font-bold text-slate-800 mb-2">${title}</h2>
                    <p class="text-slate-600 mb-6 text-sm leading-relaxed">${message}</p>
                    <button type="button" class="close-btn bg-slate-800 hover:bg-slate-900 text-white font-bold py-2.5 px-6 rounded-lg w-full transition shadow-md">${_('btn_understood')}</button>
                </div>`;
        ...
        const toggleMonthlyInputs = () => {
            if (enableMonthlyCheckbox.checked) {
                monthlyInputsContainer.classList.remove('opacity-50', 'pointer-events-none');
                monthlyTierSelect.disabled = false;
                monthlyTierSelect.required = true; 
                ...
                monthlyTierSelect.disabled = true;
                monthlyTierSelect.required = false; 
            }
        };

        // 4. Click Interceptor
        enableMonthlyCheckbox.onclick = (e) => {
            // Logic: If trying to UNCHECK (disable) AND plan is active -> BLOCK
            if (isPlanActive && !enableMonthlyCheckbox.checked) {
                e.preventDefault(); 
                enableMonthlyCheckbox.checked = true; // Revert visual state
                showLockedPlanModal(currentDueDate);
            } else {
                // Otherwise allow toggle (Enable plan, or Disable expired plan)
                toggleMonthlyInputs();
            }
        };

        const resetMonthlyFormUI = () => {
            enableMonthlyCheckbox.checked = false;
            monthlyPlanAmountInput.value = '';
            planStartDateInput.value = '';
            paymentDueDateInput.value = '';
            monthlyTierSelect.value = '';
            estimatedAttendanceInput.value = '';
            toggleMonthlyInputs();
        };
        // --- END: Active Plan Lock Logic ---
                ...
                const cardHTML = `
                    <div class="print-card" style="background: white; padding: 60px 40px; border-radius: 30px; text-align: center; border: 1px solid #e2e8f0; width: 100%; max-width: 450px; margin: auto; box-shadow: 0 20px 40px -10px rgba(0,0,0,0.1);">
                        ...
                        <div style="font-size: 16px; color: #94a3b8; font-weight: 600; letter-spacing: 0.5px;">
                            Station Mode • ${appState.studioProfile.name}
                        </div>
                    </div>
                `;
    ...
    function handleAdminSettingsSave(e) {
        ...
        const newDefaults = {
            // --- NEW: Use Helper for Normalization ---
            time: normalizeTimeInput(form.querySelector('#defaultTime').value),
            // ----------------------------------------
    ...
    async function renderAdminPage(container) {
        ...
        // FIX: Include isSuperAdmin in full access check
        const hasFullAccess = isOwner || isSuperAdmin || isManager;
                    ...
                    <div class="mb-6 p-4 rounded-lg text-white shadow-md border border-white/10 relative overflow-hidden" style="background-color: ${color}">
                        ...
                        <div class="relative z-10">
                            ...
                            <div class="mt-3 text-xs opacity-75 border-t border-white/20 pt-2 flex items-center gap-1">
                                ...
                                ${formatShortDateWithYear(currentAnnounce.postedAt)} - ${formatTime(new Date(currentAnnounce.postedAt))}
                            </div>
                        </div>
                    </div>`;
            }
        ...
        // --- START: NEW PERMISSION CARD LOGIC ---
        let permissionsCardHTML = '';
        
        // Condition: Owner (Non-Basic) OR SuperAdmin
        if ((isOwner && plan !== 'basic') || isSuperAdmin) {
            
            // --- A. SUPERADMIN EXCLUSIVE: OWNERS LIST ---
            let ownersListHTML = '';
            if (isSuperAdmin) {
                const owners = appState.users.filter(u => u.role === 'owner');
                
                if (owners.length > 0) {
                    const ownerRows = owners.map(u => {
                        const currentPlan = (u.subscriptionStatus || 'basic').toLowerCase();
                        
                        // Badge Styling Helper
                        let badgeClass = 'bg-slate-200 text-slate-500'; // Basic
                        let badgeText = 'BASIC';
                        
                        if (currentPlan === 'premium') {
                            badgeClass = 'badge-premium shadow-sm'; // Uses CSS class
                            badgeText = 'PREMIUM';
                        } else if (currentPlan === 'pro') {
                            badgeClass = 'badge-pro shadow-sm'; // Uses CSS class
                            badgeText = 'PRO';
                        }

                        return `
                        <div class="flex flex-col sm:flex-row justify-between items-center bg-indigo-50/50 p-3 rounded-lg border border-indigo-100 gap-3 mb-2">
                            <div class="flex items-center gap-3 w-full sm:w-auto">
                                <div class="h-10 w-10 rounded-full bg-indigo-600 flex items-center justify-center text-white font-bold flex-shrink-0 shadow-sm">
                                ...
                                <div class="min-w-0">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span class="text-[10px] uppercase font-bold px-2 py-0.5 rounded-full ${badgeClass}">${badgeText}</span>
                                    </div>
                                    <p class="text-xs text-slate-500 truncate">${u.email}</p>
                                </div>
                            </div>
                            <button class="manage-plan-btn w-full sm:w-auto bg-white border border-indigo-200 text-indigo-700 hover:bg-indigo-600 hover:text-white font-semibold py-1.5 px-4 rounded-md text-sm transition shadow-sm" data-id="${u.id}">
                                ${_('btn_manage_plan')}
                            </button>
                        </div>`;
                    }).join('');
                    ...
                    ownersListHTML = `
                        <div class="mb-6">
                            <h4 class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3">${_('header_studio_owner')}</h4>
                            ${ownerRows}
                        </div>
        ...
        // FIX: Allow SuperAdmin to see the 3-column layout
        const gridClassesBottom = (isOwner || isSuperAdmin) ? 'grid-cols-1 md:grid-cols-2 lg:grid-cols-3' : 'grid-cols-1 md:grid-cols-2';
        ...
        container.innerHTML = `
            <div class="space-y-8">
                ...
                <div class="grid ${gridClassesBottom} gap-8 items-start">
                    ...
                    <!-- Only Owner/SuperAdmin sees Locations -->
        ...
        try {
            ...
            if (isOwner || isSuperAdmin) {
                // ... [Toggle Info Btn Logic remains same] ...
                ...
                if (toggleInfoBtn && infoBox) {
                    toggleInfoBtn.onclick = () => infoBox.classList.toggle('hidden');
                }

                // ... [Team List Logic remains same] ...
                ...
                // --- NEW: Owner Plan Management Listener (SuperAdmin Only) ---
                if (isSuperAdmin) {
                    const planBtns = container.querySelectorAll('.manage-plan-btn');
                    planBtns.forEach(btn => {
                        btn.onclick = () => {
                            const ownerId = btn.dataset.id;
                            const owner = appState.users.find(u => u.id === ownerId);
                            if (owner) openChangePlanModal(owner);
                        };
                    });
                }
            }
            ...
            if (settingsForm) {
                ...
                if (inputTime) {
                    inputTime.value = defaults.time || '';
                    setupTimeInputMask(inputTime); // <--- Apply Helper Here
                }
    ...
    function renderAdminLists() {
        ...
        const PLAN_LIMITS = {
            ...
            premium: { creditTypes: Infinity, monthlyTiers: Infinity, branches: 2, formulas: 10 }
        };

        let currentLimits = PLAN_LIMITS[plan] || PLAN_LIMITS.basic;

        // --- SUPERADMIN BYPASS ---
        // Force maximum limits regardless of the actual plan
        if (appState.currentUser?.role === 'superadmin') {
            currentLimits = { 
                creditTypes: Infinity, 
                monthlyTiers: Infinity, 
                branches: Infinity, 
                formulas: Infinity 
            };
        }
    ...
    function openBranchModal(branchToEdit = null) {
            ...
            <div class="bg-white p-6 md:p-8 rounded-2xl shadow-2xl w-full max-w-md transform transition-all duration-300 scale-95 opacity-0 modal-content relative">
                ...
                <form id="branchForm">
                    ...
                    <div class="space-y-4">
                        ...
                        <!-- Phone (New) -->
                        <div>
                            <label data-lang-key="label_phone_optional" class="block text-slate-600 text-sm font-semibold mb-2"></label>
                            <div class="flex gap-2">
                                <input type="text" id="branchCountryCode" class="form-input w-24" placeholder="${_('placeholder_country_code_example')}">
                                <input type="tel" id="branchPhone" class="form-input flex-grow" placeholder="${_('placeholder_tutor_phone')}">
                            </div>
                        </div>
        ...
        const form = modal.querySelector('form');
        const branchPhoneInput = form.querySelector('#branchPhone');
        const branchCountryCodeInput = form.querySelector('#branchCountryCode');

        // Input Mask Logic
        branchPhoneInput.oninput = (e) => {
            const digitsOnly = e.target.value.replace(/\D/g, '');
            e.target.value = digitsOnly.replace(/(\d{4})(?=\d)/g, '$1 ');
        };
        ...
        if (branchToEdit) {
            ...
            // Populate Phone
            const { countryCode, number } = parsePhoneNumber(branchToEdit.phone);
            branchCountryCodeInput.value = countryCode;
            branchPhoneInput.value = number;
        } else {
            form.querySelector('#branchModalId').value = '';
            form.querySelector('#branchColor').value = '#22c55e'; // Default Green-ish
        }
    ...
    function handleBranchFormSubmit(e) {
        ...
        const countryCode = form.querySelector('#branchCountryCode').value.trim();
        const phoneNumber = form.querySelector('#branchPhone').value;
        const fullPhoneNumber = constructPhoneNumber(countryCode, phoneNumber);
        ...
        const branchData = {
            ...
            phone: fullPhoneNumber, // Save to object
    ...
    function openPermissionModal(user) {
        ...
        modal.innerHTML = `
            <div class="bg-white p-6 md:p-8 rounded-2xl shadow-2xl w-full max-w-lg transform transition-all duration-300 scale-95 opacity-0 modal-content relative">
                ...
                <div class="text-center mb-6">
                    <h2 class="text-2xl font-bold text-slate-800">${_('title_edit_permissions')}</h2>
                    <p class="text-slate-500 font-medium mt-1">${user.name}</p>
                </div>
    ...
    function openChangePlanModal(owner) {
        // Reuse generic modal
        const modal = DOMElements.sportTypeModal;
        
        const currentPlan = (owner.subscriptionStatus || 'basic').toLowerCase();
        ...
        modal.innerHTML = `
            <div class="bg-white p-8 rounded-2xl shadow-2xl w-full max-w-sm transform transition-all duration-300 scale-95 opacity-0 modal-content relative">
                ...
                <div class="text-center mb-6">
                    <h2 class="text-2xl font-bold text-slate-800">${_('title_manage_subscription')}</h2>
                    <p class="text-slate-500 font-medium mt-1">${owner.name}</p>
                </div>
                ...
                <form id="changePlanForm">
                    <div class="space-y-3">
                        ...
                        <!-- BASIC -->
                        <label class="flex items-center p-3 border rounded-lg cursor-pointer hover:bg-slate-50 transition border-slate-200 has-checked:border-slate-500 has-checked:bg-slate-50 relative overflow-hidden group">
                            <input type="radio" name="planSelect" value="basic" class="mt-0.5 h-4 w-4 text-slate-600 border-gray-300 focus:ring-slate-500" ${currentPlan === 'basic' ? 'checked' : ''}>
                            <div class="ml-3">
                                <span class="block text-sm font-bold text-slate-700">${_('plan_name_basic')}</span>
                                <span class="block text-xs text-slate-500">${_('plan_desc_basic')}</span>
                            </div>
                        </label>

                        <!-- PRO -->
                        <label class="flex items-center p-3 border rounded-lg cursor-pointer hover:bg-orange-50 transition border-slate-200 has-checked:border-orange-500 has-checked:bg-orange-50 relative overflow-hidden group">
                            <input type="radio" name="planSelect" value="pro" class="mt-0.5 h-4 w-4 text-orange-600 border-gray-300 focus:ring-orange-500" ${currentPlan === 'pro' ? 'checked' : ''}>
                            <div class="ml-3 relative z-10">
                                <span class="block text-sm font-bold text-orange-700">${_('plan_name_pro')}</span>
                                <span class="block text-xs text-orange-600/70">${_('plan_desc_pro')}</span>
                            </div>
                        </label>

                        <!-- PREMIUM (Clean Classes) -->
                        <label class="flex items-center p-3 border rounded-lg cursor-pointer hover:bg-yellow-50 transition border-slate-200 has-checked:border-yellow-500 has-checked:bg-yellow-50 relative overflow-hidden group">
                            <div class="absolute inset-0 bg-gradient-to-r from-yellow-50 via-white to-yellow-50 opacity-0 group-hover:opacity-100 transition-opacity"></div>
                            <input type="radio" name="planSelect" value="premium" class="mt-0.5 h-4 w-4 text-yellow-600 border-gray-300 focus:ring-yellow-500 relative z-10" ${currentPlan === 'premium' ? 'checked' : ''}>
                            <div class="ml-3 relative z-10">
                                <!-- Uses classes from input.css -->
                                <span class="block text-sm font-bold text-premium">${_('plan_name_premium')}</span>
                                <span class="block text-xs text-premium-sub">${_('plan_desc_premium')}</span>
                            </div>
                        </label>
        ...
        const form = modal.querySelector('#changePlanForm');
        ...
        form.onsubmit = (e) => {
            e.preventDefault();
            const selectedPlan = form.querySelector('input[name="planSelect"]:checked').value;
            const submitBtn = form.querySelector('button[type="submit"]');
            submitBtn.disabled = true;
            submitBtn.textContent = _('status_saving');

            database.ref(`/users/${owner.id}/subscriptionStatus`).set(selectedPlan)
                .then(() => {
                    showMessageBox(_('success_plan_updated').replace('{plan}', selectedPlan.toUpperCase()), 'success');
                    closeModal(modal);
                    
                    // --- FIX: FORCE REFRESH OF ADMIN PAGE ---
                    // Since the owner data update doesn't automatically trigger the admin page listener logic in all cases,
                    // we force a re-render here to update the badge immediately.
                    if (appState.activePage === 'admin') {
                        renderAdminPage(document.getElementById('page-admin'));
                    }
                })
                .catch(err => {
                    console.error(err);
                    showMessageBox(_('error_update_failed_generic'), 'error');
                    submitBtn.disabled = false;
                });
        };
...
return Math.min(base + (A - threshold) * bonus, cap);`;
    ...
    async function renderSalaryPage(container) {
        ...
        // State to control "Jump to Today" behavior
        let shouldJumpToToday = true;
        ...
        const loadPeriodData = async (isRefresh = false) => {
            const period = periodSelect.value;
            appState.selectedFilters.salaryPeriod = period;

            if (!period) return;

            // Only block UI if it's a full load (changing months), not pagination refresh
            if (!isRefresh) {
            ...
            try {
                ...
                if (isRefresh) {
                    // If refreshing (pagination), just re-render the table with new data
                    updateSalaryTable();
                } else {
                    // If initial load, enable controls and run full setup
        ...
        const renderSalaryDetails = async (filteredClasses) => {
            ...
            // --- NEW: Auto-Jump to Today Logic ---
            if (shouldJumpToToday && key === 'date' && direction === 'asc') {
                const todayIso = getIsoDate(new Date());
                const period = periodSelect.value;
                
                // Only if looking at current month
                if (period === todayIso.substring(0, 7)) {
                    // Find first class equal to or after today
                    const todayIndex = clsDetails.findIndex(c => c.date >= todayIso);
                    
                    if (todayIndex !== -1) {
                        const targetPage = Math.ceil((todayIndex + 1) / itemsPerPage.salary);
                        appState.pagination.salary.page = targetPage;
                    }
                }
                shouldJumpToToday = false; 
            }
            ...
            // --- NEW: Reset date tracker for divider logic ---
            let lastDate = null;
                    ...
                    <div class="overflow-x-auto">
                        ...
                        <tbody>
                            ${paginatedClsDetails.map((c, index) => {
                                ...
                                // --- NEW: Check for new day ---
                                const isNewDay = c.date !== lastDate;
                                lastDate = c.date;
            ...
            // --- UPDATED: REFRESH DATA ON PAGINATION ---
            renderPaginationControls(paginationContainer, page, totalPages, clsDetails.length, itemsPerPage.salary, (newPage) => {
                ...
                loadPeriodData(true); // Fetch fresh data from DB on navigation
            });
        ...
        periodSelect.onchange = () => {
            ...
            shouldJumpToToday = true; // Reset flag on month change
            loadPeriodData();
        };
        ...
        exportBtn.onclick = async () => {}; // Placeholder
    ...
    async function renderClassesPage(container) {
        ...
        // State to control "Jump to Today" behavior
        // We only want to jump to today when the month is first loaded or changed, 
        // not every time the table re-renders (like after sorting).
        let shouldJumpToToday = true;

        // --- 1. Prepare Branch Filter HTML (With Persistence) ---
        let branchFilterHTML = '';
        const hasEffectiveMulti = effectiveBranches.length > 1;

        // Ensure state exists
        if (!appState.selectedFilters.classesBranchId) {
            appState.selectedFilters.classesBranchId = 'all';
        }
        const savedBranchId = appState.selectedFilters.classesBranchId;

        if (hasEffectiveMulti) {
            let options = `<option value="all" ${savedBranchId === 'all' ? 'selected' : ''}>${_('filter_all_branches')}</option>`;
            options += effectiveBranches.map(b => {
                const isSelected = b.id === savedBranchId ? 'selected' : '';
                return `<option value="${b.id}" ${isSelected}>${getBranchName(b)}</option>`;
            }).join('');
                    ...
                    <table class="w-full text-left min-w-[900px]">
                        <thead>
                            <tr class="border-b">
                                ...
                                <th class="p-2 sortable cursor-pointer whitespace-nowrap" data-sort-key="creditType">${_('label_credit_type')}<span class="sort-icon"></span></th>
        ...
        const updateDynamicFilters = (classesData) => {
            ...
            // Fix: Read from AppState (Persistent), NOT from DOM (which is reset)
            const currentSportSelection = appState.selectedFilters.classesSportTypeId || 'all';
            const currentTutorSelection = appState.selectedFilters.classesTutorId || 'all';

            populateSportTypeFilter(sportTypeFilter, filteredSportTypes);
            populateTutorFilter(tutorFilter, 'all', filteredTutors); 

            // Restore selection if valid, otherwise reset state to 'all'
        ...
        const fetchAndRenderClasses = async (keepPage = false) => {
            const selectedMonth = monthFilter.value;
            const colSpan = hasMultipleBranches ? 10 : 9;
            ...
            // Only show loading if we are NOT keeping the page (i.e., new month filter)
            // If keeping page (pagination), a full spinner might be too jarring, but we want indicators.
            // For now, we will show loading to indicate freshness.
            ...
            // Initial filter based on permissions
            monthlyClasses = filterClassesByBranchContext(monthlyClasses, branches, effectiveBranches, 'all');

            if (!keepPage) {
                // Reset to "Jump Mode" on new data load (Month Change)
                shouldJumpToToday = true;
                updateDynamicFilters(monthlyClasses);
            }
            
            updateClassesTable(keepPage);
        };
        ...
        const getClsCreditTypeName = (cls) => {
            const typeId = cls.costCreditTypeId || 'general';
            const typeDef = appState.creditTypes ? appState.creditTypes.find(ct => ct.id === typeId) : null;
            return typeDef ? getCreditTypeName(typeDef) : (typeId === 'general' ? _('label_general') : _('label_credits'));
        };

        const updateClassesTable = (keepPage = false) => {
            ...
            } else if (appState.selectedFilters.classesBranchId) {
                selectedBranchId = appState.selectedFilters.classesBranchId; 
            ...
            filteredClasses.sort((a, b) => {
                ...
                switch (key) {
                    ...
                    case 'branchName': 
                        ...
                        break;
                    case 'creditType':
                        valA = getClsCreditTypeName(a).toLowerCase();
                        valB = getClsCreditTypeName(b).toLowerCase();
                        break;
                    default:
                        valA = a[key];
                        valB = b[key];
            ...
            // --- NEW: Auto-Jump to Today Logic ---
            // If the flag is set, and we are sorting by date asc, find the page with today's date
            if (shouldJumpToToday && key === 'date' && direction === 'asc') {
                const todayIso = getIsoDate(new Date());
                const selectedMonth = monthFilter.value;
                
                // Only if looking at current month
                if (selectedMonth === todayIso.substring(0, 7)) {
                    // Find first class equal to or after today
                    const todayIndex = filteredClasses.findIndex(c => c.date >= todayIso);
                    
                    if (todayIndex !== -1) {
                        const targetPage = Math.ceil((todayIndex + 1) / itemsPerPage.classes);
                        appState.pagination.classes.page = targetPage;
                    }
                }
                shouldJumpToToday = false; // Consume the flag so we don't jump on sort click
            } else if (!keepPage && shouldJumpToToday) {
                // If not jumping (different sort or month), reset to 1
            ...
            tableBody.innerHTML = paginatedClasses.map((cls, index) => {
                ...
                const buttonsHTML = showRowButtons 
                    ? `<button class="row-edit-btn font-semibold text-indigo-600" data-id="${cls.id}">${_('btn_edit')}</button>
                       <button class="row-delete-btn font-semibold text-red-600" data-id="${cls.id}">${_('btn_delete')}</button>`
                    : `<span class="text-xs text-slate-400 italic">${_('label_view_only') || 'View Only'}</span>`;

                const typeName = getClsCreditTypeName(cls);
                ...
                return `
                    <tr class="border-b border-slate-100 cursor-pointer hover:bg-slate-50 transition-colors ${isNewDay && index > 0 ? 'day-divider' : ''}" data-date="${cls.date}" data-branch-id="${targetBranchId}">
                        ...
                        <td class="p-2 whitespace-nowrap">${typeName}</td>
            ...
            // --- NEW: Refresh Data on Pagination ---
            // We pass 'true' to fetchAndRenderClasses so it knows to keep the current page (managed here)
            // but just reload the data behind the scenes.
            renderPaginationControls(paginationContainer, page, totalPages, filteredClasses.length, itemsPerPage.classes, (newPage) => {
                ...
                fetchAndRenderClasses(true); // Trigger refresh, keeping page state
            });
        };

        // --- 5. Event Bindings (Now updating AppState) ---

        monthFilter.onchange = () => {
            appState.selectedFilters.classesPeriod = monthFilter.value;
            appState.pagination.classes.page = 1; 
            fetchAndRenderClasses(false); // Reset jump flag
        };
        ...
        if (branchFilter) {
            branchFilter.onchange = () => {
                appState.selectedFilters.classesBranchId = branchFilter.value;
                appState.pagination.classes.page = 1; 
                updateClassesTable();
            };
        }
        ...
        container.querySelectorAll('th.sortable').forEach(header => {
            header.onclick = () => {
                ...
                updateClassesTable(true); // Keep page on sort? Or reset? Typically table sorts keep page or reset. Let's keep for now.
            };
        });
    ...
    async function renderIncomePage(container) {
        ...
        const renderIncomeDetails = async () => {
            ...
            if (shouldJumpToToday && key === 'date' && direction === 'asc') {
                ...
                // Only if looking at current month
                if (period === todayIso.substring(0, 7)) {
                    // Find first entry equal to or after today
                    const todayIndex = dailyDetails.findIndex(d => d.date >= todayIso);
                    
                    if (todayIndex !== -1) {
                        const targetPage = Math.ceil((todayIndex + 1) / itemsPerPage.income);
                        appState.pagination.income.page = targetPage;
                    }
                }
                shouldJumpToToday = false; 
            }
            ...
            renderPaginationControls(paginationContainer, page, totalPages, dailyDetails.length, itemsPerPage.income, (newPage) => {
                ...
                renderIncomeDetails(); // Re-fetch on navigation
            });
        ...
        periodSelect.onchange = () => {
            ...
            shouldJumpToToday = true; // Reset jump flag when changing month manually
            renderIncomeDetails();
        };
            ...
            const renderContent = () => {
                ...
                listContainer.innerHTML = paginatedBookings.map(cls => {
                    ...
                    return `<div class="bg-slate-100 p-4 rounded-lg flex justify-between items-center">
                        ...
                        ${isAttended 
                            ? `<span class="text-sm font-semibold text-green-600 whitespace-nowrap">${_('status_completed')}</span>`
    ...
    const setupClassSubscription = (branchId) => {
        ...
        const isVisibleForMember = (cls) => {
            ...
            if (isOwner || isSuperAdmin || isStaff || isManager) return true; 
        ...
        const onChildChanged = (snapshot) => {
            ...
            if (isVisibleForMember(updatedCls)) {
                ...
                if (index > -1) {
                    ...
                    // Compare previous state with new state to detect NEW bookings
                    const oldCls = appState.classes[index];
                    const oldBookings = oldCls.bookedBy ? Object.keys(oldCls.bookedBy) : [];
                    const newBookings = updatedCls.bookedBy ? Object.keys(updatedCls.bookedBy) : [];

                    // If the number of bookings increased
                    if (newBookings.length > oldBookings.length) {
                        // Find the ID that wasn't there before
                        const newMemberId = newBookings.find(id => !oldBookings.includes(id));
                        
                        // Condition: User exists AND it wasn't the current user who performed the action 
                        // (Current user already sees the "Success" modal, avoiding double notification)
                        if (newMemberId && newMemberId !== appState.currentUser.id) {
                            ...
                            if (member && sportType) {
                                showBookingNotification({
                                    ...
                                    clsName: getSportTypeName(sportType),
                                    clsTime: updatedCls.time,
                                    duration: updatedCls.duration
                                });
                            }
                        }
                    }
    ...
    const initDataListeners = () => {
        ...
        // 2. Routing Logic
        // Owners & SuperAdmins ALWAYS get full access.
        // Managers/Staff only get full access if plan is NOT Basic.
        if (isOwner || isSuperAdmin || ((isManager || isStaff) && !isBasic)) {
            initOwnerListeners();
        } else {
    ...
    const initOwnerListeners = () => {
        ...
        // Staff/Manager Plan Sync Logic (UPDATED: Best Plan Wins)
        if (appState.currentUser.role === 'staff' || appState.currentUser.role === 'manager') {
            if (ownerPlanQuery) ownerPlanQuery.off();
            // Remove .limitToFirst(1) to get ALL owners
            ownerPlanQuery = database.ref('/users').orderByChild('role').equalTo('owner');
            ...
            const handleOwnerPlanUpdate = (snapshot) => {
                ...
                if (ownerData) {
                    ...
                    // Priority Logic: Premium > Pro > Basic
                    const planWeight = { 'basic': 0, 'pro': 1, 'premium': 2 };
                    
                    owners.forEach(o => {
                        const p = (o.subscriptionStatus || 'basic').toLowerCase();
                        if (planWeight[p] > planWeight[bestPlan]) {
                            bestPlan = p;
                        }
                    });

                    const oldPlan = appState.studioPlan;
                    appState.studioPlan = bestPlan;

                    // Downgrade Protection
                    if (bestPlan === 'basic' && oldPlan !== 'basic') {
                        initDataListeners();
                        return;
                    }
        ...
        dataListeners.users = (snapshot) => {
            ...
            // --- UPDATE: Removed 'statistics' to prevent auto-refresh/flickering ---
            const pagesRequiringUserListRender = ['members'];
    ...
    const initMemberListeners = () => {
        ...
        // Listen for Owner's Subscription Status (UPDATED: Best Plan Wins)
        if (ownerPlanQuery) ownerPlanQuery.off();
        // Remove .limitToFirst(1) to get ALL owners
        ownerPlanQuery = database.ref('/users').orderByChild('role').equalTo('owner');
        ...
        const handleOwnerPlanUpdate = (snapshot) => {
            ...
            if (ownerData) {
                ...
                // "Escape Hatch": If we discover we are actually Premium/Pro, restart listeners
                if (bestPlan !== 'basic' && (oldPlan === 'basic' || !oldPlan)) {
                    initDataListeners(); 
                    return; 
                }
    ...
    const init = () => {
        ...
        // --- NEW: FETCH STUDIO PROFILE ---
        database.ref('/studioProfile').once('value').then(snapshot => {
            if (snapshot.exists()) {
                appState.studioProfile = snapshot.val();
                updateStudioIdentity(); // Update HTML immediately
            }
        });
        ...
        document.body.addEventListener('click', e => {
            ...
            if (clsBlock && !e.target.closest('button') && !e.target.closest('input') && !e.target.closest('.time-slot-editable') && !e.target.closest('.participant-counter-editable') && !e.target.closest('.participant-dial-trigger')) {
                ...
                // FIX: Include SuperAdmin in Admin check
                const isAdmin = isOwner || isSuperAdmin || isManager || isStaff;
                ...
                if (isAdmin) {
                    ...
                    // Staff/Manager restrictions (SuperAdmin & Owner bypass this and stay true)
                    if (isStaff || isManager) {
                        const user = appState.currentUser;
                        const staffLevel = user.staffAccessLevel || 'home_only';
                        if (staffLevel !== 'global_write') {
