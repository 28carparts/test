...
${_('whatsapp_closing')}
    ...
    async function openJoinedMembersModal(cls) {
        ...
        addMemberSearchResults.onclick = (e) => {
            ...
            if (target) {
                ...
                // 1. Check Monthly Plan Eligibility
                if (memberToAdd.monthlyPlan) {
                    if (currentCls.notForMonthly) {
                        // Class strictly not for monthly
                        paymentMethod = 'none';
                ...
                if (paymentMethod === 'none') {
                    // Fail: No valid plan and insufficient credits
                ...
                updates[`/classes/${currentCls.id}/bookedBy/${memberId}`] = {
                    ...
                    bookedBy: appState.currentUser.name, // "Owner" or "Staff"
                    paymentMethod: paymentMethod, // Store how they paid!
                    monthlyCreditValue: memberToAdd.monthlyCreditValue || 0, // Snapshot for potential stats
    ...
    function openBookingModal(cls) {
        ...
        // 1. Setup Credit Info (Required for fallbacks even if Monthly is active)
        ...
        let planIneligibilityReason = null; // 'not_for_monthly', 'tier_restricted', 'none'
        ...
        // Check B: Credit Wallet (Fallback if Monthly failed or didn't exist)
        if (paymentMethod === 'none') {
        ...
        if (paymentMethod === 'monthly') {
            ...
            const mpPillStyle = tier ? `background-color: ${tierColor}26; color: ${tierColor};` : 'background-color: #dcfce7; color: #166534;';
            ...
            // FIX: Applied Pill Styling to Balance
            ...
            // -- CREDIT MODE (or Insufficient) --
            
            // FIX 1: Show specific Credit Type Name inside the pill (e.g., "Small Class")
            ...
            // FIX 2: Apply Pill Styling to Balance
            if (userBalance < classCost) {
                // Insufficient: Red styling but kept in a pill shape
                ...
                // Sufficient: Standard colored pill style
                balanceHtml = `<span class="inline-block text-sm font-bold px-3 py-1 rounded-full" style="${creditPillStyle}">${formatCredits(userBalance)} ${creditTypeName}</span>`;
            }

            // Add Note if they *have* a plan but couldn't use it
            if (currentUser.monthlyPlan && planIneligibilityReason) {
                const reasonText = planIneligibilityReason === 'not_for_monthly' ? _('info_class_not_in_plan') : _('info_tier_restricted');
        ...
        confirmBtn.onclick = () => {
            ...
            if (cls.bookedBy && cls.bookedBy[memberId]) {
                ...
                // If fell through waterfall to 'none', it means no plan coverage AND insufficient credits
                ...
                const bookingData = {
                    bookedAt: new Date().toISOString(),
                    bookedBy: 'member',
                    paymentMethod: paymentMethod, // SAVING THE METHOD IS CRITICAL
                    creditsPaid: 0,
                    creditTypeId: requiredTypeId // Always store the relevant currency type
                };

                if (paymentMethod === 'monthly') {
                    // Plan Booking
                    bookingData.monthlyCreditValue = currentUser.monthlyCreditValue || 0;
                    bookingData.creditsPaid = 0; // No deduction
                } else {
                    // Credit Booking
    ...
    function renderAccountPage(container) {
        ...
        const getProfileContent = () => {
            const joinDateHtml = `<div><p data-lang-key="label_join_date" class="text-base font-bold text-slate-700 mb-1"></p><p class="text-base text-slate-600">${formatShortDateWithYear(member.joinDate)}</p></div>`;
            ...
            if (member.monthlyPlan) {
                ...
                <div class="mt-4 pt-4 border-t border-slate-100">
                    <div><p data-lang-key="label_plan" class="text-base font-bold text-slate-700 mb-2"></p>${planPill}</div>
                    <div class="mt-3"><p data-lang-key="label_renews_every_month" class="text-sm text-slate-500"></p><p class="font-bold text-sm text-slate-800">${renewsText}</p></div>
            ...
            if (creditsListHtml) {
                content += `<div class="mt-4 pt-4 border-t border-slate-100"><p data-lang-key="label_credits_remaining" class="text-base font-bold text-slate-700 mb-3"></p>${creditsListHtml}</div>`;
            }
                ...
                <div class="md:col-span-2">
                    <div class="card p-6">
                        <div class="flex flex-wrap justify-between items-center mb-4 gap-2">
                            <h3 class="text-2xl font-bold text-slate-800 whitespace-nowrap">${_('header_my_bookings')} <span id="accountBookingCount"></span></h3>
    ...
    function openMemberModal(memberToEdit) {
        ...
        const refreshMemberData = () => {
            ...
            database.ref(`/users/${memberToEdit.id}`).once('value', snapshot => {
                ...
                // This ensures if the expiry changed during the transaction, the input updates visually
                if (currentTypeId) {
                    ...
                    if (updatedWallet[currentTypeId] && updatedWallet[currentTypeId].expiryDate) {
                        ...
                        // Trigger the iOS date input fix helper defined earlier in openMemberModal
                        updateExpiryUI(); 
                    }
                }
            });
        ...
        // --- UPDATED CREDIT TRANSACTION LOGIC WITH FIX ---
             ...
             try {
                 ...
                 if(historyId) {
                     ...
                     if (oldTypeId === newTypeId) {
                        // Case A: Same Type Update (Simple Diff)
                        ...
                        // Simply adjust current balance by the difference
                        ...
                        // Case B: Type Change (Transfer with Deficit Calculation)
                        ...
                        // 1. Calculate Old Wallet Impact
                        // Calculate what the balance WOULD be if we removed the original purchase
                        const theoreticalOldBal = oldBal - originalEntryCredits;
                        
                        // If theoretical is negative, it means those credits were USED. We record that as a deficit.
                        const usedCreditsDeficit = theoreticalOldBal < 0 ? Math.abs(theoreticalOldBal) : 0;
                        
                        // The actual new balance for the old wallet cannot be less than 0
                        const finalOldBal = Math.max(0, theoreticalOldBal);
                        const finalOldInit = Math.max(0, oldInit - originalEntryCredits);
                        
                        // 2. Apply to Old Wallet
                        ...
                        // We add the NEW credits, but we SUBTRACT the deficit (credits already consumed)
                        // This ensures the user doesn't magically gain back used credits.
                        // We assume the deficit is carried over to the new type.
                        const finalNewBal = Math.max(0, newBal + credits - usedCreditsDeficit);
                        
                        // Initial credits should reflect the full purchase capacity, regardless of usage
    ...
    async function renderIncomePage(container) {
        ...
        // Updated grid layout to support 5 summary cards
        container.innerHTML = `
            <div class="card p-6 md:p-8">
                <div class="flex flex-wrap gap-4 justify-between items-center mb-6">
        ...
        const renderIncomeDetails = async () => {
            ...
            classesInPeriod.forEach(cls => {
                if (!dailyStatsMap[cls.date]) {
                    dailyStatsMap[cls.date] = { 
                        ...
                        enrollments: 0, 
                        capacity: 0, 
                        grossRevenue: 0, // Changed from 'revenue' to explicit 'grossRevenue'
                        netRevenue: 0,   // New Field
                        enrolledNames: []
                    };
                }
            ...
            // 7. Render HTML
            // Updated Grid to 5 columns (Gross, Net, Classes, Enrollments, Attendees)
