    ...
    function renderCheckInPage(container) {
        ...
        const renderUpcomingList = (targetContainerId) => {
            ...
            if (todaysUnattendedBookings.length > 0) {
                ...
                todaysUnattendedBookings.sort((a,b) => a.time.localeCompare(b.time)).map(cls => {
                    ...
                    // FIX: Replaced bg-indigo-50 with bg-white and added border styling for clearer selection
                    const baseClass = "w-full p-4 rounded-lg transition-all cursor-pointer flex flex-col items-center justify-center gap-1 border";
                ...
                upcomingSection.addEventListener('click', (e) => {
                    ...
                    // FIX: Clear error message and Resume Scanner if it was paused waiting for selection
                    const resultContainer = document.getElementById('memberScanResult');
                    if (resultContainer) resultContainer.innerHTML = '';
                    
                    if (window.memberScanner && window.memberScanner.getState() === 2) { // 2 = PAUSED
                        console.log("Selection made, resuming scanner...");
                        window.memberScanner.resume();
                    }
                });
            } else {
        ...
        const renderScanStudioTab = () => {
            ...
            contentArea.innerHTML = `
                ...
                <!-- Removed the confusing 'Point your camera...' text here to clean up UI -->
