...
${_('whatsapp_closing')}
    ...
    function renderCheckInPage(container) {
            ...
            <div class="w-full max-w-sm mx-auto">
                <div class="card p-6">
                    <div class="flex border-b border-slate-200 mb-6">
                        <button id="tabMyCode" class="flex-1 pb-2 text-sm font-bold text-indigo-600 border-b-2 border-indigo-600 transition-colors">
                            ${_('tab_my_code')}
                        </button>
                        <button id="tabScanStudio" class="flex-1 pb-2 text-sm font-bold text-slate-500 hover:text-indigo-600 border-b-2 border-transparent transition-colors">
                            ${_('tab_scan_studio')}
                        </button>
                    </div>
        ...
        // --- SUB-FUNCTION: Render "My Code" (Existing Logic) ---
        const renderMyCodeTab = () => {
            ...
            if (window.memberScanner) {
                window.memberScanner.stop().then(() => window.memberScanner.clear());
                window.memberScanner = null;
            }
            ...
            // Render Upcoming List (Existing Logic)
            const upcomingSection = contentArea.querySelector('#upcomingCheckInSection');
            if (todaysUnattendedBookings.length > 0) {
                upcomingSection.innerHTML = `<h5 class="text-center font-semibold text-slate-600">${_('header_upcoming_checkins')}</h5>` +
                todaysUnattendedBookings.sort((a,b) => a.time.localeCompare(b.time)).map(cls => {
                    ...
                    const branchBadge = branchName ? `<span class="inline-block mt-1 text-[10px] font-bold px-2 py-0.5 rounded-full border" style="background-color: ${branchColor}15; color: ${branchColor}; border-color: ${branchColor}30;">${_('info_class_branch')}: ${branchName}</span>` : '';
                ...
                }).join('') + (todaysUnattendedBookings.length > 1 ? `<p class="text-center text-sm text-slate-500 mt-2">${_('check_in_preselect_prompt')}</p>` : '');
                ...
                upcomingSection.addEventListener('click', (e) => {
                    ...
                    upcomingSection.querySelectorAll('button').forEach(btn => btn.classList.remove('checkin-selected', 'ring-2', 'ring-indigo-500', 'bg-indigo-50'));
                    if (newSelectionId) clickedButton.classList.add('checkin-selected', 'ring-2', 'ring-indigo-500', 'bg-indigo-50');
                    database.ref(`/users/${member.id}/selectedCheckInClassId`).set(newSelectionId);
                });
            } else {
                upcomingSection.innerHTML = `<p class="text-center text-slate-500 mt-4">${_('info_no_booked_class_today')}</p>`;
            }

            // Bind Firebase Listeners for Auto-Success (Existing Logic)
            ...
            const processCheckIns = () => {
                ...
                newlyCheckedInIds.forEach(id => document.querySelector(`[data-checkin-cls-id="${id}"]`)?.remove());
                
                database.ref(`/users/${member.id}/selectedCheckInClassId`).set(null);
                setTimeout(() => { if (resultContainer) resultContainer.innerHTML = ''; }, 3000);
                
                if (upcomingSection.querySelectorAll('[data-checkin-cls-id]').length === 0) {
                    upcomingSection.innerHTML = `<p class="text-center text-slate-500 mt-4">${_('info_no_booked_class_today')}</p>`;
                }
        ...
        // --- SUB-FUNCTION: Render "Scan Studio" (New Logic) ---
        const renderScanStudioTab = () => {
            ...
            // Clean up listeners from Tab 1
            Object.values(memberCheckInListeners).forEach(({ ref, listener }) => ref.off('value', listener));
            memberCheckInListeners = {};

            contentArea.innerHTML = `
                <h4 class="text-xl font-bold text-slate-800 mb-4">${_('tab_scan_studio')}</h4>
                <div id="member-qr-reader" class="bg-slate-900 rounded-lg overflow-hidden w-full h-64"></div>
                <div id="memberScanResult" class="mt-4 min-h-[3rem]"></div>
                <p class="text-xs text-slate-400 mt-4">${_('check_in_prompt')}</p>
            `;
            ...
            window.memberScanner.start({ facingMode: "environment" }, config, (decodedText) => {
                if (window.memberScanner && window.memberScanner.isScanning) {
                    ...
                    handleMemberScanningStudio(decodedText, () => {
                        // On success or error handled, resume
                        setTimeout(() => window.memberScanner.resume(), 2000);
                    });
                }
            }).catch(err => {
                console.error("Camera error", err);
                contentArea.querySelector('#memberScanResult').innerHTML = `<p class="text-red-500">${_('check_in_error_camera_start_failed')}</p>`;
            });
        };
    ...
    function openCheckInModal() {
        ...
        DOMElements.checkInModal.innerHTML = `
            <div class="bg-white p-6 rounded-2xl shadow-2xl w-full max-w-lg transform transition-all duration-300 scale-95 opacity-0 modal-content relative">
                ...
                <!-- Tabs -->
                <div class="flex border-b border-slate-200 mb-6">
                    <button id="tabOwnerScanMember" class="flex-1 pb-2 text-sm font-bold text-indigo-600 border-b-2 border-indigo-600 transition-colors">
                        ${_('tab_scan_member')}
                    </button>
                    <button id="tabOwnerShowQR" class="flex-1 pb-2 text-sm font-bold text-slate-500 hover:text-indigo-600 border-b-2 border-transparent transition-colors">
        ...
        // --- SUB-FUNCTION: Owner Scans Member (Existing Logic) ---
        const renderOwnerScan = () => {
            ...
            // Device Location Logic (Existing)
            const branches = appState.branches || [];
            ...
            if (branches.length > 1) {
                ...
                locationSelector = `
                    <div class="mb-4 flex justify-center items-center gap-2 bg-slate-50 p-2 rounded-lg border border-slate-200 w-full">
                        ...
                        <select id="modalDeviceBranchSelect" class="form-select py-1 pl-2 pr-8 text-sm font-semibold w-auto bg-transparent border-none focus:ring-0 cursor-pointer">${options}</select>
                    </div>`;
            }
            ...
            contentArea.innerHTML = `
                ...
                ${locationSelector}
                <div id="qr-reader" class="bg-slate-900 rounded-lg overflow-hidden w-full h-64"></div>
        ...
        // --- SUB-FUNCTION: Owner Shows Dynamic QR (New Logic) ---
        const renderOwnerShow = () => {
            ...
            // Branch Selection for the QR
            const branches = appState.branches || [];
            const defaultBranch = appState.selectedScheduleBranch || (branches[0] ? branches[0].id : null);
            let branchOptions = '';
            
            if (branches.length > 0) {
                branchOptions = branches.map(b => {
                    const bName = (appState.currentLanguage === 'zh-TW' && b.name_zh) ? b.name_zh : b.name;
                    return `<option value="${b.id}" ${b.id === defaultBranch ? 'selected' : ''}>${bName}</option>`;
                }).join('');
            } else {
                // Fallback if no branches are set up
                branchOptions = `<option value="main">Main Studio</option>`;
            }
            ...
            contentArea.innerHTML = `
                ...
                <div class="mb-4 flex justify-center items-center gap-2">
                    <label class="text-sm font-bold text-slate-600">${_('label_branch')}:</label>
                    <select id="qrTargetBranch" class="form-select py-1 pl-2 pr-8 text-sm font-semibold">${branchOptions}</select>
                </div>
            ...
            const branchSelect = contentArea.querySelector('#qrTargetBranch');
            ...
            const generate = () => {
                ...
                const payload = {
                    t: 's', // Type: Studio
                    b: branchSelect.value, // Branch ID
                    ts: Date.now() // Timestamp
                };
                
                new QRCode(qrContainer, {
                    text: JSON.stringify(payload),
                    width: 230, // Adjusted for container padding
                    height: 230,
                    colorDark: "#1e293b",
                    colorLight: "#ffffff",
                ...
                // Animation for refresh visual
                timerLabel.classList.remove('opacity-50');
                void timerLabel.offsetWidth; // Trigger reflow
                timerLabel.classList.add('opacity-50');
            };

            generate();
            
            // Refresh QR every 15 seconds to prevent static photo abuse
            dynamicQrInterval = setInterval(generate, 15000); 
            
            // Re-generate immediately on branch change
            branchSelect.onchange = generate; 
        };
        ...
        openModal(modal);
        renderOwnerScan(); // Default to scan mode
    }
    ...
    function handleMemberScanningStudio(decodedText, onResumeCallback) {
        ...
        // Note: For static paper QRs, you might remove the timestamp check or use a specific flag like payload.static = true
        const now = Date.now();
        ...
        // 1. Get all bookings for today for this member
        const allBookingsToday = appState.classes.filter(cls => 
            cls.date === today && cls.bookedBy && cls.bookedBy[memberId]
        );

        // 2. Filter by Branch (Payload Branch must match Class Branch)
        // If class has no branch (Legacy), assuming it matches Main Studio (index 0)
        ...
        if (validBranchBookings.length === 0) {
            // Specific Error: Wrong Branch vs No Booking
            if (allBookingsToday.length > 0) {
                // They have a booking today, just not here.
        ...
        // 4. Perform Check-in
        // If multiple classes at this branch, check in ALL of them or just the first?
        // Usually, if a member walks into the branch, they are here for the imminent class. 
        // For simplicity and user ease, we check them into the earliest upcoming unattended class.
        
        // Sort by time
        unattended.sort((a,b) => a.time.localeCompare(b.time));
        const targetClass = unattended[0];
        ...
        database.ref(`/classes/${targetClass.id}/attendedBy/${memberId}`).set(true)
            .then(() => {
                ...
                const sportType = appState.sportTypes.find(st => st.id === targetClass.sportTypeId);
                const className = getSportTypeName(sportType);
                
                resultEl.innerHTML = `<div class="bg-green-100 text-green-700 p-3 rounded-lg text-center shadow-sm">
                    <div class="font-bold text-lg">${_('success_check_in_complete')}</div>
                    <div class="text-sm mt-1">${className}</div>
                    <div class="text-xs opacity-75">${getTimeRange(targetClass.time, targetClass.duration)}</div>
                </div>`;
                
                // Don't resume immediately, let them see the success message
            })
            .catch(err => {
                console.error(err);
