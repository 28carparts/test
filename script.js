...
${_('whatsapp_closing')}
    ...
    function openBookingModal(cls) {
        ...
        let paymentMethod = 'none'; // 'monthly', 'credit', 'none'
        let planIneligibilityReason = null; // 'not_for_monthly', 'tier_restricted', 'branch_restricted', 'none'
        let creditIneligibilityReason = null; // 'insufficient', 'branch_restricted'
        ...
        if (currentUser.monthlyPlan) {
            ...
            // --- NEW: Check Branch Validity for Plan ---
            if (!planIneligibilityReason) {
                // If tier exists, check its setting. If legacy (null tier), assume global (true).
                const tierConfig = tier || { allowGlobalAccess: true };
            ...
            // If no blocks, we can use monthly
            if (!planIneligibilityReason) {
                paymentMethod = 'monthly';
            }
        }
        ...
        if (paymentMethod === 'none') {
            if (userBalance >= classCost) {
                // --- NEW: Check Branch Validity for Credit Type ---
                // If type def exists, check setting. If legacy/general, assume global.
                const creditConfig = creditTypeDef || { allowGlobalAccess: true };
        ...
        // 3. Build UI Elements based on determination
        ...
        if (paymentMethod === 'monthly') {
            ...
            // -- CREDIT MODE (or Insufficient/Restricted) --
            ...
            if (creditIneligibilityReason === 'insufficient') {
                ...
                // New UI for Credit Branch Restriction
            ...
            // Add Note if they *have* a plan but couldn't use it
            if (currentUser.monthlyPlan && planIneligibilityReason) {
                let reasonText = '';
                if (planIneligibilityReason === 'not_for_monthly') reasonText = _('info_class_not_in_plan');
        ...
        if (hasMultipleBranches) {
            // Resolve branch (handle legacy data defaulting to first branch)
            const branchId = cls.branchId || (branches[0] ? branches[0].id : null);
            const branch = branches.find(b => b.id === branchId);
        ...
        confirmBtn.onclick = () => {
            ...
            if (cls.bookedBy && cls.bookedBy[memberId]) {
                ...
                if (creditIneligibilityReason === 'branch_restricted') {
                    const typeName = creditTypeName;
                    showMessageBox(`${_('error_branch_restricted_credits')} (${typeName})`, 'error');
                } else {
