...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const renderCurrentPage = () => {
        ...
        // If a Staff member tries to access an owner-only page (Manager is EXEMPT from this check)
    ...
    function handleDeleteClsRequest(cls) {
        ...
        if (hasBookings) {
            // Future class with bookings -> Go to Notification/Refund flow
            openDeleteClsNotifyModal(cls);
        } else {
            // Future class, empty -> Standard delete confirmation
            const sportType = appState.sportTypes.find(st => st.id === cls.sportTypeId);
...
${_('whatsapp_closing')}
    ...
    function createClsElement(cls) {
        ...
        const isOwner = appState.currentUser?.role === 'owner';
        const isManager = appState.currentUser?.role === 'manager'; // NEW: Recognize Manager
        const isStaff = appState.currentUser?.role === 'staff';
        const isAdmin = isOwner || isManager || isStaff; // UPDATED: Include Manager in Admin check
        
        const currentBookings = cls.bookedBy ? Object.keys(cls.bookedBy).length : 0;
        
        // --- STAFF PERMISSION LOGIC (UPDATED) ---
        let hasWritePermission = true;
        // Managers act like Owners (Global Write by default), so only restrict Staff
        if (isStaff) {
            const user = appState.currentUser;
    ...
    function renderOwnerSchedule(container) {
        ...
        // --- STAFF/MANAGER PERMISSION LOGIC ---
        const user = appState.currentUser;
        const isOwner = user.role === 'owner';
        // Note: Managers are now subject to the same branch restrictions as Staff
        let staffAccessLevel = 'home_only'; 
        
        // DETERMINE EFFECTIVE BRANCHES (Scope)
        let effectiveBranches = branches; 

        // Filter branches if the user is NOT the Owner (Applies to Staff AND Managers)
        if (!isOwner) {
            staffAccessLevel = user.staffAccessLevel || 'home_only';
            
            if (staffAccessLevel === 'home_only') {
                // Calculate Allowed Branches
                let allowedIds = [];
                if (user.allowedBranches && Object.keys(user.allowedBranches).length > 0) {
                    allowedIds = Object.keys(user.allowedBranches);
                } else {
                    // Fallback to homeBranchId
                    allowedIds = [user.homeBranchId || (branches.length > 0 ? branches[0].id : null)];
                }
                
                // Filter the global branch list
                effectiveBranches = branches.filter(b => allowedIds.includes(b.id));
            }
        }
        ...
        if (hasMultipleOptions) {
            ...
            // Show static indicator using segment style
            const currentB = effectiveBranches[0];
            const bName = (appState.currentLanguage === 'zh-TW' && currentB.name_zh) ? currentB.name_zh : currentB.name;
        ...
        if (branches.length > 1 && appState.selectedScheduleBranch) {
        ...
        // --- ADD BUTTON VISIBILITY LOGIC (UPDATED) ---
        ...
        if (creditTypesList) {
            ...
            if (credits.length === 0) {
                ...
                creditTypesList.innerHTML = credits.map((ct, idx) => {
                    ...
                    <li class="flex justify-between items-center bg-slate-100 p-2 rounded-md">
                        <div class="flex items-center gap-3">
                            <span class="h-5 w-5 rounded-full flex-shrink-0" style="background-color: ${ct.color || '#ccc'}"></span>
...
return Math.min(base + (A - threshold) * bonus, cap);`;
    ...
    async function renderSalaryPage(container) {
        ...
        // If NOT Owner, filter branches based on access level
        if (!isOwner) {
        ...
        const updateSalaryTable = () => {
            ...
            if (selectedBranchId !== 'all') {
                filteredClasses = filteredClasses.filter(c => {
                    const cBranch = c.branchId || getDefaultBranchId();
        ...
        const loadPeriodData = async () => {
            ...
            try {
                ...
                // --- SECURITY FILTER: Restrict data for Managers/Staff ---
                if (!isOwner) {
                    const allowedIds = effectiveBranches.map(b => b.id);
                    monthlyClasses = rawClasses.filter(c => {
                        const cBranch = c.branchId || getDefaultBranchId(); // Use safe helper
                        return allowedIds.includes(cBranch);
                    });
                } else {
                    monthlyClasses = rawClasses;
                }
        ...
        const renderSalaryDetails = (filteredClasses) => {
            ...
            const clsDetails = filteredClasses.map(cls => {
                ...
                // Use Centralized Helper for Math
                ...
                let branchName = '';
                if (hasMultipleBranches) {
                    const bId = cls.branchId || getDefaultBranchId();
    ...
    async function renderStatisticsPage(container) {
        ...
        const renderFilteredStats = async () => {
            ...
            // 1. Permission Filter: Restrict data for Managers/Staff
            if (!isOwner) {
                const allowedIds = effectiveBranches.map(b => b.id);
                filteredClasses = filteredClasses.filter(c => {
    ...
    async function renderClassesPage(container) {
        ...
        if (!isOwner) {
            ...
            // If Access Level is 'home_only', filter the global branch list
            if (staffAccessLevel === 'home_only') {
                let allowedIds = [];
                if (user.allowedBranches && Object.keys(user.allowedBranches).length > 0) {
                    allowedIds = Object.keys(user.allowedBranches);
        ...
        // We show the filter if the user has access to more than 1 branch (regardless of total branches)
        if (hasEffectiveMulti) {
            let options = `<option value="all">${_('filter_all_branches')}</option>`;
            options += effectiveBranches.map(b => {
                const bName = (appState.currentLanguage === 'zh-TW' && b.name_zh) ? b.name_zh : b.name;
        ...
        const fetchAndRenderClasses = async () => {
            ...
            // --- FILTER: For 'home_only' Staff/Managers, limit data via allowedBranches ---
            if (!isOwner && staffAccessLevel === 'home_only') {
                const effectiveBranchIds = effectiveBranches.map(b => b.id);
                monthlyClasses = monthlyClasses.filter(c => {
                    const cBranch = c.branchId || branches[0].id;
                    return effectiveBranchIds.includes(cBranch);
                });
            }
    ...
    async function renderIncomePage(container) {
        ...
        // --- 1. Prepare Branch Filter HTML ---
        let branchFilterHTML = '';
        // Check effective length to decide if we show the dropdown
        const hasEffectiveMulti = effectiveBranches.length > 1;
        ...
        const renderIncomeDetails = async () => {
            ...
            // 1. Security Filter: Limit data to allowed branches first (for Manager/Staff)
            if (!isOwner) {
                const allowedIds = effectiveBranches.map(b => b.id);
                filteredClasses = filteredClasses.filter(c => {
                    const cBranch = c.branchId || branches[0].id; // Handle legacy
                    return allowedIds.includes(cBranch);
                });
            }

            // 2. UI Filter: Apply Dropdown Selection
            if (hasEffectiveMulti && branchSelect && branchSelect.value !== 'all') {
    ...
    const initDataListeners = () => {
        ...
        // --- START: MODIFIED LOGIC ---
        if (isAdmin) { // This now correctly routes owner, manager, and staff
            initOwnerListeners();
        } else {
            initMemberListeners();
        }
    ...
    const initOwnerListeners = () => {
        ...
        // --- NEW: Staff/Manager Plan Sync Logic ---
        // If the current user is Staff OR Manager, we must listen to the Owner's node to get the valid Subscription Plan.
        ...
        Object.entries(nonUserRefs).forEach(([key, ref]) => {
            dataListeners[key] = (snapshot) => {
                ...
                if (key === 'currentUser') {
                    ...
                    // If Owner, use own plan. If Staff/Manager, we ignore this and use the listener set up above.
