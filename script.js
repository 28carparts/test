...
Thank you for your understanding.
    ...
    function renderOwnerSchedule(container) {
        ...
        if (appState.scheduleScrollIndex !== null) {
            initialScrollIndex = appState.scheduleScrollIndex;
        } else {
            initialScrollIndex = daysToLookBack;
        }
    ...
    function renderMemberSchedulePage(container) {
        ...
        if (appState.scheduleScrollIndex !== null) {
            initialScrollIndex = appState.scheduleScrollIndex;
             ...
             initialScrollIndex = MEMBER_PAST_DAYS;
        }
        appState.scheduleScrollIndex = null; // Consume the index
                ...
                <form id="editMemberAccountForm" class="space-y-4">
                    ...
                    <div>
                        ...
                        <div class="flex gap-2">
                            <input type="text" id="editMemberCountryCode" class="form-input w-24" placeholder="e.g., 852" value="${member.phone.split(' ')[0].replace('+', '') || ''}">
                            <input type="tel" id="editMemberPhone" required class="form-input flex-grow" value="${formatDigitsWithSpaces(member.phone.split(' ').slice(1).join(''))}">
                        </div>
                    </div>
                    <div class="flex justify-center mt-8">
    ...
    function handleEditMemberAccountSubmit(e) {
        ...
        const phoneNumberDigits = form.querySelector('#editMemberPhone').value.replace(/\D/g, '');
        const phone = countryCode ? `+${countryCode} ${phoneNumberDigits}` : phoneNumberDigits;
    ...
    function renderMembersPage(container) {
            ...
            <div class="card p-6 md:p-8">
                <div class="flex flex-wrap gap-4 justify-between items-center mb-6">
                    ...
                    <div class="flex gap-4 items-center">
                        ...
                        <button id="addMemberBtn" class="bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2 px-4 rounded-lg transition">Add Member</button>
                    </div>
                </div>
        ...
        container.querySelector('#addMemberBtn').onclick = () => openMemberModal();
        updateTable();
    }
    ...
    function openMemberModal(memberToEdit = null) {
        DOMElements.memberModal.innerHTML = `
            <div class="bg-white p-8 rounded-2xl shadow-2xl w-full max-w-lg transform transition-all duration-300 scale-95 opacity-0 modal-content relative">
                <button class="modal-close-btn"><svg class="h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" /></svg></button>
                <h2 id="memberModalTitle" class="text-3xl font-bold text-slate-800 mb-6 text-center"></h2>
                <form id="memberForm" class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div><label for="memberPassword" class="block text-slate-600 text-sm font-semibold mb-2">Password</label><input type="password" id="memberPassword" ${memberToEdit ? '' : 'required'} class="form-input"></div>
                    </div>
        ...
        addCreditEntryBtn.onclick = () => {
            ...
            if (historyIdInput.value) {
                showMessageBox('Clear the current edit (by clicking another entry or re-opening) before adding a new one.', 'error');
                return;
            }

            showConfirmation('Add Credit Entry', `Confirm adding ${credits} credits for ${formatCurrency(amount)}? This will be added to the member's history.`, () => {
                const memberId = form.querySelector('#memberModalId').value;
                if (!memberId) {
                    showMessageBox('Cannot add credits to a new, unsaved member. Please save the member first.', 'error');
                    return;
                }
                const member = appState.members.find(m => m.id === memberId);
                if (member) {
                    const newPurchase = { id: Date.now(), date: new Date().toISOString(), amount: amount, credits: credits };
                    if (!member.purchaseHistory) member.purchaseHistory = [];
                    member.purchaseHistory.push(newPurchase);
                    member.credits = (member.credits || 0) + credits;
                    member.initialCredits = (member.initialCredits || 0) + credits;
                    
                    saveData();
                    _renderMemberPurchaseHistory(member, historyContainer, historyIdInput, purchaseAmountInput, creditsInput);
        ...
        if (memberToEdit) {
            ...
            form.querySelector('#memberPassword').placeholder = "Leave blank to keep current password";

            const fullPhone = memberToEdit.phone || '';
            const phoneParts = fullPhone.split(' ');
            if (phoneParts.length > 1 && phoneParts[0].startsWith('+')) {
                memberCountryCodeInput.value = phoneParts[0].substring(1);
                memberPhoneInput.value = formatDigitsWithSpaces(phoneParts.slice(1).join(''));
            } else {
                memberCountryCodeInput.value = '';
                memberPhoneInput.value = formatDigitsWithSpaces(fullPhone);
            }
            
            form.querySelector('#monthlyPlan').checked = memberToEdit.monthlyPlan;
            form.querySelector('#expiryDate').value = memberToEdit.expiryDate;
            ...
            form.querySelector('#memberModalId').value = '';
            const today = new Date();
            planStartDateInput.value = today.toISOString().split('T')[0];
            addCreditEntryBtn.disabled = true;
            addCreditEntryBtn.title = "Save member first to add credit entries";
            addCreditEntryBtn.classList.add('opacity-50', 'cursor-not-allowed');
        }
    ...
    function handleMemberFormSubmit(e) {
        ...
        const password = form.querySelector('#memberPassword').value;
        ...
        const phoneNumberDigits = form.querySelector('#memberPhone').value.replace(/\D/g, '');
        const fullPhoneNumber = countryCode ? `+${countryCode} ${phoneNumberDigits}` : phoneNumberDigits;
        ...
        if (id) {
            ...
            memberData.id = `m_${Date.now()}`;
            memberData.password = password;
            memberData.lastBooking = null;
            memberData.purchaseHistory = [];
            
            if (!isMonthly && topUpAmount > 0 && creditsToAdd > 0) {
                 const newPurchase = { id: Date.now(), date: new Date().toISOString(), amount: topUpAmount, credits: creditsToAdd };
                memberData.purchaseHistory.push(newPurchase);
                memberData.credits = creditsToAdd;
                memberData.initialCredits = creditsToAdd;
            } else {
                memberData.credits = 0;
                memberData.initialCredits = 0;
            }

            appState.members.push(memberData);
            showMessageBox('Member added successfully.', 'success');
        }
    ...
    function openTutorModal(tutorToEdit = null) {
        ...
        if (tutorToEdit) {
            ...
            const fullPhone = tutorToEdit.phone;
            const phoneParts = fullPhone.split(' ');
            if (phoneParts.length > 1 && phoneParts[0].startsWith('+')) {
                tutorCountryCodeInput.value = phoneParts[0].substring(1);
                tutorPhoneInput.value = formatDigitsWithSpaces(phoneParts.slice(1).join(''));
            } else {
                tutorCountryCodeInput.value = '';
                tutorPhoneInput.value = formatDigitsWithSpaces(fullPhone);
            }
    ...
    function handleTutorFormSubmit(e) {
        ...
        const phoneNumberDigits = form.querySelector('#tutorPhone').value.replace(/\D/g, '');
        const fullPhoneNumber = countryCode ? `+${countryCode} ${phoneNumberDigits}` : phoneNumberDigits;
    ...
    const init = () => {
        ...
        // --- Event Listeners ---
        DOMElements.openLoginModalBtn.onclick = () => {
            DOMElements.loginModal.innerHTML = `
                <div class="bg-white p-8 rounded-2xl shadow-2xl w-full max-w-sm transform transition-all duration-300 scale-95 opacity-0 modal-content relative">
                    ...
                    <form id="loginForm" class="space-y-4">
                        <div>
                            <label for="loginEmail" class="block text-slate-600 text-sm font-semibold mb-2">Email</label>
                            <input type="email" id="loginEmail" required class="form-input" placeholder="user@example.com">
                        </div>
                        <div>
                            <label for="loginPassword" class="block text-slate-600 text-sm font-semibold mb-2">Password</label>
                            <input type="password" id="loginPassword" required class="form-input" placeholder="••••••••">
                        </div>
                        <div class="flex justify-center pt-4">
                            <button type="submit" class="bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2 px-8 rounded-lg">Login</button>
                        </div>
                    </form>
                </div>`;
            openModal(DOMElements.loginModal);
            DOMElements.loginModal.querySelector('form').onsubmit = (e) => {
                e.preventDefault();
                handleLogin(
                    DOMElements.loginModal.querySelector('#loginEmail').value,
                    DOMElements.loginModal.querySelector('#loginPassword').value
                );
            };
        };
