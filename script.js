...
${_('whatsapp_closing')}
            ...
            if (tutor) {
                ...
                if (skill) {
                    newClsDataFromForm.payoutDetails = {
                        ...
                        // --- FIX: Save the Formula ID to the class ---
            ...
            <div class="space-y-8">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
                    ...
                    <div class="card p-6 flex flex-col h-full">
                        <div class="flex flex-wrap gap-4 justify-between items-center mb-4">
                            <h3 class="text-2xl font-bold text-slate-800">${_('header_salary_formulas')} (${appState.salaryFormulas.length})</h3>
                        </div>
                    ...
                    if (type === 'formula') {
                        if (target.classList.contains('edit-btn')) {
                            ...
                            showConfirmation(title, _('confirm_delete_generic_desc').replace('{name}', name), () => {
                                // 1. Check Usage by Tutors (Active Config)
                                const usedByTutors = appState.tutors.some(t => 
                                    t.skills && t.skills.some(s => s.salaryFormulaId === id && s.salaryType === 'custom')
                                );

                                // 2. Check Usage by Classes (Historical Data)
                                // We scan all classes to see if any payouts rely on this formula ID
                                ...
                                if (usedByTutors || usedByClasses) {
                                    // SOFT DELETE: Mark as deleted so it hides from UI, but keep for history
                                    database.ref('/salaryFormulas/' + id).update({ isDeleted: true }).then(() => {
                                        const msg = usedByTutors 
                                            ? "Formula archived. (Note: It is still assigned to active Tutors!)" 
                                            : "Formula archived to preserve class history.";
    ...
    function renderAdminLists() {
        ...
        // (Header selector remains the same)
    ...
    function addSkillRow(container, skill = null) {
        ...
        skillRow.innerHTML = `
            ...
            <div class="flex gap-1 rounded-lg bg-slate-200 p-1 salary-type-container flex-wrap">
                ...
                <button type="button" data-value="custom" class="salary-type-btn flex-1 p-1 rounded-md text-xs whitespace-nowrap text-orange-700 font-bold hover:bg-orange-100/50">${_('salary_type_custom')}</button>
            </div>
    ...
    function openFormulaModal(formulaToEdit = null) {
        // Create a temporary modal ID if not exists or reuse existing generic modal structure
        // Here we reuse sportTypeModal container for simplicity but inject unique content
        const modal = DOMElements.sportTypeModal; 
...
return Math.min(base + (A - threshold) * bonus, cap);`;
                ...
                <div class="flex-grow space-y-4">
                    ...
                    <form id="formulaForm">
                        ...
                        <div class="mb-2">
                            ...
                            <textarea id="formulaScript" class="form-input font-mono text-sm bg-slate-900 text-green-400 p-4 h-64 w-full" spellcheck="false"></textarea>
        ...
        const runPreview = () => {
            ...
            const context = {
                ...
                duration: 60, // Mock default
                credits: 1    // Mock default
            };
