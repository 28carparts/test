...
${_('whatsapp_closing')}
    ...
    async function renderSalaryPage(container) {
        ...
        const updateAvailableTutors = () => {
            ...
            if (activeTutors.length > 0) {
                ...
                // Clear the table immediately if no tutors match branch
                // FIX: Do NOT reset page here. updateSalaryTable handles rendering safely.
                updateSalaryTable(); 
                return;
            }

            // 4. Finally, update the table with the specific tutor selected
            // FIX: Removed automatic page reset here.
            updateSalaryTable();
        };
        ...
        const updateSalaryTable = () => {
            ...
            // We get branch ID again just for double-checking logic
            const selectedBranchId = branchSelect ? branchSelect.value : 'all';
        ...
        const renderSalaryDetails = (filteredClasses) => {
            ...
            // Safety: If data shrank, force within bounds.
            if (page > totalPages) page = totalPages;
            if (page < 1) page = 1; // Always at least 1 if empty or reset
            ...
            // Column Span
            const colSpan = hasMultipleBranches ? 7 : 6;
                        ...
                        <tbody>
                            ${paginatedClsDetails.map(c => `
                                <tr class="border-b border-slate-100">
                                    <td class="p-2 whitespace-nowrap text-slate-700">${formatShortDateWithYear(c.date)}</td>
                                ...
                                </tr>`).join('') || `<tr><td colspan="${colSpan}" class="text-center p-4 text-slate-500">${_('info_no_classes_in_period')}</td></tr>`}
                        </tbody>
                    </table></div>
    ...
    const initOwnerListeners = () => {
        ...
        dataListeners.users = (snapshot) => {
            ...
            const newUsersArray = firebaseObjectToArray(newUsersRaw);
            const oldUsersArray = appState.users || [];

            // 1. Always update the local data state so we have the latest info (even languages)
            appState.users = newUsersArray;

            // 2. Decide if we need to refresh the UI
            // If the schedule is active, we rely on the class listener, not the user listener
            if (appState.activePage === 'schedule') return;

            // 3. Compare Old vs New to see if a VISUAL update is needed
            let needsRender = false;

            if (newUsersArray.length !== oldUsersArray.length) {
                // If a user was added or removed, we must render
                needsRender = true;
            } else {
                // If counts are same, check specific relevant fields for changes
                // We convert arrays to Maps for faster lookup by ID
                const oldMap = new Map(oldUsersArray.map(u => [u.id, u]));
                
                // Fields that affect the Admin Tables (Members, Salary, etc.)
                // Note: 'language' is purposely EXCLUDED
                const visualKeys = [
                    'name', 'email', 'phone', 'role', 'isDeleted', 'homeBranchId',
                    'credits', 'initialCredits', 'expiryDate', // Legacy
                    'monthlyPlan', 'monthlyPlanAmount', 'planStartDate', 'paymentDueDate', 'monthlyPlanTierId',
                    // Wallet is an object, so we'll check it separately via stringify
                ];

                for (const newUser of newUsersArray) {
                    const oldUser = oldMap.get(newUser.id);
                    
                    if (!oldUser) { needsRender = true; break; } // Should be caught by length check, but safety first

                    // A. Check basic fields
                    const hasBasicChange = visualKeys.some(key => newUser[key] !== oldUser[key]);
                    if (hasBasicChange) { needsRender = true; break; }

                    // B. Check Complex Objects (Wallet)
                    // We use JSON.stringify for a quick deep comparison of the wallet object
                    if (JSON.stringify(newUser.wallet) !== JSON.stringify(oldUser.wallet)) {
                        needsRender = true; break;
                    }
                    
                    // C. Check Purchase/Payment History (if counting credits on Account page)
                    // Checking length is usually enough for a list update trigger
                    const newPurchLen = newUser.purchaseHistory ? Object.keys(newUser.purchaseHistory).length : 0;
                    const oldPurchLen = oldUser.purchaseHistory ? Object.keys(oldUser.purchaseHistory).length : 0;
                    if (newPurchLen !== oldPurchLen) { needsRender = true; break; }
                }
            }

            // 4. Only render if something visual changed
            if (needsRender) {
                renderCurrentPage();
            }
        };
