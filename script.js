...
${_('whatsapp_closing')}
    ...
    function renderAccountPage(container) {
        ...
        // 2. Identify Unique Credit Types used in bookings
        ...
        const renderFilterPills = () => {
            ...
            if (!pillsContainer || usedCreditTypeIds.size <= 1) return; // Hide if 0 or 1 type
            ...
            pillsHTML += `<button class="filter-pill px-3 py-1 text-xs font-bold rounded-full transition-all ${allClass}" data-type-id="all">${_('filter_all')}</button>`;
            ...
            usedCreditTypeIds.forEach(typeId => {
                ...
                pillsHTML += `<button class="${className}" style="${style}" data-type-id="${typeId}">${typeName}</button>`;
            });
    ...
    function _renderMemberPurchaseHistory(member, container, historyIdInput, purchaseAmountInput, creditsInput, onEditStart, creditTypeSelect, expiryDateInput) {
        ...
        if (purchaseHistory.length > 0) {
            ...
            container.innerHTML = sortedHistory.map(p => {
                ...
                return `
                    <div class="flex items-start gap-2 bg-slate-50 p-2 rounded-md transition border border-slate-100 mb-2 ${isDeleted ? 'opacity-50' : ''}" data-history-item-id="${p.id}">
                        <div class="flex-grow cursor-pointer hover:bg-slate-100 p-1 rounded-md -ml-1">
                            <div class="font-bold text-slate-800 text-sm mb-0.5 ${isDeleted ? 'line-through' : ''}">${formatShortDateWithYear(p.date)}:</div>
                            <div class="text-sm text-slate-700 font-medium mb-0.5">${entryText} ${typePill}</div>
                            <div class="text-xs text-slate-500">${costPerCreditText}</div>
                            ${auditMessage}
                        </div>
                        <button type="button" class="remove-history-btn text-slate-400 hover:text-red-500 font-bold text-lg p-1 ${isDeleted ? 'hidden' : ''}" data-history-id="${p.id}" title="Remove entry">
    ...
    function openMemberModal(memberToEdit) {
                ...
                <form id="memberForm" class="space-y-4">
                    ...
                    <div class="pt-4 border-t">
                        ...
                        <div id="creditFields" class="space-y-4">
                            ...
                            <div id="purchaseHistoryContainer" class="space-y-2 max-h-32 overflow-y-auto p-1 border-t border-slate-100 pt-2"></div>
                        </div>
        ...
        // --- NEW LOGIC: Add Placeholder and specific Legacy selection ---
        let tierOptionsHTML = `<option value="" disabled selected>${_('placeholder_select_monthly_tier')}</option>`;
        
        if (showLegacyOption) {
            // We use a data attribute to uniquely identify the Legacy option in the DOM,
            // even though its value is "" (same as placeholder)
        ...
        if (memberToEdit.monthlyPlanTierId) {
            ...
            // If user has a plan but no ID, they are Legacy.
            // We must explicitly select the legacy option DOM element because 'value=""' is ambiguous.
            if (showLegacyOption) {
                const legacyOpt = monthlyTierSelect.querySelector('option[data-is-legacy="true"]');
                if (legacyOpt) legacyOpt.selected = true;
            } else {
                // Otherwise (New Plan), select the Placeholder (index 0)
        ...
        const setPaymentButtonToAddMode = () => {
            ...
            // Reset Dropdown to Current Member Tier using the NEW selection logic
            if (memberToEdit.monthlyPlanTierId) {
                monthlyTierSelect.value = memberToEdit.monthlyPlanTierId;
            } else {
                if (showLegacyOption) {
        ...
        const updatePlanFields = () => { 
            ...
            // Visual State: Toggle button styles based on selection
            if (isMonthly) {
                ...
                if (!planStartDateInput.value) planStartDateInput.value = getIsoDate(new Date());
            } else {
        ...
        // Attach listeners to buttons to toggle the hidden checkbox
    ...
    async function openMemberBookingHistoryModal(member) {
        ...
        try {
            ...
            // 1. Identify Unique Credit Types
            const usedCreditTypeIds = new Set();
            memberBookings.forEach(cls => {
                const booking = cls.bookedBy[member.id];
                const typeId = booking.creditTypeId || cls.costCreditTypeId || 'general';
            ...
            const getPillsHTML = () => {
                if (usedCreditTypeIds.size <= 1) return ''; 
                ...
                pillsHTML += `<button class="modal-filter-pill px-3 py-1 text-xs font-bold rounded-full transition-all ${allClass}" data-type-id="all">${_('filter_all')}</button>`;
