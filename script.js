...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const renderSubscriptionBadge = () => {
        ...
        // --- A. SUPER ADMIN (Cosmic Nebula) ---
        if (role === 'superadmin') {
             if (!document.getElementById('super-admin-style')) {
                const style = document.createElement('style');
                style.id = 'super-admin-style';
                style.innerHTML = `
                    @keyframes superBadgeAnim {
                        0% { background-position: 0% 50%; }
                        50% { background-position: 100% 50%; }
                        100% { background-position: 0% 50%; }
                    }
                    .super-badge-style {
                        background: linear-gradient(90deg, #4f46e5, #db2777, #0ea5e9, #4f46e5);
                        background-size: 300% 100%;
                        animation: superBadgeAnim 4s ease infinite;
                        color: white;
            ...
            badge.classList.add('super-badge-style');
            badge.textContent = 'SUPER';
        } 
        
        // --- B. MANAGER (Purple Identity) ---
        else if (role === 'manager') {
            // Using Hex codes to guarantee visibility
            badge.style.backgroundColor = '#9333ea'; // Vibrant Purple
            badge.style.color = '#ffffff';
            badge.style.border = '1px solid #7e22ce';
            badge.textContent = 'MANAGER';
        }

        // --- C. STAFF (Blue Identity) ---
        else if (role === 'staff') {
            // Using Hex codes to guarantee visibility
            badge.style.backgroundColor = '#2563eb'; // Vibrant Blue
            badge.style.color = '#ffffff';
        ...
        // --- D. OWNER (Plan-Based) ---
            ...
            // Get the plan (Prioritize studioPlan which is synced)
            const plan = (appState.studioPlan || appState.currentUser.subscriptionStatus || 'basic').toLowerCase();
...
${_('whatsapp_closing')}
    ...
    function renderConsistencyHeatmap(bookings, memberId) {
        ...
        for (let w = 0; w <= weeksToShow; w++) {
            ...
            // Width 16px = dot(12px) + gap(4px)
            monthLabelsHTML += `<div class="text-[10px] text-slate-400 font-medium text-left overflow-visible whitespace-nowrap" style="width: 16px;">${label}</div>`;
        }
        ...
        // Color Palette
        const colors = {
            0: '#f1f5f9', 1: '#4ade80', 2: '#facc15', 3: '#f97316', 4: '#ef4444'
        };
        ...
        while (loopDate <= today) {
            ...
            if (count === 1) bgColor = colors[1];
            else if (count === 2) bgColor = colors[2];
            else if (count === 3) bgColor = colors[3];
            else if (count >= 4) bgColor = colors[4];
            ...
            gridHTML += `<div class="w-3 h-3 rounded-sm cursor-help transition-opacity hover:opacity-75 relative" style="background-color: ${bgColor}; ${separatorStyle}" title="${tooltipText}"></div>`;

            loopDate.setDate(loopDate.getDate() + 1);
        }

        // --- NEW: Generate Day Labels (Left Column) ---
        // We fetch from i18n and apply styling to match Month Labels (text-[10px] font-medium)
        ...
        return `
            <div class="card p-6 mb-8 border border-slate-200 shadow-sm">
                ...
                <div class="flex">
                    <!-- NEW: Left Column (Day Labels) -->
                    <div class="flex flex-col shrink-0 mr-2">
                        <!-- Spacer: Matches the height of the Month Labels row so days align with dots -->
                        <div class="text-[10px] mb-1 invisible" aria-hidden="true">&nbsp;</div>
                        <!-- Day Grid: Matches spacing of the dot grid -->
                    ...
                    <div id="heatmap-scroll-container" class="overflow-x-auto pb-2 scroll-smooth">
                        <div style="width: max-content;">
                            <!-- Month Labels -->
                            <div class="flex gap-[4px] mb-1 pl-[1px]">
                                ${monthLabelsHTML}
                            </div>
                            <!-- Dots Grid -->
                            <div style="display: grid; grid-template-rows: repeat(7, 1fr); grid-auto-flow: column; gap: 4px;">
                                ${gridHTML}
                            </div>
                        </div>
                    </div>
                ...
                <div class="flex items-center justify-end gap-1.5 mt-3 text-xs text-slate-400 font-medium">
                    ...
                    <div class="w-3 h-3 rounded-sm" style="background-color: ${colors[0]};" title="0"></div>
                    <div class="w-3 h-3 rounded-sm" style="background-color: ${colors[1]};" title="1"></div>
                    <div class="w-3 h-3 rounded-sm" style="background-color: ${colors[2]};" title="2"></div>
                    <div class="w-3 h-3 rounded-sm" style="background-color: ${colors[3]};" title="3"></div>
                    <div class="w-3 h-3 rounded-sm" style="background-color: ${colors[4]};" title="4+"></div>
    ...
    function _renderMemberPurchaseHistory(member, container, historyIdInput, purchaseAmountInput, creditsInput, onEditStart, creditTypeSelect, expiryDateInput, editLabel) {
        ...
        // Inline style to auto-hide the delete button when the parent row is highlighted
        const styleTag = `<style>.history-entry-highlighted .remove-history-btn { display: none !important; }</style>`;
        ...
        if (purchaseHistory.length > 0) {
            ...
            container.innerHTML = styleTag + `<div class="text-xs text-slate-400 mb-2 text-right font-medium">${fullText}</div>` + sortedHistory.map(p => {
            ...
            if (isSuperAdmin) {
                ...
                if (owners.length > 0) {
                    const ownerRows = owners.map(u => {
                        ...
                        let badgeText = 'BASIC';
                        let badgeStyle = ''; // NEW: Helper for inline styles
                        
                        if (currentPlan === 'premium') {
                            // Inline style for Premium Gold Gradient
                            badgeStyle = 'background: linear-gradient(to right, #ca8a04, #eab308, #ca8a04); color: white; border: 1px solid rgba(250, 204, 21, 0.5);';
                            badgeClass = 'shadow-sm'; // Keep layout classes, remove color classes
                            badgeText = 'PREMIUM';
                        } else if (currentPlan === 'pro') {
                            // Inline style for Pro Orange Gradient
                            badgeStyle = 'background: linear-gradient(to right, #fbbf24, #f97316); color: white;';
                            ...
                            <div class="flex items-center gap-3 w-full sm:w-auto">
                                ...
                                <div class="min-w-0">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <!-- UPDATED BADGE SPAN -->
                                        <span class="text-[10px] uppercase font-bold px-2 py-0.5 rounded-full ${badgeClass}" style="${badgeStyle}">${badgeText}</span>
                                    </div>
                ...
                <form id="changePlanForm">
                    <div class="space-y-3">
                        ...
                        <label class="flex items-center p-3 border rounded-lg cursor-pointer hover:bg-yellow-50 transition border-slate-200 has-checked:border-yellow-500 has-checked:bg-yellow-50 relative overflow-hidden group">
                            ...
                            <div class="ml-3 relative z-10">
                                <!-- INLINE STYLE FIX: Hardcoded Dark Yellow/Gold Colors -->
                                <span class="block text-sm font-bold" style="color: #a16207;">${_('plan_name_premium')}</span>
                                <span class="block text-xs" style="color: rgba(202, 138, 4, 0.7);">${_('plan_desc_premium')}</span>
                            </div>
                        </label>
