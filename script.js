...
${_('whatsapp_closing')}
    ...
    async function openJoinedMembersModal(cls) {
        ...
        if (bookedMembers.length === 0) {
            ...
            // --- Handler 2: Locked Checkboxes (Long Press Logic) ---
            listEl.querySelectorAll('.locked-checkbox').forEach(checkbox => {
                let pressTimer;
                let isLongPress = false;

                // Prevent immediate checking via click
                checkbox.addEventListener('click', (e) => {
                    e.preventDefault(); // Always stop the browser's default toggle
                    if (!isLongPress) {
                        // Short Click: Show Toast explaining why it's locked
                        // CHANGED: Uses '{date}' placeholder now
                ...
                const startPress = (e) => {
                    ...
                    pressTimer = setTimeout(() => {
                        ...
                        // LOGIC FIX: Check current state to determine action (Toggle)
                        if (checkbox.checked) {
                            // --- CASE A: ALREADY CHECKED -> UNDO (Revert) ---
                            showConfirmation(
                            ...
                            // --- CASE B: UNCHECKED -> FORCE CHECK-IN ---
                            showConfirmation(
                                ...
                                () => {
                                    ...
                                    attendedRef.set(true).then(() => {
                                        // Update UI: Check and visually unlock (remove gray scale)
                ...
                // Mouse Events
                checkbox.addEventListener('mousedown', startPress);
                checkbox.addEventListener('mouseup', cancelPress);
                checkbox.addEventListener('mouseleave', cancelPress);

                // Touch Events
                checkbox.addEventListener('touchstart', startPress);
                checkbox.addEventListener('touchend', cancelPress);
                checkbox.addEventListener('touchmove', cancelPress); // Cancel if scrolling
            });
        }
