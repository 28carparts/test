...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const canManageBranch = (user, targetBranchId) => {
        // 1. Owners and Managers (usually) have full access
        // Note: If you want Managers to be restricted, remove the 'isManager' check here
        if (user.role === 'owner' || user.role === 'manager') return true;
        
        // 2. Staff: Global Write
        if (user.staffAccessLevel === 'global_write') return true;

        // 3. Staff: Specific Branch Access
    ...
    const renderCurrentPage = () => {
        ...
        if (plan === 'basic' && !isOwner) {
            const allowedBasicPages = ['schedule', 'check-in', 'account', 'filter'];
