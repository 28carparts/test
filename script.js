    ...
    function renderCheckInPage(container) {
        ...
        const renderUpcomingList = (targetContainerId) => {
            ...
            if (todaysUnattendedBookings.length > 0) {
                upcomingSection.innerHTML = `<h5 class="text-center font-semibold text-slate-600 mb-2 pt-3 border-t border-slate-100">${_('header_upcoming_checkins')}</h5>` +
        ...
        const renderScanStudioTab = () => {
            ...
            // 1. Added arbitrary Tailwind selectors [&_video]:... to force the video element to cover the square
            // 2. Reduced mt-4/mb-4/min-h-3rem to smaller values to reduce whitespace
            contentArea.innerHTML = `
                ...
                <p class="text-xs text-slate-400 mb-2">${_('check_in_prompt')}</p>
    ...
    function handleMemberScanningStudio(decodedText, onResumeCallback) {
        ...
        database.ref(`/classes/${targetClass.id}/attendedBy/${memberId}`).set(true)
            .then(() => {
                ...
                // 1. Prepare Destination State
                appState.highlightBookingId = targetClass.id;
                showMessageBox(_('success_check_in_complete'), 'success');

                // 2. Stop Scanner Gracefully
                if (window.memberScanner) {
                    window.memberScanner.stop()
                        .catch(err => console.warn("Scanner stop warning:", err)) // Ignore if already stopped
                        .finally(() => {
                            // 3. Clear Instance & Nullify
                            // IMPORTANT: Setting to null prevents switchPage from trying to stop it again
                            window.memberScanner.clear();
                            window.memberScanner = null; 
