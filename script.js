...
${_('whatsapp_closing')}
            ...
            if (tutor) {
                ...
                if (skill) {
                    newClsDataFromForm.payoutDetails = {
                        ...
                        // FIX: If Employee OR Custom Formula, force salaryValue to 0.
                        // Otherwise, use the value from the skill settings.
            ...
            <div class="space-y-8">
                ...
                <div class="card p-6 md:p-8">
                    <div class="flex justify-between items-center mb-4">
                        ...
                        <!-- STATUS INDICATOR WITH QUICK CLEAR BUTTON -->
                        <div class="flex items-center gap-2">
                            ${currentAnnounce ? `
                ...
                <!-- NEW GRID: Export Schedule & Custom Formulas -->
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
                    
                    <!-- Export Schedule Card -->
                    ...
                    <!-- Custom Formulas Card (Moved Here) -->
                    ${isOwner ? `
                    <div class="card p-6 flex flex-col h-full">
                        <div class="flex flex-wrap gap-4 justify-between items-center mb-4">
                            <!-- FIX: Added ID 'formulasHeader' and filtered the initial count -->
                            <h3 id="formulasHeader" class="text-2xl font-bold text-slate-800">
                                ${_('header_salary_formulas')} (${appState.salaryFormulas.filter(f => !f.isDeleted).length})
                            </h3>
                        </div>
                        ...
                        <div class="flex-grow">
                            <ul id="formulasList" class="admin-list space-y-2">
                                ${appState.salaryFormulas.length === 0 ? `<li class="text-center text-slate-500 p-4">No formulas defined</li>` : 
                                appState.salaryFormulas.map(f => `
                ...
                fList.addEventListener('click', (e) => {
                    ...
                    if (type === 'formula') {
                        if (target.classList.contains('edit-btn')) {
                            ...
                            // 1. PRE-CHECK: Look for Active Tutors using this formula
                            const activeTutors = appState.tutors.filter(t => 
                                t.skills && t.skills.some(s => s.salaryFormulaId === id && s.salaryType === 'custom')
                            );

                            if (activeTutors.length > 0) {
                                // BLOCKING STATE: Tutors are using it. Show list and stop.
                                const listHtml = activeTutors.map(t => `
                                    <li class="flex justify-between items-center bg-orange-50 p-2 rounded border border-orange-100 text-sm">
                                ...
                                DOMElements.confirmationModal.innerHTML = `
                                    <div class="bg-white p-8 rounded-2xl shadow-2xl w-full max-w-md transform transition-all duration-300 scale-95 opacity-0 modal-content">
                                        ...
                                        <div class="text-center">
                                            ...
                                            <h2 class="text-2xl font-bold text-slate-800 mb-2">${_('error_formula_in_use_title')}</h2>
                                            <p class="text-slate-600 mb-4 text-sm">${_('error_formula_in_use_desc')}</p>
                                        </div>
                                        <ul class="space-y-2 mb-6 max-h-48 overflow-y-auto">
                                            ${listHtml}
                                        </ul>
                            ...
                            // 2. HAPPY PATH: No tutors involved. Ask for final confirmation.
                            const title = _('confirm_delete_generic_title').replace('{type}', _('label_formula_name') || 'Formula');
                            
                            showConfirmation(title, _('confirm_delete_generic_desc').replace('{name}', name), () => {
                                
                                // 3. Check Usage by Classes (Historical Data only now, since Tutors are cleared)
                                const usedByClasses = appState.classes.some(c => 
                                    c.payoutDetails && c.payoutDetails.salaryFormulaId === id && c.payoutDetails.salaryType === 'custom'
                                );

                                if (usedByClasses) {
                                    // SOFT DELETE: Mark as deleted to preserve history, but hide from UI
                                    database.ref('/salaryFormulas/' + id).update({ isDeleted: true }).then(() => {
                                        showMessageBox("Formula archived to preserve class history.", 'success');
                                    });
                                } else {
                                    // HARD DELETE: Safe to remove completely (clean slate)
                                    database.ref('/salaryFormulas/' + id).remove().then(() => {
                                        showMessageBox("Formula deleted permanently.", 'info');
                                    });
                                }
                            });
                        }
    ...
    function renderAdminLists() {
        ...
        const formulasHeader = document.getElementById('formulasHeader'); // Get the header reference

        if (formulasList) {
            // FILTER: Only show active (not deleted) formulas in the UI
            const activeFormulas = appState.salaryFormulas.filter(f => !f.isDeleted);

            // FIX: Update the header count dynamically whenever data changes
            if (formulasHeader) {
    ...
    async function renderSalaryPage(container) {
        ...
        const renderSalaryDetails = (filteredClasses) => {
            ...
            const clsDetails = filteredClasses.map(cls => {
                ...
                if (cls.payoutDetails) { // Removed the 'salaryValue' check because scripts might not have it
                    const { salaryType, salaryValue, salaryFormulaId } = cls.payoutDetails;
                    ...
                    // --- NEW: Custom Formula Logic for Salary Page ---
                    else if (salaryType === 'custom') {
                        const formula = appState.salaryFormulas.find(f => f.id === salaryFormulaId);
                        if (formula && formula.script) {
                            const context = {
                                attendees: enrollmentsCount,
                                revenue: clsGrossRevenue,
                                duration: cls.duration || 60,
                                credits: cls.credits || 0
                            };
                            earnings = executeSalaryScript(formula.script, context);
                            calculation = `${_('salary_type_custom')}: ${formula.name}`;
                        } else {
            ...
            filteredClasses.forEach(cls => {
                ...
                if (clsGrossRevenue > 0 || isFutureView) {
                    if (cls.payoutDetails) {
                        ...
                        // --- NEW: Custom Formula Logic for Statistics Page ---
                        else if (salaryType === 'custom') {
                            const formula = appState.salaryFormulas.find(f => f.id === salaryFormulaId);
                            if (formula && formula.script) {
                                const bookingsCount = cls.bookedBy ? Object.keys(cls.bookedBy).length : 0;
                                const context = {
                                    attendees: bookingsCount,
                                    revenue: clsGrossRevenue,
                                    duration: cls.duration || 60,
                                    credits: cls.credits || 0
                                };
                                clsPayout = executeSalaryScript(formula.script, context);
                            }
                        }
                    }
                }
        ...
        const renderIncomeDetails = async () => {
            ...
            filteredClasses.forEach(cls => {
                ...
                if (cls.payoutDetails) {
                    ...
                    // --- NEW: Custom Formula Logic for Income Page ---
                    else if (salaryType === 'custom') {
                        const formula = appState.salaryFormulas.find(f => f.id === salaryFormulaId);
                        if (formula && formula.script) {
                            const context = {
                                attendees: bookingsForClass.length,
                                revenue: clsGross,
                                duration: cls.duration || 60,
                                credits: cls.credits || 0
                            };
