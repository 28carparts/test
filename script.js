...
${_('whatsapp_closing')}
     ...
     * Shows the last 24 weeks (168 days) of activity.
     * 
    ...
    function renderConsistencyHeatmap(bookings, memberId) {
        ...
        bookings.forEach(cls => {
            // Only count if marked as Attended
        ...
        // Calculate Start Date: Go back 24 weeks
        const weeksToShow = 24; 
        const startDate = new Date(today);
        startDate.setDate(today.getDate() - (weeksToShow * 7));
        
        // Adjust start date to the previous Sunday to ensure grid alignment
        startDate.setDate(startDate.getDate() - startDate.getDay());

        // 3. Generate Grid Items
        let gridHTML = '';
        const loopDate = new Date(startDate);
        
        // Color Palette (Temperature Gradient)
        const colors = {
            0: '#f1f5f9', // Slate-100 (Empty)
            1: '#4ade80', // Green-400 (Active)
            2: '#facc15', // Yellow-400 (Heating Up)
            3: '#f97316', // Orange-500 (High Intensity)
            4: '#ef4444'  // Red-500 (On Fire)
        };
        ...
        while (loopDate <= today) {
            ...
            // Tooltip Text
            const tooltipText = _('label_heatmap_tooltip')
                .replace('{count}', count)
                .replace('{date}', displayDate);

            // Grid Item
            gridHTML += `<div class="w-3 h-3 rounded-sm cursor-help transition-opacity hover:opacity-75" style="background-color: ${bgColor};" title="${tooltipText}"></div>`;
        ...
        // FIX: Added 'whitespace-nowrap' to the span class list below to prevent text wrapping
        return `
