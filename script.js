...
${_('whatsapp_closing')}
    ...
    function openCheckInModal() {
        ...
        // --- SUB-FUNCTION: Owner Scans Member (Updated Layout) ---
        const renderOwnerScan = () => {
            ...
            // Updated HTML to match Member View (Square Camera)
            contentArea.innerHTML = `
        ...
        // --- SUB-FUNCTION: Owner Shows Dynamic QR (Updated UI) ---
        const renderOwnerShow = () => {
            ...
            // 1. Determine Initial Selected Branch
            // Priority: Currently Selected Schedule Branch > First Branch > 'main'
            let selectedBranchId = appState.selectedScheduleBranch || (branches.length > 0 ? branches[0].id : 'main');

            // 2. Build Segmented Control HTML
            let branchControlHTML = '';
            
            if (branches.length > 1) {
                // If many branches, use a horizontal scroll container
                const buttonsHtml = branches.map(b => {
            ...
            // Bind Click Events for Branch Buttons
            ...
            buttons.forEach(btn => {
                btn.onclick = () => {
                    ...
                    if (newId !== selectedBranchId) {
                        ...
                        // Reset Interval to align with user interaction
