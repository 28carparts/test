...
return Math.min(base + (A - threshold) * bonus, cap);`;
    ...
    async function renderSalaryPage(container) {
        ...
        const renderSalaryDetails = (filteredClasses) => {
            ...
            const clsDetails = filteredClasses.map(cls => {
                ...
                // --- UPDATED: Use Centralized Helper for Math ---
                const earnings = calculateTutorPayout(cls, clsGrossRevenue, enrollmentsCount, appState.salaryFormulas);
                // -----------------------------------------------

                // --- FIX: This line was missing! Accumulate the total ---
                totalEarnings += earnings;
                // --------------------------------------------------------

                // UI String Construction (Keep strictly for display text)
                ...
                if (cls.payoutDetails) { 
                    ...
                    if (salaryType === 'perCls') {
                        ...
                        // UPDATED: Removed "Custom Script:" prefix to save space
    ...
    const initOwnerListeners = () => {
        ...
        // --- NEW: Staff Plan Sync Logic ---
        // If the current user is Staff, we must listen to the Owner's node to get the valid Subscription Plan.
            ...
            const handleOwnerPlanUpdate = (snapshot) => {
                ...
                if (ownerData) {
                    ...
                    if (owner) {
                        ...
                        // Refresh UI elements that depend on the Plan
                        renderNav();
                        renderSubscriptionBadge();
                        // Refresh current page if it relies on permissions
                        if (['admin', 'check-in', 'filter', 'income', 'salary', 'statistics'].includes(appState.activePage)) {
                            renderCurrentPage();
                        }
                    }
                }
        ...
        Object.entries(nonUserRefs).forEach(([key, ref]) => {
            dataListeners[key] = (snapshot) => {
                ...
                if (key === 'currentUser') {
                    ...
                    // If Owner, use own plan. If Staff, we ignore this and use the listener set up above.
    ...
    const initMemberListeners = () => {
        ...
        // --- DELETED REDUNDANT CLEANUP BLOCK HERE ---
