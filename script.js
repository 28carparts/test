...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const renderFooterInfo = () => {
        ...
        const parts = sortedBranches.map(b => {
            ...
            let content = '';
            if (isMulti) {
                content = `<strong class="text-slate-700">${name}:</strong> ${details}`;
            } else {
                content = details;
            }

            // CSS Update: Use 'block' on mobile (new line), 'inline' on small screens+
            return `<span class="block sm:inline">${content}</span>`;
        }).filter(Boolean);

        // CSS Update: Separator is 'hidden' on mobile, 'inline' on small screens+
        container.innerHTML = parts.join('<span class="hidden sm:inline mx-2 text-slate-300">|</span>');
    };
            ...
            listEl.innerHTML = bookedMembers.map(member => {
                ...
                return `<div class="bg-slate-100 p-3 rounded-lg flex justify-between items-center mb-2">
                    <div class="flex items-center min-w-0 flex-1 mr-2">
                       <input type="checkbox" data-member-id="${member.id}" class="h-5 w-5 rounded text-indigo-600 mr-4 flex-shrink-0 attendance-checkbox" ${isAttended ? 'checked' : ''}>
                       <div class="min-w-0">
