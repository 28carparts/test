...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function calculateClsRevenueAndPayout(cls, allUsers, allTutors, allClasses) {
        ...
        // Construct the specific booking list for this single class
        ...
        // This will now use the updated multi-credit logic inside calculateRevenueForBookings
    ...
    const renderNav = () => {
        ...
        if (isAdmin) {
            ...
            navButtonsHTML.splice(3, 0, `<button id="navGoToBtn" class="nav-btn font-semibold px-3 py-1 text-sm sm:text-base">${_('nav_goto')}</button>`);

            // --- UPDATE THIS BLOCK ---
            if (isOwner) {
                // 1. Insert Income after Classes (Index 5)
                navButtonsHTML.splice(5, 0, `<button data-page="income" class="nav-btn font-semibold px-3 py-1 text-sm sm:text-base">${_('nav_income')}</button>`);
                
                // 2. Insert Salary after Income (Index 6)
                navButtonsHTML.splice(6, 0, `<button data-page="salary" class="nav-btn font-semibold px-3 py-1 text-sm sm:text-base">${_('nav_salary')}</button>`);
                
                // 3. Insert Statistics after Salary (Index 7)
                navButtonsHTML.splice(7, 0, `<button data-page="statistics" class="nav-btn font-semibold px-3 py-1 text-sm sm:text-base">${_('nav_statistics')}</button>`);
            }
            // ------------------------
        } else { 
            navButtonsHTML = [
        ...
        DOMElements.mainNav.querySelectorAll('button').forEach(btn => {
            ...
            } else if (btn.id === 'navAnnounceBtn') {
                // Keep strictly for safety, though element is removed from HTML above
    ...
    const renderSubscriptionBadge = () => {
        ...
        // 3. Only show for Owners
        if (appState.currentUser?.role !== 'owner') return;

        // 4. Get the plan (Default to 'basic' if missing)
        const plan = (appState.currentUser.subscriptionStatus || 'basic').toLowerCase();
    ...
    const renderCurrentPage = () => {
        ...
        const ownerOnlyPages = ['statistics', 'salary', 'income']; // Pages only for Owner
        ...
        } else if (pageIdToRender === 'statistics' || pageIdToRender === 'classes' || pageIdToRender === 'income') { // Added 'income'
            if (appState.users.length === 0) {
                pageElement.innerHTML = `<p class="text-center p-8">${_('status_loading_required_data')}</p>`;
                database.ref('/users').once('value').then(snapshot => {
                    appState.users = firebaseObjectToArray(snapshot.val());
    ...
    function openDeleteClsNotifyModal(cls) {
        ...
        modal.querySelector('#final-delete-btn').onclick = () => {
            ...
            const refundPromises = bookedMemberIds.map(memberId => {
                ...
                // --- UPDATED REFUND LOGIC FOR CLASS DELETION ---
                ...
                if (member && !attended && isCreditBooking) {
...
${_('whatsapp_closing')}
    ...
    function openBookingModal(cls) {
        ...
        if (paymentMethod === 'monthly') {
            ...
            if (creditIneligibilityReason === 'credits_expired') {
                ...
                balanceHtml = `<span class="inline-block text-sm font-bold px-3 py-1 rounded-full bg-orange-100 text-orange-600 border border-orange-200" title="${_('error_branch_restricted_credits')}">${formatCredits(userBalance)} (${_('info_branch_restricted')})</span>`;
            } else {
                balanceHtml = `<span class="inline-block text-sm font-bold px-3 py-1 rounded-full" style="${creditPillStyle}">${formatCredits(userBalance)} ${creditTypeName}</span>`;
            }
    ...
    function handleClsFormSubmit(e) {
        ...
        if (conflict) {
            ...
            showConfirmation(
                ...
                proceedWithSave // Callback if they click "Confirm"
            );
        } else {
    ...
    function createClsElement(cls) {
        ...
        // --- STYLE UPDATES START ---
        // 1. Removed the inset shadow to eliminate the "double-edge" look.
        // 2. Kept the outer shadow for depth.
        // 3. Kept the slight gradient for the "sunshine" feel.
        el.className = `cls-block p-3 rounded-xl shadow-lg text-white mb-2 flex flex-col justify-between border border-white/20 transition-transform duration-200 hover:scale-[1.02] ${isEndedToday ? 'cls-block-ended' : ''}`;

        const baseColor = sportType?.color || '#64748b';
        
        // Apply the radiant background logic
        el.style.backgroundColor = baseColor;
        el.style.backgroundImage = 'linear-gradient(135deg, rgba(255,255,255,0.25) 0%, rgba(0,0,0,0.1) 100%)';
        // UPDATED: Removed 'inset 0 1px 0 rgba(255, 255, 255, 0.3)' from the end
        ...
        if (creditTypeDef) {
            // Use specific type name (e.g., "1 Yoga Pass")
            creditDisplayName = getCreditTypeName(creditTypeDef);
        } else {
            // Legacy/General fallback: Keep standard "credit"/"credits" grammar
        ...
        el.innerHTML = `
            ...
            <div class="mt-2 flex justify-between items-center">
                ${isAdmin
                    ? (window.matchMedia('(any-pointer: fine)').matches
                        ? `<p class="font-bold text-base bg-black/20 backdrop-blur-sm px-2 py-1 rounded-md inline-block time-slot time-slot-editable shadow-inner">${getTimeRange(cls.time, cls.duration)}</p>`
                        : `<div class="relative inline-block">
                               <p class="font-bold text-base bg-black/20 backdrop-blur-sm px-2 py-1 rounded-md shadow-inner">${getTimeRange(cls.time, cls.duration)}</p>
                               <input type="time" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer" value="${cls.time}" />
                           </div>`
                    )
                    : `<p class="font-bold text-base bg-black/20 backdrop-blur-sm px-2 py-1 rounded-md inline-block shadow-inner">${getTimeRange(cls.time, cls.duration)}</p>`
                }
    ...
    function _reSortDayColumn(dateIso) {
        ...
        // 4. Re-append the newly sorted (and filtered) class elements
    ...
    function renderOwnerSchedule(container) {
        ...
        // 4. Attach Event Listeners for Switcher
        if (hasMultipleBranches) {
            if (branches.length <= 3) {
                container.querySelectorAll('.branch-switch-btn').forEach(btn => {
                    btn.onclick = () => {
                        ...
                        if (appState.selectedScheduleBranch !== newBranchId) {
                            ...
                            // This re-render will hit the Optimization Trigger above
        ...
        // 5. Render Carousel (Using the data currently in appState.classes)
        ...
        // Filter Logic: 
        // Even though we optimized the listener, we still filter locally 
        // to ensure no cross-contamination (e.g. race conditions) and apply other filters (Day/Tutor)
        let filteredClasses = appState.classes;
        if (hasMultipleBranches && appState.selectedScheduleBranch) {
            filteredClasses = appState.classes.filter(c => {
                // If the class has no branchId (legacy) and we are on Main Branch (index 0), show it.
                // Or if ids match.
                const cBranch = c.branchId || branches[0].id;
                return cBranch === appState.selectedScheduleBranch;
            });
        }

        const initialScrollIndex = getInitialScheduleIndex(datesArray, daysToLookBack);
        
        _renderScheduleCarousel(carouselContainer, ownerStartDate, ownerEndDate, datesArray, initialScrollIndex, true, filteredClasses);
        updateCopyUI();
    }
    ...
    function renderMemberSchedulePage(container) {
        ...
        // Filter Logic (Client-Side Safety Check)
        let filteredClasses = appState.classes;
        
        // Filter by Branch (even though listener is optimized, this handles legacy/edge cases)
        if (hasMultipleBranches && appState.selectedScheduleBranch) {
            filteredClasses = appState.classes.filter(c => {
                const cBranch = c.branchId || branches[0].id;
                return cBranch === appState.selectedScheduleBranch;
    ...
    /**
     * Generates the JSON string for the Studio QR Code.
     */
    const generateStudioQrData = (branchId, isDynamic = false) => {
        const data = {
            type: isDynamic ? 'studio_dynamic' : 'studio_static',
            branchId: branchId,
            branchName: 'Main Studio' 
        };
        if (isDynamic) data.ts = Date.now();
        return JSON.stringify(data);
    };

    function openBranchQrModal(branchId) {
        // Attempt to find the branch object in the state
        const branch = appState.branches.find(b => b.id === branchId);
        
        // Logic:
        // 1. If branch exists: Use localized name (name_zh for TW, name for EN).
        // 2. If branch is NOT found (Legacy/Default): Use the translated system label for "Main Studio".
        let displayName;
        
        if (branch) {
            displayName = (appState.currentLanguage === 'zh-TW' && branch.name_zh) ? branch.name_zh : branch.name;
        } else {
            // This pulls '總店' (ZH) or 'Main Studio' (EN) from your i18n file
            displayName = _('label_legacy_branch');
        }

        // Handle legacy ID fallback for QR generation
        const qrId = (branch && branch.id) ? branch.id : 'legacy';
        const qrData = generateStudioQrData(qrId, false);
        
        const modal = DOMElements.sportTypeModal;
        
        modal.innerHTML = `
            <div class="bg-white p-8 rounded-2xl shadow-2xl w-full max-w-md transform transition-all duration-300 scale-95 opacity-0 modal-content relative print:shadow-none print:w-full">
                <button class="modal-close-btn print:hidden"><svg class="h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" /></svg></button>
                <div class="text-center print:text-center">
                    <h2 class="text-2xl font-bold text-slate-800 mb-2">${displayName}</h2>
                    <p class="text-slate-500 mb-6 print:mb-4">${_('label_static_qr_instruction')}</p>
                    <div id="staticQrDisplay" class="mx-auto mb-6 border-4 border-slate-800 rounded-xl p-2 inline-block"></div>
                    <button id="printQrBtn" class="bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2 px-6 rounded-lg transition print:hidden">
                        ${_('btn_print_qr')}
                    </button>
                </div>
            </div>`;
        ...
        setTimeout(() => {
            const container = modal.querySelector('#staticQrDisplay');
            if (container) {
                container.innerHTML = '';
        ...
        modal.querySelector('#printQrBtn').onclick = () => window.print();
        openModal(modal);
    }

    function handleMemberSelfCheckIn(decodedText, html5QrCodeInstance) {
        const resultEl = document.getElementById("checkInResult");
        const member = appState.currentUser;
        
        // 1. Visual Feedback
        if (resultEl) {
            resultEl.innerHTML = `<div class="p-3 bg-indigo-50 text-indigo-700 rounded-lg font-bold text-center animate-pulse">Processing...</div>`;
        }
        ...
        try {
            ...
            if (resultEl) resultEl.innerHTML = `<div class="check-in-result-banner check-in-error">${_('error_qr_invalid_format')}</div>`;
            return;
        }

        // 3. Validate Security
        if (qrData.type !== 'studio_static' && qrData.type !== 'studio_dynamic') {
            if (resultEl) resultEl.innerHTML = `<div class="check-in-result-banner check-in-error">${_('error_qr_invalid_format')}</div>`;
            return;
        }

        if (qrData.type === 'studio_dynamic') {
            const now = Date.now();
            const qrTime = qrData.ts || 0;
            // 60 seconds validity
            if (now - qrTime > 60000) { 
                if (resultEl) resultEl.innerHTML = `<div class="check-in-result-banner check-in-error">${_('error_qr_dynamic_expired')}</div>`;
                return;
            }
        }

        // 4. Find Bookings (Using appState.classes directly)
        const todayIso = getIsoDate(new Date());
        
        // Find ALL bookings for today for this member
        const bookingsToday = appState.classes.filter(cls => 
            cls.date === todayIso && 
            cls.bookedBy && cls.bookedBy[member.id] &&
            !(cls.attendedBy && cls.attendedBy[member.id])
        );
        
        console.log("Bookings found for today:", bookingsToday.length);

        // 5. Filter by Branch
        const validBookings = bookingsToday.filter(cls => {
            if (!qrData.branchId) return true; // Legacy QR (Main Studio) matches all
            
            // Resolving Class Branch: Use specific ID, or fallback to first branch
            const clsBranchId = cls.branchId || (appState.branches[0] ? appState.branches[0].id : 'legacy');
            return clsBranchId === qrData.branchId;
        });

        // 6. Decision Logic
        if (bookingsToday.length > 0 && validBookings.length === 0) {
            // User is at the wrong physical location
            if (resultEl) resultEl.innerHTML = `<div class="check-in-result-banner check-in-error">${_('error_qr_wrong_branch')}</div>`;
            // Pause scanner to let them read the error
            if (html5QrCodeInstance) try { html5QrCodeInstance.pause(); } catch(e){}
            return;
        }

        if (validBookings.length === 0) {
            // No valid booking found
            if (resultEl) resultEl.innerHTML = `<div class="check-in-result-banner check-in-notice">${_('error_no_booking_redirect')}</div>`;
        ...
        // Valid Booking Found -> Stop/Pause Scanner
        if (html5QrCodeInstance) try { html5QrCodeInstance.pause(); } catch(e){}

        if (validBookings.length === 1) {
            performCheckIn(validBookings[0].id, member.id, html5QrCodeInstance);
        } else {
            showSelfCheckInChoiceModal(validBookings, member.id, html5QrCodeInstance);
        }
    }

    async function performCheckIn(clsId, memberId, scannerInstance) {
        const resultEl = document.getElementById("checkInResult");
        const cls = appState.classes.find(c => c.id === clsId);
        const sportType = appState.sportTypes.find(st => st.id === cls.sportTypeId);

        try {
            await database.ref(`/classes/${clsId}/attendedBy/${memberId}`).set(true);
            
            playSuccessSound();
            if (navigator.vibrate) navigator.vibrate([100, 50, 100]); // Success pattern

            if (resultEl) {
                resultEl.innerHTML = `<div class="check-in-result-banner check-in-success">${_('success_check_in_complete')} <br> ${getSportTypeName(sportType)}</div>`;
            }

            // Cleanup & Resume (Optional, usually we stop after success)
            setTimeout(() => {
                if (scannerInstance) scannerInstance.stop().then(() => scannerInstance.clear()).catch(console.error);
            ...
            console.error(e);
            if (resultEl) resultEl.innerHTML = `<div class="check-in-result-banner check-in-error">${_('error_generic')}</div>`;
        }
    }

    function showSelfCheckInChoiceModal(classes, memberId, scannerInstance) {
        // Reuse the result container to show buttons
        const resultEl = document.getElementById("checkInResult");
        if (!resultEl) return;

        const buttonsHtml = classes.map(cls => {
            const sportType = appState.sportTypes.find(st => st.id === cls.sportTypeId);
            return `<button class="w-full text-left p-3 bg-indigo-100 hover:bg-indigo-200 rounded-lg transition mb-2 border border-indigo-200 self-checkin-select-btn" data-cls-id="${cls.id}">
                <div class="font-bold text-indigo-900">${getSportTypeName(sportType)}</div>
                <div class="text-sm text-indigo-700">${getTimeRange(cls.time, cls.duration)}</div>
            </button>`;
        }).join('');

        resultEl.innerHTML = `
            <div class="p-4 bg-white border border-slate-200 rounded-xl shadow-lg mt-[-50px] relative z-20">
                <h4 class="font-bold text-center mb-3 text-slate-800">${_('label_select_class_checkin')}</h4>
                ${buttonsHtml}
                <button id="cancelSelfCheckIn" class="w-full mt-2 text-slate-500 text-sm py-2 hover:text-slate-700">${_('btn_cancel')}</button>
            </div>
        `;

        resultEl.querySelectorAll('.self-checkin-select-btn').forEach(btn => {
            btn.onclick = () => performCheckIn(btn.dataset.clsId, memberId, scannerInstance);
        });

        document.getElementById('cancelSelfCheckIn').onclick = () => {
    ...
    function renderCheckInPage(container) {
        ...
        if (isOwner) {
            tabs = [
                { id: 'tabScanMember', label: _('tab_scan_member'), active: true },
                { id: 'tabStationMode', label: _('tab_station_mode'), active: false } // New Combined Tab
            ];
        } else {
            tabs = [
                { id: 'tabScanStudio', label: _('tab_scan_studio'), active: true }, // Default for Members
                { id: 'tabMyCode', label: _('tab_my_code'), active: false }
            ];
        }

        // --- 2. Render Container & Tabs ---
        const tabsHtml = `
            <div class="flex border-b border-slate-200 mb-6">
                ${tabs.map(t => `
                    <button class="checkin-tab-btn flex-1 pb-3 text-sm sm:text-base font-bold transition-colors ${t.active ? 'text-indigo-600 border-b-2 border-indigo-600' : 'text-slate-500 hover:text-indigo-600 border-b-2 border-transparent'}" data-tab-id="${t.id}">
                        ${t.label}
                    </button>
                `).join('')}
            </div>`;
        ...
        container.innerHTML = `
            <div class="w-full max-w-md mx-auto">
                <div class="card p-6 min-h-[450px]">
                    <h4 data-lang-key="title_qr_code" class="text-xl font-bold text-slate-800 mb-4 text-center"></h4>
                    ${tabsHtml}
                    <div id="checkInContentArea" class="flex flex-col items-center"></div>
                </div>
            </div>`;
        ...
        const contentArea = container.querySelector('#checkInContentArea');
        ...
        // --- View A: Scanner (Used by Owner to scan Member, or Member to scan Studio) ---
        const startScannerMode = (isSelfCheckIn = false) => {
            cleanup();
            const promptKey = isSelfCheckIn ? 'tab_scan_studio' : 'check_in_prompt';
            
            contentArea.innerHTML = `
                <p class="text-slate-500 mb-4 text-center">${_(promptKey)}</p>
                <div id="qr-reader" class="bg-slate-900 rounded-lg overflow-hidden w-64 h-64 border-4 border-slate-200 shadow-inner"></div>
                <div id="checkInResult" class="w-full mt-4 min-h-[3rem]"></div>
            `;
            ...
            const onScanSuccess = (decodedText) => {
                if (isSelfCheckIn) {
                    handleMemberSelfCheckIn(decodedText, currentScanner);
                } else {
                    if (currentScanner) currentScanner.pause();
                    handleCheckIn(decodedText); // Standard Owner -> Member scan
                }
            };

            currentScanner.start({ facingMode: "environment" }, config, onScanSuccess, () => {})
                .catch(err => {
                    console.error("Camera error", err);
                    contentArea.innerHTML += `<p class="text-red-500 text-center mt-2 text-sm">${_('check_in_error_camera_start_failed')}</p>`;
                });
        };

        // --- View B: Station Mode (Owner Only - Dynamic + Print) ---
        const showStationMode = () => {
            ...
            const branches = appState.branches || [];
            const hasMultipleBranches = branches.length > 1;
            let currentBranchId = appState.currentUser.homeBranchId || (branches[0] ? branches[0].id : 'legacy');
            ...
            if (hasMultipleBranches) {
                const options = branches.map(b => {
                    const bName = (appState.currentLanguage === 'zh-TW' && b.name_zh) ? b.name_zh : b.name;
                    return `<option value="${b.id}" ${b.id === currentBranchId ? 'selected' : ''}>${bName}</option>`;
                }).join('');
                
                branchSelectHtml = `
                    <div class="w-full mb-4">
                        <label class="block text-xs font-bold text-slate-500 mb-1 text-center">${_('label_station_branch_select')}</label>
                        <select id="stationBranchSelect" class="form-select text-sm py-1">${options}</select>
                    </div>`;
            }
            ...
            contentArea.innerHTML = `
                ...
                <!-- Dynamic QR Section -->
                <div class="bg-indigo-50 p-4 rounded-xl w-full flex flex-col items-center border border-indigo-100">
                    <p class="text-indigo-900 font-bold mb-2 text-sm">${_('tab_show_dynamic_qr')}</p>
                    <div id="dynamicQrDisplay" class="bg-white p-2 rounded-lg shadow-sm mb-2">
                        <div id="innerQr"></div>
                    </div>
                    <div class="w-full h-1 bg-slate-200 rounded-full overflow-hidden mt-1 max-w-[150px]">
                        <div id="qrProgressBar" class="h-full bg-indigo-500 transition-all duration-1000 ease-linear" style="width: 100%"></div>
                    </div>
                    <p class="text-[10px] text-slate-400 mt-1">${_('label_dynamic_qr_instruction')}</p>
                </div>

                <!-- Divider -->
                <div class="relative w-full my-6">
                    <div class="absolute inset-0 flex items-center"><div class="w-full border-t border-slate-200"></div></div>
                    <div class="relative flex justify-center"><span class="bg-white px-2 text-xs text-slate-400 font-bold uppercase">OR</span></div>
                </div>

                <!-- Print Static Button -->
                <button id="triggerPrintStaticBtn" class="flex items-center gap-2 bg-white border-2 border-slate-200 hover:border-slate-800 text-slate-700 hover:text-slate-900 font-bold py-3 px-6 rounded-xl transition-all shadow-sm w-full justify-center group">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-slate-400 group-hover:text-slate-800 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            ...
            // Function to render Dynamic QR
            const renderDynamicQR = () => {
                const inner = document.getElementById('innerQr');
                if (!inner) return;
                inner.innerHTML = '';
                const data = generateStudioQrData(currentBranchId, true); // Dynamic = true
                new QRCode(inner, {
                    text: data,
                    width: 180,
                    height: 180,
            ...
            const selectEl = document.getElementById('stationBranchSelect');
            if (selectEl) {
                selectEl.onchange = () => {
                    currentBranchId = selectEl.value;
                    timeLeft = 0; // Trigger immediate refresh
                };
            }

            // Handle Print Click
            document.getElementById('triggerPrintStaticBtn').onclick = () => {
                openBranchQrModal(currentBranchId);
            };
        };

        // --- View C: My Code (Member) ---
        const showStaticMemberCode = () => {
            cleanup();
            contentArea.innerHTML = `
                <div id="qrCodeContainer" class="w-64 h-64 mx-auto mb-4 border-4 border-white shadow-sm rounded-xl overflow-hidden bg-white"></div>
                <div id="upcomingCheckInSection" class="w-full mt-2 space-y-2"></div>
            `;
            
            const qrContainer = contentArea.querySelector('#qrCodeContainer');
            ...
            renderUpcomingClassesList(contentArea.querySelector('#upcomingCheckInSection'));
        };
        ...
        container.querySelectorAll('.checkin-tab-btn').forEach(btn => {
            btn.onclick = () => {
                ...
                if (tabId === 'tabScanMember') startScannerMode(false);
                else if (tabId === 'tabStationMode') showStationMode();
                else if (tabId === 'tabScanStudio') startScannerMode(true);
                else if (tabId === 'tabMyCode') showStaticMemberCode();
            };
        });

        // --- 5. Initial State Load ---
        if (isOwner) {
            // Owner defaults to "Scan Member"
            startScannerMode(false);
        } else {
            // Member defaults to "Scan Studio" (Self Check-in) per request
            startScannerMode(true);
        }
        
        updateUIText();
    }

    // --- Helper: Render Upcoming List (Extracted from old renderCheckInPage) ---
    function renderUpcomingClassesList(wrapper) {
        const member = appState.currentUser;
        ...
        if (todaysUnattendedBookings.length === 0) {
            wrapper.innerHTML = `<p class="text-center text-slate-500 mt-4">${_('info_no_booked_class_today')}</p>`;
            return;
        }

        wrapper.innerHTML = `<h5 class="text-center font-semibold text-slate-600 text-xs uppercase tracking-wide mb-2">${_('header_upcoming_checkins')}</h5>` + 
        todaysUnattendedBookings
            .sort((a,b) => a.time.localeCompare(b.time))
            .map(cls => {
                const sportType = appState.sportTypes.find(st => st.id === cls.sportTypeId);
                return `
                <div class="w-full p-3 bg-slate-50 rounded-lg flex items-center justify-between border border-slate-100">
                    <div>
                        <div class="font-bold text-slate-700 text-sm">${getSportTypeName(sportType)}</div>
                        <div class="text-xs text-slate-500">${getTimeRange(cls.time, cls.duration)}</div>
                    </div>
                    <span class="text-xs bg-slate-200 text-slate-600 px-2 py-1 rounded">Pending</span>
                </div>`;
            }).join('');
    }
    ...
    function renderAccountPage(container) {
        ...
        // 2. Prepare History Data (Pre-calculate for Counts in Tabs)
        ...
        // 3. Identify Credit Types for Filter
        ...
        // 4. Render Bookings
        const renderBookingList = (filterId) => {
            ...
            listContainer.innerHTML = paginatedBookings.map(cls => {
                ...
                return `<div class="${isHighlighted ? 'booking-highlight' : 'bg-slate-100'} p-4 rounded-lg flex justify-between items-center" data-cls-id="${cls.id}">
                    <div>
                        ...
                        <div class="text-xs text-slate-600 mt-1 flex items-center">${_('label_credits_used')} ${creditsUsed} ${typePill}</div>
            ...
            renderPaginationControls(paginationContainer, page, totalPages, filteredBookings.length, itemsPerPage, (newPage) => {
                appState.pagination.accountBookings.page = newPage; renderBookingList(currentFilterId);
            });
            listContainer.querySelectorAll('.cancel-booking-btn-dash').forEach(btn => {
                btn.onclick = () => { const cls = appState.classes.find(c => c.id === btn.dataset.clsId); handleCancelBooking(cls); };
            });
        };
        ...
        const renderFilterPills = () => {
            ...
            let pillsHTML = '';
            const allActive = currentFilterId === 'all';
            const allClass = allActive ? 'bg-slate-800 text-white shadow-md' : 'bg-slate-100 text-slate-600 hover:bg-slate-200';
            pillsHTML += `<button class="filter-pill px-3 py-1 text-xs font-bold rounded-full transition-all ${allClass}" data-type-id="all">${_('filter_all')} ${_('label_used_credits_suffix').replace('{count}', formatCredits(creditUsageMap['all'] || 0))}</button>`;
            if (usedCreditTypeIds.size > 1) {
                usedCreditTypeIds.forEach(typeId => {
                    const typeDef = appState.creditTypes ? appState.creditTypes.find(ct => ct.id === typeId) : null;
                    const typeName = typeDef ? getCreditTypeName(typeDef) : (typeId === 'general' ? _('label_general') : _('label_credits'));
                    const baseColor = typeDef?.color || '#64748b';
                    const isActive = currentFilterId === typeId;
                    ...
                    let className = 'filter-pill px-3 py-1 text-xs font-bold rounded-full transition-all ml-2 ' + (isActive ? '' : 'hover:opacity-80');
                    pillsHTML += `<button class="${className}" style="${style}" data-type-id="${typeId}">${typeName} ${usageText}</button>`;
                });
            }
            pillsContainer.innerHTML = pillsHTML;
            pillsContainer.querySelectorAll('.filter-pill').forEach(btn => { btn.onclick = () => { currentFilterId = btn.dataset.typeId; appState.pagination.accountBookings.page = 1; renderFilterPills(); renderBookingList(currentFilterId); }; });
        };

        // 5. Combined Profile Content
        const getProfileContent = () => {
            ...
            // --- NEW: Branch Display Logic ---
            let branchHtml = '';
            const branches = appState.branches || [];
            
            if (branches.length > 1) {
                // Resolve branch (fallback to first/main if null/legacy)
                const branchId = member.homeBranchId || branches[0].id;
                const branch = branches.find(b => b.id === branchId);
            ...
            // Section A: Monthly Plan (If Active)
            if (member.monthlyPlan) {
                const tierId = member.monthlyPlanTierId;
                const tier = appState.monthlyPlanTiers ? appState.monthlyPlanTiers.find(t => t.id === tierId) : null;
                ...
                const planPill = `<span class="text-sm font-bold px-2.5 py-1 rounded-full" style="${pillStyle}">${formatCurrency(member.monthlyPlanAmount)}${_('label_mo')}${tierLabel}</span>`;
                ...
                <div class="md:col-span-2">
                    <div class="card p-6">
                        <div class="flex flex-wrap justify-between items-center mb-4 gap-2">
                            <h3 class="text-xl font-bold text-slate-800 whitespace-nowrap">${_('header_my_bookings')} <span id="accountBookingCount"></span></h3>
                            <div id="bookingFilterPills" class="flex flex-wrap gap-2 justify-end"></div>
                        </div>
        ...
        // Render History Lists (Updated to use pre-calculated lists)
        ...
        renderBookingList(currentFilterId);
        renderFilterPills();
    ...
    function renderMembersPage(container) {
        const memberCount = appState.users.filter(u => u.role !== 'member' && u.role !== 'staff' && !u.isDeleted).length;
        const isOwner = appState.currentUser?.role === 'owner';
        const branches = appState.branches || [];
        const hasMultipleBranches = branches.length > 1;
        ...
        const updateTable = (searchTerm = '') => {
            ...
            const filteredUsers = appState.users.filter(u => {
                // Base check
                if (u.role === 'owner' || u.role === 'staff' || u.isDeleted) return false;
                ...
                if (activeWalletEntries.length > 0) {
                    activeWalletEntries.forEach(([typeId, data]) => {
                        ...
                        if (data.expiryDate) {
                            ...
                            dateStyle = isExpired ? 'text-red-600 font-bold' : 'text-slate-900';
                        } else if (rawDate === '') {
                            dateText = _('label_na'); // Show N/A if no date set but allow edit
                        }

                        // --- ADDED: Quick Edit Button Logic for Wallet ---
                        ...
                        displayRows.push({
                            planHtml: `<span class="font-bold text-xs px-2.5 py-1 rounded-full inline-block" style="background-color: ${color}26; color: ${color};" title="${name}">${formatCredits(data.balance)} / ${formatCredits(initial)} ${name}</span>`,
                            expiryHtml: dateHtml
                        });
                    });
    ...
    function _renderMemberPurchaseHistory(member, container, historyIdInput, purchaseAmountInput, creditsInput, onEditStart, creditTypeSelect, expiryDateInput) {
        ...
        if (purchaseHistory.length > 0) {
            ...
            const fullText = `${_('header_purchase_history')} - ${countsText}`;
    ...
    function _renderMemberPaymentHistory(member, container, historyIdInput, monthsPaidInput, paymentAmountInput, onEditStart, monthlyTierSelect) {
        ...
        if (paymentHistory.length > 0) {
            ...
            const fullText = `${_('header_payment_history')} - ${countsText}`;
    ...
    function openMemberModal(memberToEdit) {
        ...
        creditActionBtn.onclick = async () => {
             ...
             try {
                 ...
                 if(historyId) {
                     ...
                     updates[`/users/${memberId}/purchaseHistory/${historyId}`] = { ...originalEntry, ...purchaseData }; 
                     ...
                     updates[`/users/${memberId}/purchaseHistory/${newRef.key}`] = purchaseData;
        ...
        if (walletKeys.length > 0) {
            ...
            if (creditTypeSelect.querySelector(`option[value="${defaultTypeId}"]`)) { creditTypeSelect.value = defaultTypeId; }
            expiryDateInput.value = wallet[defaultTypeId].expiryDate || '';
        } else { expiryDateInput.value = memberToEdit.expiryDate || ''; }
        updateExpiryUI();

        creditTypeSelect.onchange = () => {
            const typeId = creditTypeSelect.value;
            if (wallet[typeId] && wallet[typeId].expiryDate) { expiryDateInput.value = wallet[typeId].expiryDate; updateExpiryUI(); }
        };
    ...
    async function handleMemberFormSubmit(e, originalMember) {
        ...
        updates[`/users/${id}/notes`] = form.querySelector('#adminNotes').value.trim();
    ...
    function renderUpcomingClassesList(wrapper) {
        ...
        // 2. Debugging Output (Visible in UI for testing)
        // You can remove this specific line later, but it helps confirm data is present
        const debugInfo = `<p class="text-xs text-slate-300 text-center mb-2" style="display:none">Debug: Found ${todaysUnattendedBookings.length} bookings for ${today}</p>`;

        if (todaysUnattendedBookings.length === 0) {
            wrapper.innerHTML = `${debugInfo}<p class="text-center text-slate-500 mt-4 bg-slate-50 p-2 rounded">${_('info_no_booked_class_today')}</p>`;
            return;
        }

        // 3. Render List
        const listHtml = todaysUnattendedBookings
            .sort((a,b) => a.time.localeCompare(b.time))
            .map(cls => {
                const sportType = appState.sportTypes.find(st => st.id === cls.sportTypeId);
                ...
                // --- Branch Badge Logic ---
                const branches = appState.branches || [];
                const clsBranchId = cls.branchId || (branches[0] ? branches[0].id : null);
                const clsBranch = branches.find(b => b.id === clsBranchId);
                const branchName = clsBranch ? ((appState.currentLanguage === 'zh-TW' && clsBranch.name_zh) ? clsBranch.name_zh : clsBranch.name) : '';
                const branchBadge = branchName ? `<span class="text-[10px] bg-indigo-50 text-indigo-600 px-1.5 py-0.5 rounded border border-indigo-100 ml-2">${branchName}</span>` : '';

                return `
                <div class="w-full p-3 bg-white hover:bg-slate-50 rounded-lg flex items-center justify-between border border-slate-200 shadow-sm">
                    <div>
                        <div class="font-bold text-slate-700 text-sm flex items-center">
                            ${getSportTypeName(sportType)}
                            ${branchBadge}
                        </div>
                        <div class="text-xs text-slate-500 mt-0.5">${getTimeRange(cls.time, cls.duration)}</div>
                    </div>
                    <span class="text-xs bg-yellow-100 text-yellow-700 px-2 py-1 rounded font-bold">Pending</span>
                </div>`;
            }).join('');

        wrapper.innerHTML = `${debugInfo}<div class="space-y-2 w-full">${listHtml}</div>`;
    }
    ...
    function openCheckInModal() {
        ...
        if (isOwner) {
            tabs = [
                { id: 'tabStationMode', label: _('tab_station_mode'), active: true },
                { id: 'tabScanMember', label: _('tab_scan_member'), active: false }
            ];
        } else {
            // Member View
            tabs = [
                { id: 'tabScanStudio', label: _('tab_scan_studio'), active: true },
                { id: 'tabMyCode', label: _('tab_my_code'), active: false }
            ];
        }

        // --- 2. Build Modal HTML ---
        // Added max-h-[85vh] and overflow-y-auto to the main content wrapper to ensure scrolling on small phones
        modal.innerHTML = `
            <div class="bg-white p-4 sm:p-6 rounded-2xl shadow-2xl w-full max-w-lg transform transition-all duration-300 scale-95 opacity-0 modal-content relative max-h-[90vh] flex flex-col">
                <button class="modal-close-btn absolute top-4 right-4"><svg class="h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" /></svg></button>
                
                <h2 class="text-xl sm:text-2xl font-bold text-slate-800 mb-4 text-center mt-2">${_('check_in_title')}</h2>
                
                <!-- Tab Navigation -->
                <div class="flex border-b border-slate-200 mb-4 shrink-0">
                    ${tabs.map(t => `
                        <button class="modal-checkin-tab flex-1 pb-3 text-sm font-bold transition-colors ${t.active ? 'text-indigo-600 border-b-2 border-indigo-600' : 'text-slate-500 hover:text-indigo-600 border-b-2 border-transparent'}" data-tab-id="${t.id}">
                            ${t.label}
                        </button>
                    `).join('')}
                </div>
                ...
                <div id="modalCheckInContent" class="flex flex-col items-center w-full overflow-y-auto" style="min-height: 400px;"></div>
            </div>
        `;
        ...
        // --- 3. Logic & State ---
        const contentArea = modal.querySelector('#modalCheckInContent');
        let dynamicQrInterval = null;

        const branches = appState.branches || [];
        const hasMultipleBranches = branches.length > 1;
        let currentStationBranchId = appState.currentUser.homeBranchId || (branches[0] ? branches[0].id : 'legacy');
        ...
        // --- View A: Scanner ---
        const renderScanner = (isSelfCheckIn) => {
            cleanup();
            const promptKey = isSelfCheckIn ? 'tab_scan_studio' : 'check_in_prompt';
            
            // Device Location Selector (Owner Only)
            let deviceLocationHtml = '';
            if (!isSelfCheckIn && hasMultipleBranches) {
                const savedDeviceBranch = localStorage.getItem('studioPulseDeviceBranch') || 'any';
                let options = `<option value="any" ${savedDeviceBranch === 'any' ? 'selected' : ''}>${_('option_device_location_any') || 'Any Location'}</option>`;
                options += branches.map(b => {
                    const bName = (appState.currentLanguage === 'zh-TW' && b.name_zh) ? b.name_zh : b.name;
                    const isSelected = savedDeviceBranch === b.id ? 'selected' : '';
                    return `<option value="${b.id}" ${isSelected}>${bName}</option>`;
                }).join('');

                deviceLocationHtml = `
                    <div class="mb-4 flex justify-center items-center gap-2 bg-slate-50 p-2 rounded-lg border border-slate-200 w-full shrink-0">
                        ...
                        <label class="text-sm font-bold text-slate-600 whitespace-nowrap">${_('label_device_location') || 'Location'}:</label>
                        <select id="modalDeviceBranchSelect" class="form-select py-1 pl-2 pr-8 text-sm font-semibold w-auto bg-transparent border-none focus:ring-0 cursor-pointer text-indigo-600">
                            ${options}
                        </select>
                    </div>`;
            }
            ...
            contentArea.innerHTML = `
                ${deviceLocationHtml}
                <div class="shrink-0 mb-4 text-center w-full">
                     <p class="text-slate-500">${_(promptKey)}</p>
                </div>
                
                <!-- Scanner -->
                <div id="qr-reader" class="bg-slate-900 rounded-lg overflow-hidden w-full max-w-[280px] aspect-square border-4 border-slate-200 shadow-inner shrink-0"></div>
                
                <!-- Result Message -->
                <div id="checkInResult" class="w-full mt-4 min-h-[3rem] shrink-0"></div>

                <!-- Upcoming Classes (Scrollable Area below scanner) -->
                ${isSelfCheckIn ? `
                    <div class="w-full mt-2 pt-4 border-t border-slate-100 flex-grow">
                        <p class="text-xs font-bold text-slate-400 uppercase mb-2 text-center">${_('header_upcoming_checkins')}</p>
                        <div id="scannerUpcomingList" class="w-full space-y-2 pb-4"></div>
                    </div>
                ` : ''}
            `;

            // RENDER LIST: Explicit call
            if (isSelfCheckIn) {
                renderUpcomingClassesList(contentArea.querySelector('#scannerUpcomingList'));
            }

            // Bind Selector
            const branchSelect = contentArea.querySelector('#modalDeviceBranchSelect');
            if (branchSelect) {
                branchSelect.onchange = () => {
                    localStorage.setItem('studioPulseDeviceBranch', branchSelect.value);
                    branchSelect.classList.add('text-green-600');
                    setTimeout(() => branchSelect.classList.remove('text-green-600'), 500);
                };
            }
            ...
            setTimeout(() => {
                ...
                const onScanSuccess = (decodedText) => {
                    if (isSelfCheckIn) {
                        handleMemberSelfCheckIn(decodedText, html5QrCode);
                    } else {
                ...
                html5QrCode.start({ facingMode: "environment" }, config, onScanSuccess, () => {})
                    .catch(err => {
                        console.error("Camera error", err);
                        const res = contentArea.querySelector('#checkInResult');
                        if(res) res.innerHTML = `<p class="text-red-500 text-center text-sm">${_('check_in_error_camera_start_failed')}</p>`;
                    });
            }, 100);
        };

        // --- View B: Station Mode (Owner) ---
        const renderStationMode = () => {
            cleanup(); 

            let branchSelectHtml = '';
            if (hasMultipleBranches) {
                const buttonsHtml = branches.map(b => {
                    const isActive = b.id === currentStationBranchId;
                    const bName = (appState.currentLanguage === 'zh-TW' && b.name_zh) ? b.name_zh : b.name;
                    const activeStyle = isActive ? `background-color: #ffffff; color: ${b.color || '#6366f1'}; box-shadow: 0 1px 2px 0 rgb(0 0 0 / 0.05);` : '';
                    const baseClass = 'station-branch-btn flex-1 py-1.5 px-2 text-xs font-bold rounded-md transition-all whitespace-nowrap overflow-hidden text-ellipsis';
                    const stateClass = isActive ? '' : 'text-slate-500 hover:text-slate-700 hover:bg-slate-200/50';
                    return `<button class="${baseClass} ${stateClass}" style="${activeStyle}" data-branch-id="${b.id}">${bName}</button>`;
                }).join('');
                ...
                branchSelectHtml = `
                    <div class="w-full mb-4 shrink-0">
                        <label class="block text-xs font-bold text-slate-500 mb-1 text-center">${_('label_station_branch_select')}</label>
                        <div class="bg-slate-100 p-1 rounded-lg flex w-full overflow-x-auto">
                            ${buttonsHtml}
                        </div>
                    </div>`;
            }
            ...
            contentArea.innerHTML = `
                ${branchSelectHtml}
                <div class="bg-indigo-50 p-6 rounded-xl w-full flex flex-col items-center border border-indigo-100 shadow-sm">
                    <div id="dynamicQrDisplay" class="bg-white p-2 rounded-lg shadow-sm mb-3">
                        <div id="innerQr"></div>
                    </div>
                    <div class="w-full h-1.5 bg-slate-200 rounded-full overflow-hidden mt-1 max-w-[180px]">
                        <div id="qrProgressBar" class="h-full bg-indigo-500 transition-all duration-1000 ease-linear" style="width: 100%"></div>
                    </div>
                    <p class="text-xs text-indigo-800 mt-2 font-medium">${_('label_dynamic_qr_instruction')}</p>
                </div>
                <button id="modalPrintStaticBtn" class="mt-6 flex items-center justify-center gap-2 bg-white border border-slate-300 hover:border-indigo-500 text-slate-600 hover:text-indigo-600 font-bold py-3 px-6 rounded-xl transition-all shadow-sm w-full group shrink-0">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-slate-400 group-hover:text-indigo-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z" />
                    </svg>
                    ${_('btn_print_static_qr')}
                </button>
            ...
            const renderQR = () => {
                ...
                if (!inner) return;
                inner.innerHTML = '';
                const data = generateStudioQrData(currentStationBranchId, true); 
                new QRCode(inner, {
                    text: data,
                    width: 200,
                    height: 200,
            ...
            dynamicQrInterval = setInterval(() => {
                timeLeft--;
                if (progressBar) progressBar.style.width = `${(timeLeft / 15) * 100}%`;
                if (timeLeft <= 0) { renderQR(); timeLeft = 15; }
            }, 1000);

            if (hasMultipleBranches) {
                contentArea.querySelectorAll('.station-branch-btn').forEach(btn => {
                    btn.onclick = () => { currentStationBranchId = btn.dataset.branchId; renderStationMode(); };
                });
            }
            document.getElementById('modalPrintStaticBtn').onclick = () => {
                cleanup(); closeModal(modal); setTimeout(() => openBranchQrModal(currentStationBranchId), 300);
            };
        };
        ...
        const renderMyCode = () => {
            cleanup();
            contentArea.innerHTML = `
                <div id="qrCodeContainer" class="w-64 h-64 mx-auto mb-4 border-4 border-white shadow-sm rounded-xl overflow-hidden bg-white shrink-0"></div>
                <p class="text-center text-slate-500 text-sm shrink-0 mb-4">${user.name}</p>
                
                <div class="w-full mt-2 pt-4 border-t border-slate-100 flex-grow">
                    <p class="text-xs font-bold text-slate-400 uppercase mb-2 text-center">${_('header_upcoming_checkins')}</p>
                    <div id="myCodeUpcomingList" class="w-full space-y-2 pb-4"></div>
                </div>
            `;
            const qrContainer = contentArea.querySelector('#qrCodeContainer');
            new QRCode(qrContainer, {
            ...
            renderUpcomingClassesList(contentArea.querySelector('#myCodeUpcomingList'));
        };

        // --- 4. Tab Switching Logic ---
        modal.querySelectorAll('.modal-checkin-tab').forEach(btn => {
            btn.onclick = () => {
                modal.querySelectorAll('.modal-checkin-tab').forEach(b => {
                    b.classList.remove('text-indigo-600', 'border-b-2', 'border-indigo-600');
                    b.classList.add('text-slate-500', 'border-transparent');
                });
                btn.classList.remove('text-slate-500', 'border-transparent');
                ...
                const tabId = btn.dataset.tabId;
                if (tabId === 'tabScanMember') renderScanner(false);
                else if (tabId === 'tabStationMode') renderStationMode();
                else if (tabId === 'tabScanStudio') renderScanner(true);
                else if (tabId === 'tabMyCode') renderMyCode();
            };
        });
    ...
    function calculateRevenueForBookings(bookings, revenueMode = 'actual') {
        // Group bookings by member to minimize user lookups
        ...
        // Optimization: Create the comparison Date object once, outside the loop
        const now = new Date();
        ...
        for (const memberId in bookingsByMember) {
            ...
            // Optimization: Sort strings directly (ISO format allows this) to avoid creating Date objects
            ...
            // Optimization: Only process history if it exists
            if (member.purchaseHistory) {
                // Convert to array manually or via helper, then sort by string ISO date
                const historyArr = firebaseObjectToArray(member.purchaseHistory);
                
                // Filter and Sort in one pass logic (String comparison is faster than Date parsing)
                historyArr
                    .filter(p => p.status !== 'deleted')
                    .sort((a, b) => (a.date || '').localeCompare(b.date || '')) // Oldest first for calculation
                    .forEach(p => {
                        const typeId = p.creditTypeId || 'general';
            ...
            for (const cls of memberClasses) {
                // Optimization: String comparison for "Has Class Started" check
                // This assumes cls.date is YYYY-MM-DD and cls.time is HH:MM
                // We construct a comparable ISO-like string prefix
                ...
                if (shouldCalculate) {
                    ...
                    // Logic: Determine Payment Method
                    ...
                    if (isPaidByMonthly) {
                        ...
                        const creditValueForThisBooking = (bookingInfo && typeof bookingInfo === 'object' && bookingInfo.monthlyCreditValue)
                            ? bookingInfo.monthlyCreditValue
                            : (member.monthlyCreditValue || 0);
                        revenue = (cls.credits || 0) * creditValueForThisBooking;
                    } else {
                        // Credit Pack Revenue (FIFO)
    ...
    async function renderAdminPage(container) {
        ...
        // --- NEW: Announcement Preview Logic ---
        ...
        if (currentAnnounce) {
            ...
            announcementPreviewHTML = `
                <div class="mb-6 p-4 rounded-lg text-white shadow-md border border-white/10 relative overflow-hidden" style="background-color: ${color}">
                    <!-- Radiant Effect Overlay (Optional, matches classes) -->
        ...
        // ---------------------------------------
        const gridClassesTop = isOwner ? 'grid-cols-1 md:grid-cols-2' : 'grid-cols-1';
        const gridClassesBottom = 'grid-cols-1 md:grid-cols-2 lg:grid-cols-3';
        ...
        container.innerHTML = `
            <div class="space-y-8">
                ...
                <!-- BOTTOM ROW: Credit Types, Monthly Tiers, Locations -->
                ${isOwner ? `
                <div class="grid ${gridClassesBottom} gap-8 items-start">
                    <!-- Credit Types -->
                    <div class="card p-6 flex flex-col">
                        <div class="flex flex-wrap gap-4 justify-between items-center mb-4">
                            <!-- CHANGED: Removed classes from span -->
                            <h3 class="text-2xl font-bold text-slate-800">
                                <span data-lang-key="header_credit_types"></span> 
                                <span id="creditTypesCount"></span>
                            </h3>
                        </div>
                        ...
                        <div class="flex-grow">
                                <ul id="creditTypesList" class="admin-list space-y-2"></ul>
                        </div>
                    </div>
                    ...
                    <div class="card p-6 flex flex-col">
                        <div class="flex flex-wrap gap-4 justify-between items-center mb-4">
                            <!-- CHANGED: Removed classes from span -->
                            <h3 class="text-2xl font-bold text-slate-800">
                                <span data-lang-key="header_monthly_tiers"></span>
                                <span id="monthlyTiersCount"></span>
                            </h3>
                        </div>
                        ...
                        <div class="flex-grow">
                                <ul id="monthlyTiersList" class="admin-list space-y-2"></ul>
                        </div>
                    </div>
                    ...
                    <div class="card p-6 flex flex-col">
                        <div class="flex flex-wrap gap-4 justify-between items-center mb-4">
                            <h3 id="locationsHeader" class="text-2xl font-bold text-slate-800"></h3>
                        </div>
                        ...
                        <div class="flex-grow">
                            <ul id="locationsList" class="admin-list space-y-2"></ul>
                        </div>
                    </div>
                </div>
                ` : ''}
                ...
                <div class="card p-6 md:p-8">
                    <div class="flex justify-between items-center mb-4">
                        ...
                        <!-- UPDATED STATUS INDICATOR WITH QUICK CLEAR BUTTON -->
                        <div class="flex items-center gap-2">
                            ${currentAnnounce ? `
                ...
                <!-- NEW: Export Schedule Card -->
                <div class="card p-6 md:p-8">
                    <h3 data-lang-key="header_export_schedule" class="text-2xl font-bold text-slate-800 mb-4">Export Schedule</h3>
                    <p data-lang-key="desc_export_schedule" class="text-slate-500 mb-6">Download schedule data for Excel or social media posts.</p>
                    
                    <div class="space-y-6">
                        <!-- Branch Segmented Control (Container) -->
                        <div id="exportScheduleBranchContainer" class="hidden">
                            <label class="block text-slate-600 text-sm font-bold mb-2" data-lang-key="label_select_branch">Select Branch</label>
                            <div id="exportScheduleBranchWrapper" class="flex bg-slate-100 p-1 rounded-lg overflow-x-auto"></div>
                        </div>

                        <!-- Month Select -->
                        <div>
                            <label class="block text-slate-600 text-sm font-bold mb-2" data-lang-key="label_select_month">Select Month</label>
                            <select id="exportScheduleMonth" class="form-select w-full md:w-64"></select>
                        </div>

                        <button id="btnAdminExportSchedule" class="bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-6 rounded-lg transition flex items-center gap-2">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M3 17a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1zm3.293-7.707a1 1 0 011.414 0L9 10.586V3a1 1 0 112 0v7.586l1.293-1.293a1 1 0 111.414 1.414l-3 3a1 1 0 01-1.414 0l-3-3a1 1 0 010-1.414z" clip-rule="evenodd" /></svg>
                            <span data-lang-key="btn_export">Export</span>
                        </button>
                    </div>
                </div>
            </div>`;
        ...
        container.querySelector('#adminAnnounceBtn').onclick = openAnnouncementModal;
        
        // --- NEW: Quick Clear Button Listener ---
        const quickClearBtn = container.querySelector('#quickClearAnnounceBtn');
        if (quickClearBtn) {
            quickClearBtn.onclick = () => {
                showConfirmation(
                    _('confirm_clear_announcement_title'),
                    _('confirm_clear_announcement_desc'),
                    () => {
                        database.ref('/announcements/current').remove().then(() => {
                            showMessageBox(_('success_announcement_cleared'), 'success');
                            // The page will auto-refresh via the existing data listener
                        });
                    }
                );
            };
        ...
        const getExportDateRange = (type) => {
            ...
            const distToMonday = dayOfWeek === 0 ? 6 : dayOfWeek - 1; // Calculate days to subtract to get Monday
            ...
            if (type === 'this_week') {
                ...
                start = mondayNextWeek;
                end = new Date(mondayNextWeek);
                end.setDate(end.getDate() + 6); // Next Sunday
            } else if (type === 'next_2_weeks') {
                start = mondayNextWeek;
                end = new Date(mondayNextWeek);
                end.setDate(end.getDate() + 13); // Sunday after next
            }
        ...
        // 1. Init Period Dropdown (Months + Weekly Options)
        // Change label text
        const periodLabel = container.querySelector('label[data-lang-key="label_select_month"]');
        if(periodLabel) periodLabel.setAttribute('data-lang-key', 'label_select_period'); // Update i18n key
        ...
        // 2. Init Branch Segmented Control
        const branches = appState.branches || [];
        ...
        if (branches.length > 1) {
            ...
            exportBranchWrapper.addEventListener('click', (e) => {
                ...
                if (btn) {
                    ...
                    renderBranchButtons(); // Re-render to update active state
                }
            });
        }
        ...
        btnExportSchedule.onclick = async () => {
            ...
            try {
                ...
                if (['this_week', 'next_week', 'next_2_weeks'].includes(selectedVal)) {
                    ...
                    filenamePeriod = selectedVal; // e.g. "this_week"
                } else {
                    // It is a Month string (YYYY-MM)
                    start = `${selectedVal}-01`;
                    end = `${selectedVal}-31`;
                    filenamePeriod = selectedVal;
                }
                ...
                // Filter by Branch
                if (exportSelectedBranchId) {
                    classes = classes.filter(c => {
                        const cBranch = c.branchId || branches[0]?.id; 
                        return cBranch === exportSelectedBranchId;
                    });
                }
                ...
                const exportData = classes.map(cls => {
                    const sportType = appState.sportTypes.find(st => st.id === cls.sportTypeId);
                    const tutor = appState.tutors.find(t => t.id === cls.tutorId);
                    const branchObj = branches.find(b => b.id === exportSelectedBranchId);
                    const branchName = branchObj ? ((appState.currentLanguage === 'zh-TW' && branchObj.name_zh) ? branchObj.name_zh : branchObj.name) : '';
                    
                    const dateObj = new Date(cls.date);
                    const dayName = new Intl.DateTimeFormat(getLocale(), { weekday: 'long' }).format(dateObj);
                    ...
                    return {
                        ...
                        [_('export_header_class_name')]: getSportTypeName(sportType),
                        [_('export_header_tutor_name')]: tutor?.name || _('unknown_tutor'),
                        [_('label_branch')]: branchName,
                        [_('label_credits')]: cls.credits,
                        [_('export_header_capacity')]: cls.maxParticipants
                    };
                });

                // --- Resolve Branch Name for Filename ---
                const exportBranchObj = branches.find(b => b.id === exportSelectedBranchId);
                const exportBranchName = exportBranchObj 
                    ? ((appState.currentLanguage === 'zh-TW' && exportBranchObj.name_zh) ? exportBranchObj.name_zh : exportBranchObj.name) 
                    : 'Main_Studio';
                const safeBranchName = exportBranchName.replace(/\s+/g, '_');
                ...
                btnExportSchedule.disabled = false;
                btnExportSchedule.innerHTML = originalBtnText;
            }
        };
    ...
    function renderAdminLists() {
                ...
                locationsList.innerHTML = branches.map((branch, idx) => {
                    ...
                    <li class="flex justify-between items-center bg-slate-100 p-2 rounded-md">
                        <div class="flex items-center gap-3">
                            <span class="h-5 w-5 rounded-full" style="background-color: ${branch.color || '#ccc'}"></span>
                            <div>
        ...
        if (creditTypesList) {
            ...
            if (creditTypesCount) {
                creditTypesCount.textContent = `(${credits.length})`;
            }
            ...
            if (credits.length === 0) {
                ...
                creditTypesList.innerHTML = credits.map((ct, idx) => {
                    ...
                    <li class="flex justify-between items-center bg-slate-100 p-2 rounded-md">
                        <div class="flex items-center gap-3">
                            <span class="h-5 w-5 rounded-full" style="background-color: ${ct.color || '#ccc'}"></span>
        ...
        if (monthlyTiersList) {
            ...
            if (monthlyTiersCount) {
                monthlyTiersCount.textContent = `(${tiers.length})`;
            }
            ...
            if (tiers.length === 0) {
                ...
                monthlyTiersList.innerHTML = tiers.map((tier, idx) => {
                    ...
                    <li class="flex justify-between items-center bg-slate-100 p-2 rounded-md">
                        <div class="flex items-center gap-3">
                            <span class="h-5 w-5 rounded-full" style="background-color: ${tier.color || '#ccc'}"></span>
    ...
    function handleAdminListClick(e) {
        // Updated selector to include .view-qr-btn
        const target = e.target.closest('.edit-btn, .delete-btn, .move-btn, .view-qr-btn');
        if (!target) return;
        ...
        if (target.classList.contains('view-qr-btn')) {
            // New Handler
            openBranchQrModal(id);
        } 
        else if (target.classList.contains('edit-btn')) {
            // ... existing logic ...
            if (type === 'sportType') openSportTypeModal(appState.sportTypes.find(st => st.id === id));
            else if (type === 'tutor') openTutorModal(appState.tutors.find(t => t.id === id));
            else if (type === 'creditType') openCreditTypeModal(appState.creditTypes.find(ct => ct.id === id));
            else if (type === 'monthlyTier') openMonthlyPlanTierModal(appState.monthlyPlanTiers.find(t => t.id === id));
            else if (type === 'branch') openBranchModal(appState.branches.find(b => b.id === id));
        } 
    ...
    async function renderSalaryPage(container) {
        ...
        const updateAvailableTutors = () => {
            ...
            let classesForTutorList = monthlyClasses;

            // 1. Filter the cached monthly classes by the selected branch
            if (selectedBranchId !== 'all') {
                classesForTutorList = monthlyClasses.filter(c => {
                    const cBranch = c.branchId || branches[0].id;
        ...
        const renderSalaryDetails = (filteredClasses) => {
                    ...
                    if (cls.bookedBy) {
                        Object.keys(cls.bookedBy).forEach(mid => {
                            if (memberIdsInPeriod.has(mid)) {
                                ...
                                if (member) relevantBookings.push({ member, cls });
                            }
                        });
                    }
                });
            ...
            const { revenueByClsId } = calculateRevenueForBookings(relevantBookings, 'actual');
            ...
            const clsDetails = filteredClasses.map(cls => {
                ...
                if (cls.payoutDetails && typeof cls.payoutDetails.salaryValue !== 'undefined') {
                    ...
                    if (salaryType === 'perCls') {
                        ...
                        const headcount = enrollmentsCount; 
                ...
                if (hasMultipleBranches) {
                    ...
                    branchName = (appState.currentLanguage === 'zh-TW' && branch?.name_zh) ? branch.name_zh : (branch?.name || _('label_legacy_branch'));
                }
    ...
    async function renderStatisticsPage(container) {
        // 1. Prepare Branch Filter
        // We use a fresh reference to appState.branches here for the UI construction
        const branches = appState.branches || [];
        const hasMultipleBranches = branches.length > 1;
        let branchFilterHTML = '';
        ...
        const renderFilteredStats = async () => {
            ...
            // --- FIX START: Apply Branch Filter using FRESH state ---
            // We re-access appState.branches here to ensure we aren't using a stale closure variable
            // if the page rendered before the branches were fully fetched.
            const currentBranches = appState.branches || [];
            const hasBranchesConfigured = currentBranches.length > 1; 
            const selectedBranchId = branchSelect ? branchSelect.value : 'all';

            let filteredClasses = rawClasses;
            
            if (hasBranchesConfigured && selectedBranchId !== 'all') {
                filteredClasses = rawClasses.filter(c => {
                    // Logic:
                    // 1. If class has a branchId, use it.
                    // 2. If class has NO branchId (legacy), it belongs to the First Branch.
                    // 3. Compare safely.
                    const effectiveBranchId = c.branchId || (currentBranches[0] ? currentBranches[0].id : null);
                    return effectiveBranchId === selectedBranchId;
                });
            }
            // --- FIX END ---
    ...
    function renderColorPicker(container, colorInput) {
        ...
        // If current color is not in the standard list, it's custom
        const isCustom = currentColor && !standardColors.includes(currentColor);

        // 3. Render Standard Swatches
        // IMPORTANT: Removed the <div> wrapper to ensure CSS alignment works correctly.
        let html = standardColors.map((color, index) => `
            <input type="radio" name="color" id="color-${index}" value="${color}" class="color-swatch-radio" ${color === currentColor ? 'checked' : ''}>
            <label for="color-${index}" class="color-swatch-label" style="background-color: ${color};" title="${color}"></label>
        `).join('');

        // 4. Render Custom Picker (Last Slot)
        // We reuse the existing 'color-swatch-label' class so it aligns perfectly with the dots.
        // Added 'flex items-center justify-center' to center the icon inside the dot.
        const customBg = isCustom ? currentColor : '#f1f5f9'; // Light gray if empty
        const customBorder = isCustom ? 'none' : '2px dashed #cbd5e1'; // Dashed border if empty
        const iconHiddenClass = isCustom ? 'hidden' : '';

        html += `
            <input type="radio" name="color" id="color-custom" value="custom" class="color-swatch-radio" ${isCustom ? 'checked' : ''}>
            <label for="color-custom" id="lbl-custom" class="color-swatch-label flex items-center justify-center relative overflow-hidden" style="background-color: ${customBg}; border: ${customBorder};" title="Custom Color">
                <!-- Plus Icon -->
                <svg id="icon-custom" xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-slate-400 ${iconHiddenClass} pointer-events-none" viewBox="0 0 20 20" fill="currentColor">
                    <path fill-rule="evenodd" d="M10 3a1 1 0 011 1v5h5a1 1 0 110 2h-5v5a1 1 0 11-2 0v-5H4a1 1 0 110-2h5V4a1 1 0 011-1z" clip-rule="evenodd" />
                </svg>
                <!-- Invisible Native Color Input covering the label -->
                <input type="color" id="inp-custom" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer" value="${isCustom ? currentColor : '#ffffff'}">
            </label>
        `;
        ...
        // 5. Attach Logic
        const customRadio = container.querySelector('#color-custom');
        const customLabel = container.querySelector('#lbl-custom');
        const customIcon = container.querySelector('#icon-custom');
        const customInput = container.querySelector('#inp-custom');

        // A. Handle Custom Input Changes (Color Picker)
        const handleCustomChange = (e) => {
            const newColor = e.target.value;
            
            // Update the hidden target input for the form
            colorInput.value = newColor;
            
            // Visually select the radio
            customRadio.checked = true;
            
            // Update the swatch appearance to show selected color
        ...
        // Ensure clicking the custom swatch selects the radio even if color isn't changed
        customInput.addEventListener('click', () => { customRadio.checked = true; });

        // B. Handle Standard Radio Changes
        // Listen on container for bubbling events to detect when standard radios are clicked
        container.addEventListener('change', (e) => {
            // If a standard radio button was clicked (not the custom one)
            if (e.target.name === 'color' && e.target.id !== 'color-custom' && e.target.type === 'radio') {
                colorInput.value = e.target.value;
                
                // Reset custom swatch visual back to "Add" state (dashed gray)
    ...
    async function renderClassesPage(container) {
        ...
        // --- 5. Helper: Populate Filters Dynamically ---
        ...
        const fetchAndRenderClasses = async () => {
            ...
            // FIX: Removed the automatic reset to page 1 here.
            // Page reset is now handled only on filter change events.
            updateClassesTable();
        };
        ...
        const getClsBranchName = (cls) => {
            const bId = cls.branchId || (branches[0] ? branches[0].id : null);
            const branch = branches.find(b => b.id === bId);
            return (appState.currentLanguage === 'zh-TW' && branch?.name_zh) ? branch.name_zh : (branch?.name || _('label_legacy_branch'));
        };
        ...
        const updateClassesTable = () => {
            ...
            // Safety check: if data shrank and current page is out of bounds, reset to last available
            if (page > totalPages) page = totalPages;
            if (page < 1) page = 1; // Ensure strictly 1 if empty or reset

            // Update appState with safe page
        ...
        // FIX: Explicitly reset pagination to 1 ONLY when the user changes a filter manually.
        
        monthFilter.onchange = () => {
            appState.selectedFilters.classesPeriod = monthFilter.value;
            appState.pagination.classes.page = 1; // Manual change -> Reset
    ...
    async function renderIncomePage(container) {
        ...
        const renderIncomeDetails = async () => {
            ...
            let filteredClasses = classesInPeriod;
            if (hasMultipleBranches && branchSelect.value !== 'all') {
                const selectedBranch = branchSelect.value;
                filteredClasses = classesInPeriod.filter(c => {
                    const cBranch = c.branchId || branches[0].id;
                    return cBranch === selectedBranch;
                });
            }
            ...
            filteredClasses.forEach(cls => {
                ...
                if (cls.payoutDetails && typeof cls.payoutDetails.salaryValue !== 'undefined') {
                    ...
                    if (salaryType === 'perCls') {
                        ...
                        clsPayout = clsGross * (salaryValue / 100);
                    } else if (salaryType === 'perHeadcount') {
                        clsPayout = bookingsForClass.length * salaryValue;
                    }
                }
    ...
    async function openMemberBookingHistoryModal(member) {
        ...
        try {
            ...
            // --- FILTER & CALCULATION LOGIC START ---
            
            // 1. Identify Unique Credit Types AND Calculate Usage
            const usedCreditTypeIds = new Set();
            const creditUsageMap = { 'all': 0 };
            ...
            memberBookings.forEach(cls => {
                ...
                // Specific Type Total
                if (!creditUsageMap[typeId]) {
                    creditUsageMap[typeId] = 0;
                }
                creditUsageMap[typeId] += creditsUsed;
            });
            ...
            // 3. Helper to Render Pills
            const getPillsHTML = () => {
                if (memberBookings.length === 0) return ''; 

                let pillsHTML = '<div class="flex flex-wrap gap-2 justify-center mb-4">';
                
                const allClass = currentFilterId === 'all' ? 'bg-slate-800 text-white shadow-md' : 'bg-slate-100 text-slate-600 hover:bg-slate-200';
                const allUsageText = _('label_used_credits_suffix').replace('{count}', formatCredits(creditUsageMap['all'] || 0));
                
                pillsHTML += `<button class="modal-filter-pill px-3 py-1 text-xs font-bold rounded-full transition-all ${allClass}" data-type-id="all">${_('filter_all')} ${allUsageText}</button>`;
                ...
                if (usedCreditTypeIds.size > 1) {
                    usedCreditTypeIds.forEach(typeId => {
                        ...
                        let style = '';
                        let className = 'modal-filter-pill px-3 py-1 text-xs font-bold rounded-full transition-all ml-1 ';
                        
                        if (isActive) {
                            style = `background-color: ${baseColor}; color: white; box-shadow: 0 4px 6px -1px ${baseColor}66;`;
                        } else {
                            style = `background-color: ${baseColor}15; color: ${baseColor};`;
            ...
            const renderContent = () => {
                ...
                const filteredBookings = memberBookings.filter(cls => {
                    if (currentFilterId === 'all') return true;
                    const booking = cls.bookedBy[member.id];
                    const typeId = booking.creditTypeId || cls.costCreditTypeId || 'general';
                    return typeId === currentFilterId;
                });
                    ...
                    return `<div class="bg-slate-100 p-4 rounded-lg flex justify-between items-center">
                        <div>
                            ...
                            <div class="text-xs text-slate-600 mt-1 flex items-center">
                                ${_('label_credits_used')} ${creditsUsed} ${typePill}
                            </div>
                            <p class="text-xs text-slate-500 mt-0.5">${formatBookingAuditText(bookingDetails)}</p>
                        </div>
                        ${isAttended 
            ...
            // 5. Update the Main Container Structure (Added pagination div)
            DOMElements.memberBookingHistoryModal.innerHTML = `
            ...
            const bindPillEvents = () => {
                ...
                container.querySelectorAll('.modal-filter-pill').forEach(btn => {
                    btn.onclick = () => {
                        currentFilterId = btn.dataset.typeId;
                        appState.pagination.memberHistory.page = 1; // Reset pagination
                        renderContent();
                        bindPillEvents();
                    };
                });
    ...
    const setupClassSubscription = (branchId) => {
        ...
        console.log(`[Optimization] Switching class subscription. Context Branch: ${branchId}`);
        ...
        // Start Date: Based on how far back the owner has scrolled (0 initially)
        const startDate = new Date();
        startDate.setUTCHours(0, 0, 0, 0);
        startDate.setUTCDate(today.getUTCDate() - daysToLookBack);

        // End Date: Cap at 60 days into the future.
        // This prevents downloading classes 6 months away that aren't visible yet.
        // If the user jumps to a future date via "Go To Date", this function re-runs 
        // and shifts the window automatically.
        const endDate = new Date(startDate);
        endDate.setUTCDate(endDate.getUTCDate() + 60);

        query = classesRef.orderByChild('date')
            .startAt(getIsoDate(startDate))
            .endAt(getIsoDate(endDate)); // <--- THE FIX
        ...
        const isVisibleForMember = (cls) => {
            ...
            if (typeof statusRaw === 'boolean') {
                ...
                return statusRaw[cls.branchId] === true;
            }
            return false; 
        };
    ...
    const initOwnerListeners = () => {
        ...
        // --- Listeners for foundational data ---
        const nonUserRefs = {
        ...
        Object.entries(nonUserRefs).forEach(([key, ref]) => {
            dataListeners[key] = (snapshot) => {
                ...
                if (key === 'currentUser') {
                    ...
                    // Force refresh to show/hide drafts if status changes
                    ...
                    // --- OPTIMIZATION START: Initialize Class Subscription ---
                    if (key === 'branches' && appState.branches.length > 0) {
                        const defaultBranch = appState.currentUser.homeBranchId || appState.branches[0].id;
                        
                        if (!appState.selectedScheduleBranch) {
                            appState.selectedScheduleBranch = defaultBranch;
                        }
        ...
        // --- OPTIMIZED USER LISTENER ---
        // Removed heavy JSON.stringify comparisons. 
        // Now relies on Page-Context logic to avoid unnecessary re-renders.
        const usersRef = database.ref('/users');
        dataListeners.users = (snapshot) => {
            // 1. Always update the local data state instantly
            const newUsersRaw = snapshot.val();
            appState.users = firebaseObjectToArray(newUsersRaw);

            // 2. Optimization: Check which page is active.
            // We ONLY re-render if the current page actually displays a list of users.
            // - 'schedule': UI depends on Classes listener. User updates (like credits/names) are resolved dynamically on click.
            // - 'account', 'check-in': Use currentUser listener.
            ...
            if (pagesRequiringUserListRender.includes(appState.activePage)) {
                // 3. Throttle: Prevent rapid-fire renders if multiple updates come in at once
                if (window.userRenderTimeout) clearTimeout(window.userRenderTimeout);
                window.userRenderTimeout = setTimeout(() => {
                    renderCurrentPage();
                }, 100);
            }
            
            // Note: Modals like "Add Walk-in" read appState.users directly when typing.
            // Since step #1 updated appState.users, the search results will be correct immediately 
            // without needing a full page re-render.
        };
    ...
    const initMemberListeners = () => {
        ...
        const keysToOff = ['tutors', 'sportTypes', 'studioSettings', 'creditTypes', 'monthlyPlanTiers', 'branches'];
        keysToOff.forEach(k => { if(dataListeners[k]) database.ref(`/${k}`).off('value', dataListeners[k]); });
        if (dataListeners.currentUser) database.ref('/users/' + appState.currentUser.id).off('value', dataListeners.currentUser);
        
        // Remove legacy class listener if it exists
        ...
        Object.entries(refs).forEach(([key, ref]) => {
            dataListeners[key] = (snapshot) => {
                ...
                if (key === 'studioSettings') {
                    ...
                    // --- OPTIMIZATION START: Initialize Class Subscription ---
                    // Re-render schedule if branches load late to ensure dropdowns appear
                    if (key === 'branches') {
                        if (appState.branches.length > 0) {
                            const defaultBranch = appState.currentUser.homeBranchId || appState.branches[0].id;
                            
                            // Initialize global selection if not set
                            if (!appState.selectedScheduleBranch) {
                                appState.selectedScheduleBranch = defaultBranch;
                            }
                            
                            // Trigger the optimized fetch immediately
                            setupClassSubscription(appState.selectedScheduleBranch);
                        }
        ...
        // Initialize Schedule Status listener for Members
        const statusRef = database.ref('/scheduleStatus');
        dataListeners.scheduleStatus = (snapshot) => {
            appState.scheduleStatus = snapshot.val() || {};
            
            // CRITICAL FIX: If on schedule page, force a data refresh.
            // Why? Classes that were "Draft" were filtered out and are not in appState.classes.
            // We must re-run the subscription so 'isVisibleForMember' logic sees the new 'Published' status
            // and downloads the missing classes.
            if (appState.activePage === 'schedule') {
                // 1. Reset the tracker so setupClassSubscription doesn't skip the request
                currentSubscribedBranchId = null; 
                
                // 2. Re-trigger the fetch for the current branch
                setupClassSubscription(appState.selectedScheduleBranch);
            }
        };
    ...
    const init = () => {
        ...
        document.body.addEventListener('click', e => {
            ...
            // --- 2. EXISTING HANDLERS (Standard Mode) ---
