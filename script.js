...
${_('whatsapp_closing')}
    ...
    function openCheckInModal() {
                ...
                <div class="relative w-full max-w-[280px] mx-auto">
                    ...
                    <div class="absolute inset-0 pointer-events-none flex items-center justify-center p-8">
                        <div class="w-full h-full border-2 border-white/50 rounded-lg relative">
                            <div class="absolute top-0 left-0 w-6 h-6 border-t-4 border-l-4 border-white rounded-tl-md"></div>
                            <div class="absolute top-0 right-0 w-6 h-6 border-t-4 border-r-4 border-white rounded-tr-md"></div>
                            <div class="absolute bottom-0 left-0 w-6 h-6 border-b-4 border-l-4 border-white rounded-bl-md"></div>
                            <div class="absolute bottom-0 right-0 w-6 h-6 border-b-4 border-r-4 border-white rounded-br-md"></div>
                        </div>
                    </div>
                </div>
        ...
        const renderOwnerShow = () => {
            ...
            contentArea.querySelector('#openPrintPreviewBtn').onclick = () => {
                ...
                // 2. Define Card HTML (Design Adjustments applied here)
                const cardHTML = `
                    <div class="print-card" style="background: white; padding: 60px 40px; border-radius: 30px; text-align: center; border: 1px solid #e2e8f0; width: 100%; max-width: 450px; margin: auto; box-shadow: 0 20px 40px -10px rgba(0,0,0,0.1);">
                        <div style="display: inline-block; background-color: ${branchColor}15; color: ${branchColor}; font-weight: 900; font-size: 18px; text-transform: uppercase; letter-spacing: 1px; padding: 12px 24px; border-radius: 100px; margin-bottom: 30px;">
                            ${branchName}
                        </div>
                        <h1 style="margin: 0 0 40px 0; font-size: 36px; color: #0f172a; line-height: 1.2;">${_('check_in_title')}</h1>
    ...
    function handleMemberScanningStudio(decodedText, onResumeCallback) {
        ...
        // 3. General Validation: Are there any bookings at this location?
        if (validBranchBookings.length === 0) {
            if (allBookingsToday.length > 0) {
                // User has bookings today, but not at this QR's location
        ...
        if (selectedId) {
            // Check if the selected class matches the scanned branch
            targetClass = validBranchBookings.find(c => c.id === selectedId);
            
            if (!targetClass) {
                // The selected class is NOT in the list of bookings for this QR code's branch.
        ...
        // SCENARIO B: No selection made (Auto-detect)
            ...
            if (unattended.length === 0) {
                ...
                // Exactly one option - Safe to auto-select
                ...
                // Don't play error sound, just a prompt
                onResumeCallback();
                return;
            }
        }

        // 5. Final Status Check (Already Checked In?)
        if (targetClass.attendedBy && targetClass.attendedBy[memberId]) {
            ...
            playSuccessSound(); // Play success/neutral sound for "already done"
            onResumeCallback();
            return;
        }
