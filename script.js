...
${_('whatsapp_closing')}
    ...
    function renderAccountPage(container) {
        ...
        // --- GENERATE PROFILE CONTENT (Unified Layout) ---
        const getProfileContent = () => {
            ...
            if (member.monthlyPlan) {
                ...
                return `
                    ${joinDateHtml}
                    <div><p data-lang-key="label_plan" class="text-sm text-slate-500 mb-1"></p>${planPill}</div>
                    <div><p data-lang-key="label_renews_every_month" class="text-sm text-slate-500"></p><p class="font-bold text-lg text-slate-800">${renewsText}</p></div>
                    <div><p data-lang-key="label_next_payment_due" class="text-sm text-slate-500"></p><p class="font-bold text-lg text-slate-800">${nextDue}</p></div>
                `;
            } else {
                // Multi-credit Logic with Pill Design
                ...
                if (activeWalletEntries.length === 0) {
                    ...
                    creditsListHtml = `
                        <div class="mb-3">
                            <div class="mb-1">
                                <span class="font-bold text-sm px-2.5 py-1 rounded-full inline-block" style="background-color: ${neutralColor}26; color: ${neutralColor};">
                                    0 ${_('label_credits')}
                                </span>
                            </div>
                        </div>`;
                ...
                return `
                    ${joinDateHtml}
                    <div class="mt-4">
                        <p data-lang-key="label_credits_remaining" class="text-sm text-slate-500 mb-2"></p>
                        ${creditsListHtml}
                    </div>
                `;
            }
                                    ...
                                    .map(p => {
                                        ...
                                        return `
                                            <div class="text-slate-600 bg-slate-50 p-2 rounded-md">
                                                <div><strong>${formatShortDateWithYear(p.date)}:</strong> ${entryText} ${tierPill}</div>
                                                ${auditMessage}
                                            </div>
                                        `;
                                    }).join('')
                            ...
                            ${purchaseHistory.length > 0 
                                ? purchaseHistory
                                    ...
                                    .map(p => {
                                        ...
                                        return `
                                            <div class="text-slate-600 bg-slate-50 p-2 rounded-md">
                                                <div><strong>${formatShortDateWithYear(p.date)}:</strong> ${entryText} ${typePill}</div>
                                                ${auditMessage}
                                            </div>
                                        `;
                                    }).join('') 
    ...
    function renderAdminLists() {
        ...
        if (creditTypesList) {
            ...
            if (credits.length === 0) {
                ...
                creditTypesList.innerHTML = credits.map(ct => {
        ...
        // 3. RENDER MONTHLY PLAN TIERS (NEW)
        ...
        if (monthlyTiersList) {
            ...
            if (tiers.length === 0) {
                ...
                monthlyTiersList.innerHTML = tiers.map(tier => {
                    const tierName = getMonthlyPlanName(tier);
                    return `
                    <li class="flex justify-between items-center bg-slate-100 p-2 rounded-md">
                        <div class="flex items-center gap-3">
    ...
    function openCreditTypeModal(creditTypeToEdit = null) {
                ...
                <form id="creditTypeForm">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="text" id="creditTypeName" class="form-input" placeholder="e.g., Personal Training" required>
                        </div>
                        <div>
                            <label for="creditTypeNameZh" data-lang-key="label_name_zh" class="block text-slate-600 text-sm font-semibold mb-2"></label>
                            <input type="text" id="creditTypeNameZh" class="form-input" placeholder="e.g., 私人教練">
                        </div>
                        <div>
        ...
        openModal(modal);
        updateUIText(); // Translate the labels
    }
        ...
        try {
            ...
            const historyListHTML = memberBookings.length === 0 ? `<p class="text-slate-500 text-center p-8">${_('no_booking_history')}</p>` :
                memberBookings.map(cls => {
                    ...
                    // Resolve Credit Type Name & Color
                    const typeId = bookingDetails.creditTypeId || cls.costCreditTypeId || 'general';
                    const typeDef = appState.creditTypes ? appState.creditTypes.find(ct => ct.id === typeId) : null;
                    // USE HELPER
                    const typeName = typeDef ? getCreditTypeName(typeDef) : (typeId === 'general' ? 'General' : 'Credits');
                    const typeColor = typeDef?.color || '#64748b'; // Fallback color
