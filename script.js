                    ...
                    ${bookedMembers.map(m => `
                        <div class="flex justify-between items-center bg-white p-3 rounded shadow-sm border-l-4 ${isPriceIncrease ? 'border-red-400' : 'border-yellow-400'}">
                            <div>
                                ...
                                <div class="text-xs text-slate-500">${_('label_paid')}: ${m.creditsPaid}</div>
                            </div>
...
${_('whatsapp_closing')}
            ...
            if (target) {
                ...
                if (memberToAdd && !memberToAdd.monthlyPlan) {
                    ...
                    if (currentBalance < parseFloat(currentCls.credits)) {
                        ...
                        const typeName = typeDef ? typeDef.name : 'Credits';
    ...
    function handleClsFormSubmit(e) {
        ...
        // --- EXISTING CHECK: Percentage Model Safeguard ---
        if (clsId) {
            const originalCls = appState.classes.find(c => c.id === clsId);
            if (originalCls) {
                const creditsChanged = originalCls.credits !== newCredits;
                const isPercentageModel = originalCls.payoutDetails && originalCls.payoutDetails.salaryType === 'percentage';
                const hasBookings = originalCls.bookedBy && Object.keys(originalCls.bookedBy).length > 0;

                if (creditsChanged && isPercentageModel && hasBookings) {
                    const errorMsg = _('error_credits_locked_percentage') === 'error_credits_locked_percentage' 
                        ? "Cannot change credits: This class has bookings and uses a Percentage salary model. Changing credits affects salary accuracy." 
                        : _('error_credits_locked_percentage');
        ...
        const newClsDataFromForm = {
            ...
            credits: newCredits, 
            costCreditTypeId: form.querySelector('#clsCreditType').value, // <--- NEW FIELD SAVED
    ...
    function renderAccountPage(container) {
        ...
        const getProfileContent = () => {
            ...
            if (member.monthlyPlan) {
                const planPill = `<span class="bg-green-100 text-green-800 text-base font-medium px-3 py-1 rounded-full">${formatCurrency(member.monthlyPlanAmount)}/mo</span>`;
                ...
                let creditsListHtml = '';
                if (entries.length === 0) {
                    creditsListHtml = `<p class="font-bold text-lg text-slate-400">0 Credits</p>`;
                } else {
    ...
    function openMemberModal(memberToEdit) {
        ...
        // FIXED: Added stroke="currentColor"
        ...
        creditActionBtn.onclick = () => {
            ...
            if (historyId) {
                ...
                const typeIdToUpdate = originalEntry.creditTypeId || 'general';
                ...
                database.ref(`/users/${memberId}/purchaseHistory/${historyId}`).update(entryUpdate)
                    .then(() => {
                        const currentBalance = wallet[typeIdToUpdate]?.balance || 0;
                        const currentInitial = (wallet[typeIdToUpdate]?.initialCredits !== undefined) ? wallet[typeIdToUpdate].initialCredits : currentBalance;
                        
                        const updates = {};
                        updates[`/users/${memberId}/wallet/${typeIdToUpdate}/balance`] = currentBalance + creditDifference;
                        updates[`/users/${memberId}/wallet/${typeIdToUpdate}/initialCredits`] = currentInitial + creditDifference;
                        updates[`/users/${memberId}/wallet/${typeIdToUpdate}/expiryDate`] = expiryDate;
    ...
    async function handleMemberFormSubmit(e, originalMember) {
        ...
        if (isMonthly) {
            ...
            // We check loosely here because balance might be in any wallet bucket
            if (!expiryDate) {
                // Ideally we check if ANY bucket has balance, but for now, enforcing it generally is safer
                showMessageBox(_('error_expiry_date_required'), 'error');
                return; 
            }
        }
    ...
    function renderAdminPage(container) {
        const isOwner = appState.currentUser?.role === 'owner';
        // Change layout to 3 columns if on large screen to accommodate the new card
        const gridClasses = isOwner ? 'grid-cols-1 md:grid-cols-2 xl:grid-cols-3' : 'grid-cols-1';
        ...
        container.innerHTML = `
            <div class="space-y-8">
                ...
                <div class="grid ${gridClasses} gap-8 items-start">
                    ...
                    <!-- NEW: Credit Types Card (Only for Owner) -->
                    ${isOwner ? `
                    <div class="card p-6 flex flex-col">
                        <div class="flex flex-wrap gap-4 justify-between items-center mb-4">
        ...
        // NEW: Attach Listener for Credit Types
    ...
    function renderAdminLists() {
        ...
        // 2. RENDER CREDIT TYPES (UPDATED)
        const creditTypesList = document.getElementById('creditTypesList');
        if (creditTypesList) {
            const credits = appState.creditTypes || [];
            if (credits.length === 0) {
                creditTypesList.innerHTML = `<li class="text-center text-slate-500 p-4">No credit types defined.</li>`;
            } else {
                creditTypesList.innerHTML = credits.map(ct => {
                    // Use helper for localized name
    ...
    function handleAdminItemDelete(type, id, name) {
        ...
        if (type === 'sportType') {
            ...
            // Check if any user has a balance in this credit type
        ...
        const itemTypeText = type === 'sportType' ? _('label_sport_type_item') : (type === 'creditType' ? 'Credit Type' : _('label_tutor_item'));
    ...
    const initOwnerListeners = () => {
        ...
        const nonUserRefs = {
            ...
            creditTypes: database.ref('/creditTypes') // <--- NEW: Added this line
        };
        ...
        Object.entries(nonUserRefs).forEach(([key, ref]) => {
            dataListeners[key] = (snapshot) => {
                ...
                if (key === 'currentUser') {
                    ...
                    // Handles lists: tutors, sportTypes, AND creditTypes
                    appState[key] = firebaseObjectToArray(val);
                    
                    // Convert array back to object map for easier lookup in UI rendering if needed, 
                    // though appState usually keeps arrays.
                    // If we are on the admin page, re-render the lists.
                    if (appState.activePage === 'admin') {
                        renderAdminLists();
                    }
                    
                    // If we are on members or account page, a change in credit types (e.g. color) 
                    // might require a re-render to show new badges.
                    if ((key === 'creditTypes') && (appState.activePage === 'members' || appState.activePage === 'account')) {
                        renderCurrentPage();
                    }
                }
            };
    ...
    const initMemberListeners = () => {
        ...
        if (dataListeners.studioSettings) database.ref('/studioSettings').off('value', dataListeners.studioSettings);
        ...
        if (dataListeners.creditTypes) database.ref('/creditTypes').off('value', dataListeners.creditTypes); // <--- NEW Cleanup
        ...
        Object.entries(refs).forEach(([key, ref]) => {
            dataListeners[key] = (snapshot) => {
                ...
                if (key === 'studioSettings') {
                    ...
                    // Handles tutors, sportTypes, AND creditTypes
                    appState[key] = firebaseObjectToArray(val);
                    // Re-render if we are viewing account page to update wallet colors/names
                    if (key === 'creditTypes' && appState.activePage === 'account') {
                        renderCurrentPage();
                    }
                }
            };
