...
${_('whatsapp_closing')}
    ...
    async function handleMemberSelfCheckIn(decodedText, html5QrCodeInstance) {
        const resultEl = document.getElementById("checkInResult");
        const member = appState.currentUser;
        
        // 1. Immediate Feedback: Pause Scanner & Show Status
        if (html5QrCodeInstance) {
            try { html5QrCodeInstance.pause(); } catch(e) {} // Ignore if already paused
        }
        if (resultEl) {
        ...
        if (qrData.type === 'studio_dynamic') {
            ...
            if (now - qrTime > 60000) { // 60 seconds validity
                if (resultEl) resultEl.innerHTML = `<div class="check-in-result-banner check-in-error">${_('error_qr_dynamic_expired')}</div>`;
                return;
            }
        }

        // 3. FETCH DATA (The Fix for "No Response")
        // We fetch today's classes directly from DB to ensure we have the data even if Schedule page wasn't loaded.
        const todayIso = getIsoDate(new Date());
        let bookingsToday = [];
        
        try {
            // Query DB for classes on this date
            const snapshot = await database.ref('/classes').orderByChild('date').equalTo(todayIso).once('value');
            const classesObj = snapshot.val() || {};
            const allClassesToday = Object.entries(classesObj).map(([id, val]) => ({ id, ...val }));
            
            // Filter for this member's bookings
            bookingsToday = allClassesToday.filter(cls => 
        ...
        // 4. Filter by Branch (if QR has specific branch)
        const validBookings = bookingsToday.filter(cls => {
            if (!qrData.branchId) return true; // Legacy/Global QR matches all
            
            // If class has no branchId, fallback to 'legacy' or first branch logic
            const clsBranchId = cls.branchId || (appState.branches[0] ? appState.branches[0].id : null);
        ...
        if (validBookings.length === 1) {
            // Single Booking - Auto Check-in
            await performCheckIn(validBookings[0].id, member.id, html5QrCodeInstance);
        } else {
            // Multiple Bookings - Explicit Choice
            // We need to ensure appState.sportTypes is loaded for the modal text
            if (appState.sportTypes.length === 0) {
                 const stSnap = await database.ref('/sportTypes').once('value');
                 appState.sportTypes = firebaseObjectToArray(stSnap.val());
            }
    ...
    function openCheckInModal() {
        ...
        // --- 2. Build Modal HTML ---
        modal.innerHTML = `
            <div class="bg-white p-6 rounded-2xl shadow-2xl w-full max-w-lg transform transition-all duration-300 scale-95 opacity-0 modal-content relative min-h-[500px]">
                <button class="modal-close-btn"><svg class="h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" /></svg></button>
                ...
                <!-- Content Container -->
                <div id="modalCheckInContent" class="flex flex-col items-center w-full"></div>
            </div>
        `;
        ...
        // --- View A: Scanner (Owner scanning Member OR Member scanning Studio) ---
        const renderScanner = (isSelfCheckIn) => {
            cleanup();
            const promptKey = isSelfCheckIn ? 'tab_scan_studio' : 'check_in_prompt';
            
            // --- RESTORED: Device Location Selector (Owner Mode Only) ---
            let deviceLocationHtml = '';
            
            if (!isSelfCheckIn && hasMultipleBranches) {
                // Load from LocalStorage (Standard Browser Cache)
            ...
            contentArea.innerHTML = `
                ${deviceLocationHtml}
                <p class="text-slate-500 mb-4 text-center">${_(promptKey)}</p>
                <div id="qr-reader" class="bg-slate-900 rounded-lg overflow-hidden w-full max-w-[300px] h-[300px] border-4 border-slate-200"></div>
                <div id="checkInResult" class="w-full mt-4 min-h-[3rem]"></div>
            `;

            // Bind Logic for Selector (Save to Cache)
        ...
        const renderStationMode = () => {
            cleanup(); // Clear previous interval and HTML

            // 1. Segmented Button Branch Selector
            let branchSelectHtml = '';
            if (hasMultipleBranches) {
                const buttonsHtml = branches.map(b => {
            ...
            contentArea.innerHTML = `
                ...
                <!-- Print Button -->
                <button id="modalPrintStaticBtn" class="mt-6 flex items-center justify-center gap-2 bg-white border border-slate-300 hover:border-indigo-500 text-slate-600 hover:text-indigo-600 font-bold py-3 px-6 rounded-xl transition-all shadow-sm w-full group">
            ...
            // Event: Branch Change (Segmented Buttons)
            if (hasMultipleBranches) {
                contentArea.querySelectorAll('.station-branch-btn').forEach(btn => {
                    btn.onclick = () => {
                        currentStationBranchId = btn.dataset.branchId;
                        renderStationMode(); 
                    };
                });
            }
            ...
            document.getElementById('modalPrintStaticBtn').onclick = () => {
                ...
                setTimeout(() => {
                    openBranchQrModal(currentStationBranchId);
                }, 300);
            };
        };
        ...
        const renderMyCode = () => {
            ...
            contentArea.innerHTML = `
                <div id="qrCodeContainer" class="w-64 h-64 mx-auto mb-4 border-4 border-white shadow-sm rounded-xl overflow-hidden bg-white"></div>
                <p class="text-center text-slate-500 text-sm">${user.name}</p>
            `;
            const qrContainer = contentArea.querySelector('#qrCodeContainer');
            new QRCode(qrContainer, {
                text: user.id,
