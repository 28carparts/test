...
${_('whatsapp_closing')}
    ...
    function openMemberModal(memberToEdit) {
        ...
        if (monthlyTiers.length > 0) { 
            ...
            // Show explicit message if no tiers exist and not a legacy member
        ...
        // FIX: Always ensure the dropdown is visible, even if empty
        monthlyTierSelect.parentElement.classList.remove('hidden');
        
        // FIX: Enforce selection using browser validation
        // This prevents submitting the form if "Enable Subscription" is checked but no tier is picked
        ...
        // Determine placeholder: If no custom types AND no legacy usage, show warning
        ...
        const refreshMemberData = () => {
            ...
            database.ref(`/users/${memberToEdit.id}`).once('value', snapshot => {
                ...
                // UPDATED CALL: Added 'creditEditModeLabel' at the end
        ...
        paymentActionBtn.onclick = () => {
            ...
            if (!monthlyTierSelect.checkValidity()) {
                monthlyTierSelect.reportValidity(); // Shows the browser's native validation bubble
                return;
            }
            ...
            if (historyId) {
                database.ref(`/users/${memberId}/paymentHistory/${historyId}`).update(entryData).then(() => {
                    ...
                    database.ref(`/users/${memberId}`).once('value', s => { 
                        ...
                        // UPDATED CALL: Added 'paymentEditModeLabel' at the end
        ...
        // --- KEY HELPER FOR RESETTING UI ---
        const toggleMonthlyInputs = () => {
            if (enableMonthlyCheckbox.checked) {
                // Enabled: Make fields interactive and Require the Tier
                ...
                // Disabled: Dim fields, Disable input, and Remove Requirement
                monthlyInputsContainer.classList.add('opacity-50', 'pointer-events-none');
                monthlyTierSelect.disabled = true;
                monthlyTierSelect.required = false; // <--- FIX: This allows the form to submit
            }
        };
