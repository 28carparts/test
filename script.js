...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const renderSubscriptionBadge = () => {
        ...
        // 3. Only show for Owners (or Staff if you prefer)
        // If you want Members to see it too, remove this check.
        ...
        badge.className = 'plan-badge ml-2 px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider align-middle';
        ...
        headerLink.classList.add('flex', 'items-center'); // Ensures badge sits next to logo vertically centered
...
${_('whatsapp_closing')}
    ...
    async function openJoinedMembersModal(cls) {
        ...
        addMemberSearchResults.onclick = (e) => {
            ...
            if (target) {
                ...
                // --- HYBRID WALKIN LOGIC WITH BRANCH VALIDATION ---
                ...
                if (paymentMethod === 'none') {
                    if (currentBalance >= classCost) {
                        if (checkAccessValidity(memberToAdd, currentCls.branchId, creditConfig)) {
                            paymentMethod = 'credit';
                            failureReason = ''; // Clear reason if credit fallback works
                        } else {
                            failureReason = 'branch_restricted'; // Update reason if credits also fail on branch
                        }
                    } else if (!failureReason) {
                        // Only set reason if not already failed due to branch
                ...
                if (paymentMethod === 'none') {
                    ...
                    if (failureReason === 'branch_restricted') {
                        // Specific message for admins
    ...
    function openBookingModal(cls) {
        ...
        if (paymentMethod === 'monthly') {
            ...
            paymentMethodHtml = `<span class="inline-block text-sm font-bold px-3 py-1 rounded-full" style="${creditPillStyle}">${creditTypeName}</span>`;
            ...
            if (creditIneligibilityReason === 'credits_expired') {
                // --- NEW: Expired UI ---
                balanceHtml = `<span class="inline-block text-sm font-bold px-3 py-1 rounded-full bg-red-100 text-red-600 border border-red-200" title="${_('status_credits_expired').replace('{type}', creditTypeName)}">${formatCredits(userBalance)} (${_('label_expired')})</span>`;
            ...
            if (currentUser.monthlyPlan && planIneligibilityReason) {
                let reasonText = '';
                if (planIneligibilityReason === 'plan_expired') reasonText = _('status_overdue');
                ...
                noteHtml = `<div class="mt-2 text-xs text-orange-600 bg-orange-50 p-2 rounded border border-orange-100 flex items-center gap-2">
        ...
        confirmBtn.onclick = () => {
            ...
            if (cls.bookedBy && cls.bookedBy[memberId]) {
                ...
                if (currentUser.monthlyPlan && planIneligibilityReason === 'plan_expired') {
                    showMessageBox(_('status_overdue'), 'error');
                }
                // 2. Credits Expired
    ...
    function _renderScheduleCarousel(container, startDate, endDate, datesArray, initialScrollIndex, showAddButton, filteredClasses = null) {
        ...
        datesArray.forEach(dateIso => {
            ...
            // Added the Delete Day button below the copy buttons
    ...
    function renderCheckInPage(container) {
        ...
        // --- BRANCH LOGIC REMOVED: Members do not need to set device location --- 
        ...
        // --- UPDATED HTML: Removed Device Selector ---
        container.innerHTML = `
                            ...
                            ? `<h5 class="text-center font-semibold text-slate-600">${_('header_upcoming_checkins')}</h5>` +
                              todaysUnattendedBookings
                                ...
                                .map(cls => {
                                    ...
                                    // Helper for Branch Name (Optional visualization for member)
                                    const branches = appState.branches || [];
                                    const hasMultipleBranches = branches.length > 1;
                                    const clsBranchId = cls.branchId || (branches.length > 0 ? branches[0].id : null);
                                    const clsBranch = branches.find(b => b.id === clsBranchId);
                                    
                                    // Localization for Branch Name & Color
                                    let branchName = '';
                                    let branchColor = '#64748b'; // Default gray
                                    
                                    if (hasMultipleBranches && clsBranch) {
                                    ...
                                    // --- CHANGED: Pill Design with Prefix ---
                                    const branchBadge = branchName 
                                        ? `<span class="inline-block mt-1 text-[10px] font-bold px-2 py-0.5 rounded-full border" style="background-color: ${branchColor}15; color: ${branchColor}; border-color: ${branchColor}30;">${_('info_class_branch')}: ${branchName}</span>` 
                                        : '';

                                    // --- CHANGED: Center Aligned, Stacked Layout ---
                                    return `
                                    <button class="w-full p-4 bg-slate-100 hover:bg-indigo-50 rounded-lg transition-colors cursor-pointer flex flex-col items-center justify-center gap-1 border border-transparent hover:border-indigo-100 ${isSelected ? 'checkin-selected ring-2 ring-indigo-500 bg-indigo-50' : ''}" data-checkin-cls-id="${cls.id}">
                                        <div class="text-lg font-bold text-slate-800 leading-tight">${getSportTypeName(sportType)}</div>
                                        <div class="text-sm font-medium text-slate-500">${_('label_at_time')} ${getTimeRange(cls.time, cls.duration)}</div>
                                        ${branchBadge}
                                    </button>`;
                                }).join('') +
            ...
            tableBody.innerHTML = paginatedUsers.map(member => {
                ...
                if (member.monthlyPlan) {
                    ...
                    displayRows.push({
                        ...
                        expiryHtml: `<span class="text-sm ${dateStyle}">${dateText}</span>`
                    });
                }
    ...
    function openCheckInModal() {
        // --- Device Location Selector Logic ---
        const branches = appState.branches || [];
        const hasMultipleBranches = branches.length > 1;
        const savedDeviceBranch = localStorage.getItem('studioPulseDeviceBranch') || 'any';
        
        let deviceLocationSelector = '';
        
        // Only render the selector if there is more than 1 branch
        if (hasMultipleBranches) {
            let options = `<option value="any">${_('option_device_location_any') || 'Any Location'}</option>`;
            ...
            deviceLocationSelector = `
                <div class="mb-4 flex justify-center items-center gap-2 bg-slate-50 p-2 rounded-lg border border-slate-200">
                    ...
                    <label class="text-sm font-bold text-slate-600 whitespace-nowrap">${_('label_device_location') || 'Device Location'}:</label>
                    <select id="modalDeviceBranchSelect" class="form-select py-1 pl-2 pr-8 text-sm font-semibold w-auto bg-transparent border-none focus:ring-0 cursor-pointer">
                        ${options}
                    </select>
                </div>`;
        }
        ...
        DOMElements.checkInModal.innerHTML = `
            <div class="bg-white p-6 rounded-2xl shadow-2xl w-full max-w-lg transform transition-all duration-300 scale-95 opacity-0 modal-content relative">
                ...
                ${deviceLocationSelector}
                <div id="qr-reader" class="bg-slate-900 rounded-lg overflow-hidden"></div>
                <div id="checkInResult" class="min-h-[4rem] mt-4"></div>
            </div>
        `;
        ...
        const branchSelect = DOMElements.checkInModal.querySelector('#modalDeviceBranchSelect');
        if (branchSelect) {
            branchSelect.onchange = () => {
                localStorage.setItem('studioPulseDeviceBranch', branchSelect.value);
                showMessageBox(_('info_device_location_saved'), 'info');
            };
        }
        ...
        // Ensure we stop any previous instance before starting a new one
        if (html5QrCode && html5QrCode.isScanning) {
             html5QrCode.stop().then(() => {
                 html5QrCode.clear();
                 startScannerInstance(onScanSuccess);
             }).catch(err => console.error("Error stopping previous scan", err));
        } else {
             startScannerInstance(onScanSuccess);
        }
    }
    ...
    function calculateRevenueForBookings(bookings, revenueMode = 'actual') {
        ...
        for (const memberId in bookingsByMember) {
            ...
            // Sort classes chronologically to simulate credit usage over time
            const memberClasses = bookingsByMember[memberId].sort((a, b) => new Date(`${a.date}T${a.time}`) - new Date(`${b.date}T${b.time}`));

            // Prepare Purchase Pools (One for each credit type)
            // We create a map: typeId -> Array of purchase entries
            const purchasePools = {};
            ...
            firebaseObjectToArray(member.purchaseHistory)
                ...
                .forEach(p => {
                    // Identify type, default to 'general' for legacy data
                    const typeId = p.creditTypeId || 'general';
                    ...
                    purchasePools[typeId].push({
                        ...
                        remainingCredits: p.credits // We will decrement this as we calculate
                    });
                });

            // Legacy Support: If we have 'general' credits, treat them as a fallback
            ...
            for (const cls of memberClasses) {
                ...
                if (shouldCalculate) {
                    ...
                    // HYBRID LOGIC: Determine Payment Method from Booking Record
                    // 1. Check explicit paymentMethod tag
                    // 2. Fallback: If tag missing (legacy), check user profile status
            ...
            <div class="space-y-8">
                ...
                <div class="card p-6 md:p-8">
                    <div class="flex justify-between items-center mb-4">
                        ...
                        <!-- TRANSLATED STATUS INDICATOR -->
                        ${currentAnnounce ? `<span class="px-2 py-1 text-xs font-bold text-green-700 bg-green-100 rounded-full">${_('status_active')}</span>` : ''}
                    </div>
    ...
    async function renderSalaryPage(container) {
        ...
        // --- 2. Build Layout (New Order: Period -> Tutor -> Branch) ---
        container.innerHTML = `
            <div class="card p-6 md:p-8">
                <div class="flex flex-wrap gap-4 justify-between items-center mb-6">
        ...
        let monthlyClasses = []; // Local cache for the selected month
        ...
        // --- 4. Master Logic: Load Data for Period -> Update Tutors -> Render ---
        const loadPeriodData = async () => {
            ...
            tutorSelect.disabled = true; // Disable while loading

            // A. Fetch Classes for Month
            ...
            // B. Identify Active Tutors
            const activeTutorIds = new Set(monthlyClasses.map(c => c.tutorId));
            const activeTutors = appState.tutors.filter(t => activeTutorIds.has(t.id));

            // C. Update Tutor Dropdown
            tutorSelect.disabled = false;
            if (activeTutors.length > 0) {
                populateDropdown(tutorSelect, activeTutors);
                
                // Restore selection if valid, otherwise default to first available
                if (appState.selectedFilters.salaryTutorId && activeTutorIds.has(appState.selectedFilters.salaryTutorId)) {
                    tutorSelect.value = appState.selectedFilters.salaryTutorId;
                } else {
                    tutorSelect.value = activeTutors[0].id;
                }
            } else {
                tutorSelect.innerHTML = `<option value="">${_('info_no_classes_in_period')}</option>`;
            }

            // D. Render Table
            // Reset pagination on new period load
        ...
        const updateSalaryTable = () => {
            ...
            // Calculate Revenue/Details for the filtered list
            renderSalaryDetails(filteredClasses);
        };

        // --- 6. Helper: Calculation & HTML Generation ---
        const renderSalaryDetails = (filteredClasses) => {
            // Need to calculate revenue for these specific classes
            // Gather Member IDs involved
            const memberIdsInPeriod = new Set();
            filteredClasses.forEach(cls => {
                if (cls.bookedBy) Object.keys(cls.bookedBy).forEach(id => memberIdsInPeriod.add(id));
            });

            // We need to pass 'all relevant bookings' to the calculator.
            // Since 'calculateRevenueForBookings' requires user objects, we map them now.
            ...
            const clsDetails = filteredClasses.map(cls => {
                ...
                return { ...cls, sportTypeName: getSportTypeName(sportType), earnings, calculation, attendeesCount, branchName };
            });
            ...
            detailsContainer.innerHTML = `
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                    ...
                    <div class="bg-slate-100 p-4 rounded-lg"><p class="text-sm text-slate-500">${_('label_classes_taught')}</p><p class="text-3xl font-bold text-slate-800">${filteredClasses.length}</p></div>
                        ...
                        <tbody>
                            ${paginatedClsDetails.map(c => `
                                <tr class="border-b border-slate-100">
                                    ...
                                    ${hasMultipleBranches ? `<td class="p-2 whitespace-nowrap text-slate-600">${c.branchName}</td>` : ''}
                                ...
                                </tr>`).join('') || `<tr><td colspan="${hasMultipleBranches ? 6 : 5}" class="text-center p-4 text-slate-500">${_('info_no_classes_in_period')}</td></tr>`}
                        </tbody>
                    </table></div>
            ...
            renderPaginationControls(paginationContainer, page, totalPages, clsDetails.length, itemsPerPage.salary, (newPage) => {
                ...
                renderSalaryDetails(filteredClasses); // Re-render purely with the same filtered list
            });
        ...
        // --- 7. Event Bindings ---
        periodSelect.onchange = loadPeriodData;
        tutorSelect.onchange = updateSalaryTable;
        if (branchSelect) branchSelect.onchange = updateSalaryTable;
        
        exportBtn.onclick = async () => {
            exportBtn.disabled = true;
            exportBtn.innerHTML = _('status_exporting');

            // Re-run logic to get current state (or reuse logic if we refactored to shared state)
            // For simplicity/safety, we re-filter `monthlyClasses` based on current dropdowns
            const tutorId = tutorSelect.value;
            if (!tutorId) { 
                // Should not happen if button is active, but safe check
            ...
            let exportClasses = monthlyClasses.filter(c => c.tutorId === tutorId);
            if (branchId !== 'all') {
                exportClasses = exportClasses.filter(c => {
                     const cBranch = c.branchId || branches[0].id;
                     return cBranch === branchId;
                });
            }
            
            exportClasses.sort((a,b) => a.date.localeCompare(b.date));

            // Calculate Revenue (reuse function logic)
            // Note: We can't easily reuse the 'renderSalaryDetails' internal calculation without refactoring
            // So we repeat the 'member mapping' logic here.
            const memberIds = new Set();
            exportClasses.forEach(c => { if (c.bookedBy) Object.keys(c.bookedBy).forEach(id => memberIds.add(id)); });
            const bookings = [];
            if (memberIds.size > 0) {
                    ...
                    if (cls.bookedBy) {
                        Object.keys(cls.bookedBy).forEach(mid => {
                            if (memberIds.has(mid)) {
                                const m = appState.users.find(u => u.id === mid);
                                if (m) bookings.push({ member: m, cls });
                            }
                        });
                    }
                });
            }
            const { revenueByClsId } = calculateRevenueForBookings(bookings, 'actual');

            const exportData = exportClasses.map(cls => {
                const sportType = appState.sportTypes.find(st => st.id === cls.sportTypeId);
                let earnings = 0;
                let calculation = _('label_na');
                const attendeesCount = cls.attendedBy ? Object.keys(cls.attendedBy).length : 0;
                ...
                if (cls.payoutDetails && typeof cls.payoutDetails.salaryValue !== 'undefined') {
                    ...
                    if (salaryType === 'perCls') {
                        ...
                        earnings = clsGross * (salaryValue / 100);
                        calculation = _('salary_calculation_percentage').replace('{revenue}', formatCurrency(clsGross)).replace('{percentage}', salaryValue);
                    } else if (salaryType === 'perHeadcount') {
                        const headcount = cls.bookedBy ? Object.keys(cls.bookedBy).length : 0;
                        earnings = headcount * salaryValue;
                        const headcountUnit = _(headcount === 1 ? 'salary_unit_attendee' : 'salary_unit_attendees');
                ...
                if (hasMultipleBranches) {
                     const bId = cls.branchId || (branches[0] ? branches[0].id : null);
                     const branch = branches.find(b => b.id === bId);
                     row[_('label_branch')] = (appState.currentLanguage === 'zh-TW' && branch?.name_zh) ? branch.name_zh : (branch?.name || _('label_legacy_branch'));
                }

                Object.assign(row, {
                    [_('export_header_class_name')]: getSportTypeName(sportType),
                    [_('export_header_attendees_capacity')]: `${attendeesCount}/${cls.maxParticipants}`,
                    [_('export_header_calculation')]: calculation,
                    [_('export_header_earnings')]: earnings.toFixed(2)
                });
                return row;
            });
            ...
            const fileName = `salary-report_${tutorName}_${periodSelect.value}`;
    ...
    function renderSalaryDetails(allClasses) {
        ...
        const branchSelect = document.getElementById('salaryBranchSelect');
        const selectedBranchId = branchSelect ? branchSelect.value : 'all';
        ...
        const tutor = appState.tutors.find(t => t.id === tutorId);
        let classesInPeriod = allClasses.filter(c => c.tutorId === tutorId && c.date.startsWith(period));
        
        // Apply Branch Filter
        if (selectedBranchId !== 'all') {
            classesInPeriod = classesInPeriod.filter(c => {
                const cBranch = c.branchId || branches[0].id;
                return cBranch === selectedBranchId;
            });
        }
        ...
        const clsDetails = classesInPeriod.map(cls => {
            ...
            // Prepare branch info for display
            let branchName = '';
            if (hasMultipleBranches) {
                const bId = cls.branchId || (branches[0] ? branches[0].id : null);
                const branch = branches.find(b => b.id === bId);
        ...
        container.innerHTML = `
            ...
            const paginationContainer = container.querySelector('#salaryPagination');
            renderPaginationControls(paginationContainer, page, totalPages, clsDetails.length, itemsPerPage.salary, (newPage) => {
                appState.pagination.salary.page = newPage;
    ...
    async function renderClassesPage(container) {
        ...
        // --- 2. Build Layout (Reordered Filters) ---
        container.innerHTML = `
            <div class="card p-6 md:p-8">
                <div class="flex flex-wrap gap-4 justify-between items-center mb-6">
        ...
        const updateDynamicFilters = (classesData) => {
            ...
            // 3. Populate Dropdowns (saving current selection if possible)
            ...
            populateTutorFilter(tutorFilter, 'all', filteredTutors); // 'all' bypasses the skill check here, we just want raw list

            // Restore selection if it still exists in the new list, else default to 'all'
            if (presentSportIds.has(currentSportSelection)) {
                sportTypeFilter.value = currentSportSelection;
            } else {
                sportTypeFilter.value = 'all';
                appState.selectedFilters.classesSportTypeId = 'all'; // Sync state
            }
            ...
            if (presentTutorIds.has(currentTutorSelection)) {
                ...
                tutorFilter.value = 'all';
                appState.selectedFilters.classesTutorId = 'all'; // Sync state
            }
        };
        ...
        const fetchAndRenderClasses = async () => {
            ...
            // --- Update Dynamic Filters based on fetched data ---
        ...
        const updateClassesTable = () => {
            ...
            // Get current filter values (which might have been reset by updateDynamicFilters)
            ...
            tableBody.innerHTML = paginatedClasses.map((cls, index) => {
                ...
                // --- CLEAN TEXT BRANCH COLUMN ---
                let branchCellHTML = '';
                if (hasMultipleBranches) {
                    const bName = getClsBranchName(cls);
                    branchCellHTML = `<td class="p-2 whitespace-nowrap text-slate-600">${bName}</td>`;
                }
        ...
        if (isOwner) {
            exportBtn.onclick = () => {
                ...
                // Export logic remains similar, just ensuring sort order matches current state
                const selectedBranchId = branchFilter ? branchFilter.value : 'all';
                ...
                // ... (Apply other filters if needed for exact match export, or just branch + sort) ...
    ...
    const setupClassSubscription = (branchId) => {
        // 1. Safety Checks
        if (!branchId) return;
        if (currentSubscribedBranchId === branchId && appState.classes.length > 0) return;

        console.log(`[Optimization] Switching class subscription to Branch: ${branchId}`);
        ...
        // 4. Define the Query
        // Optimization: Query strictly by branchId to reduce bandwidth
        const classesRef = database.ref('/classes');
        let query;

        if (branchId === 'all') {
            // Fallback for "All" view: Use Date-based window (Legacy behavior)
            const today = new Date();
            const daysToLookBack = appState.ownerPastDaysVisible || 7;
            const startDate = new Date();
            startDate.setUTCHours(0, 0, 0, 0);
            startDate.setUTCDate(today.getUTCDate() - daysToLookBack);
            query = classesRef.orderByChild('date').startAt(getIsoDate(startDate));
        } else {
            // Targeted Branch Query
            query = classesRef.orderByChild('branchId').equalTo(branchId);
        }

        activeClassesRef = query;

        // 5. Helper: Visibility Check for Members
        // Members should only see classes if the day is "Published"
        const isVisibleForMember = (cls) => {
            const isAdmin = appState.currentUser?.role === 'owner' || appState.currentUser?.role === 'staff';
            if (isAdmin) return true; // Admins see everything (Drafts included)

            const scheduleStatus = appState.scheduleStatus || {};
            const todayIso = getIsoDate(new Date());
            
            // Past classes are always visible
            if (cls.date < todayIso) return true;

            // Check Status Map
            const statusRaw = scheduleStatus[cls.date];
            if (typeof statusRaw === 'boolean') {
                // Legacy Boolean: Applies to Main Branch (index 0) or Global
                const mainBranchId = appState.branches[0]?.id;
                return !cls.branchId || cls.branchId === mainBranchId;
            } else if (statusRaw && typeof statusRaw === 'object') {
                // New Object: Check specific branch key
        ...
        // We force a render once the initial payload is ready
        query.once('value', () => {
            if (appState.activePage === 'schedule') {
                renderCurrentPage();
            }
        ...
        Object.entries(nonUserRefs).forEach(([key, ref]) => {
            dataListeners[key] = (snapshot) => {
                ...
                if (key === 'currentUser') {
                    ...
                    // Once branches are loaded, we can set the initial subscription.
                    // We default to the user's home branch or the first available branch.
