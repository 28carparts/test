...
document.addEventListener('DOMContentLoaded', function() {
    ...
    /**
     * calculateWalletStatus: Analyzing the whole wallet to find alerts
     * (e.g., is ANY pack expiring soon? is ANY pack low on credits?)
     */
    const getWalletStatus = (user) => {
        ...
        let isWarning = false;
        let statusMessages = []; // Changed to array to support multiple messages
        ...
        if (user.monthlyPlan) {
            ...
            // --- Credit Plan Logic (Multi-Wallet) ---
            
            // Iterate through ALL active wallet entries to check status
            Object.entries(wallet).forEach(([typeId, data]) => {
                ...
                // B. Low Balance Check (< 5) for this specific credit type
                if (balance > 0 && balance < 5) {
                    // Only trigger if not already critical (Expired takes precedence)
                    if (!isCritical) isWarning = true; 

                    // Resolve Name for Clarity (e.g., "Yoga", "PT")
                    let typeName = 'Credits';
                    ...
                    // Format: Language specific order handled via i18n placeholders
                    // EN: "Low on credits (2.5 Yoga remaining)."
                    // ZH: "點數不足 (剩餘 2.5 點 - Yoga)。"
        ...
        auth.createUserWithEmailAndPassword(email, password)
            .then(userCredential => {
                ...
                const newUserProfile = {
                    ...
                    purchaseHistory: {},
                    subscriptionStatus: 'basic' // <--- NEW: Default plan
                };
...
${_('whatsapp_closing')}
        ...
        addMemberSearchResults.onclick = (e) => {
            ...
            if (target) {
                ...
                if (memberToAdd && !memberToAdd.monthlyPlan) {
                    ...
                    if (currentBalance < parseFloat(currentCls.credits)) {
                        // Show error with specific type name
                    ...
                    // Deduct from specific wallet bucket
                    updates[`/users/${memberId}/wallet/${requiredTypeId}/balance`] = currentBalance - parseFloat(currentCls.credits);
                ...
                updates[`/classes/${currentCls.id}/bookedBy/${memberId}`] = {
                    ...
                    creditTypeId: requiredTypeId // Save currency type
                };
    ...
    function openBookingModal(cls) {
        ...
        confirmBtn.onclick = () => {
            ...
            if (cls.bookedBy && cls.bookedBy[memberId]) {
            ...
            } else if (!currentUser.monthlyPlan && (userBalance < parseFloat(cls.credits))) {
                ...
                if (!currentUser.monthlyPlan) {
                    const newBalance = userBalance - parseFloat(cls.credits);
    ...
    function handleCancelBooking(cls, memberIdToUpdate = null) {
        ...
        showConfirmation(title, message, () => {
            ...
            // We need the user object to calculate current wallet state
            ...
            const creditsToRefund = bookingDetails.creditsPaid;
            // Fallback to 'general' if old booking record lacks ID
            ...
            if (!memberToUpdate.monthlyPlan) {
                ...
                const newBalance = currentBalance + parseFloat(creditsToRefund);
                
                updates[`/users/${memberId}/wallet/${refundTypeId}/balance`] = newBalance;
                
                // If legacy credits field exists, clean it up
            ...
            database.ref().update(updates).then(() => {
                if (memberIdToUpdate) {
                    // If admin cancelled for a member, refresh their history view
                    // We need to fetch fresh user data first to show updated balance if needed
                    // For now, just re-opening the modal works as it fetches fresh history
                    const updatedMember = { ...memberToUpdate }; // (Optimistic update not critical here)
                    openMemberBookingHistoryModal(updatedMember);
                } 
                else {
                    // If member cancelled, update local state immediately for UI responsiveness
    ...
    function openClsModal(dateIso, clsToEdit = null) {
                ...
                <form id="clsForm">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="pt-4 border-t space-y-4">
                            ...
                            <!-- NEW: Monthly Restriction UI (Only shown if tiers exist) -->
                            <div id="monthlyTierRestrictions" class="hidden ml-7">
                                ...
                                <div id="tierCheckboxes" class="grid grid-cols-2 gap-2 p-3 bg-slate-50 rounded-lg border border-slate-200 hidden">
        ...
        let ctOptions = `<option value="general">General Credits</option>`;
    ...
    function handleClsFormSubmit(e) {
        ...
        let allowedPlanTiers = null; // Null means ALL allowed
        ...
        if (tierAccessType === 'restricted') {
            ...
            // Safety: If restricted but nothing checked, assume nothing allowed (keep empty obj)
        }
        ...
        const newClsDataFromForm = {
            ...
            allowedPlanTiers: allowedPlanTiers // Save the restrictions
        };

        // --- (Existing Payout logic and Update vs Create logic remains identical below) ---
        // Payout logic reconstruction
        ...
        if (clsId) {
            ...
            // Check for credit change on existing bookings
    ...
    function renderMemberSchedulePage(container) {
        ...
        // --- START OF NEW LOGIC ---
        const filterDay = appState.selectedFilters.memberDay; // 'all', '0', '1', etc.
        
        const datesArray = [];
        for (let d = new Date(memberStartDate); d <= memberEndDate; d.setUTCDate(d.getUTCDate() + 1)) {
            // If a day filter is active, check if the current day matches
            if (filterDay !== 'all') {
                const currentDayIndex = d.getDay(); // 0 (Sun) - 6 (Sat)
                if (String(currentDayIndex) !== filterDay) {
        ...
        // If filtered result is empty, user might get confused, so let's handle index carefully
    ...
    function renderAccountPage(container) {
        ...
        // 1. Prepare Base Data & Filter Deleted History for Counts
        ...
        // Filter history immediately to get accurate counts for the headers
        const activePurchaseHistory = firebaseObjectToArray(member.purchaseHistory).filter(p => p.status !== 'deleted');
        const activePaymentHistory = firebaseObjectToArray(member.paymentHistory).filter(p => p.status !== 'deleted');
        
        const purchaseHistoryCount = activePurchaseHistory.length;
        const paymentHistoryCount = activePaymentHistory.length;

        // 2. Identify Unique Credit Types & Calculate Usage Sums
        const usedCreditTypeIds = new Set();
        const creditUsageMap = { 'all': 0 }; // Initialize 'all' sum
        ...
        // 3. Setup Internal State for Filtering (Closure)
        // If the user hasn't selected a filter yet, default to 'all'
        let currentFilterId = 'all';

        // --- HELPER: Render the Bookings List ---
        const renderBookingList = (filterId) => {
            ...
            listContainer.innerHTML = filteredBookings.map(cls => {
                const sportType = appState.sportTypes.find(st => st.id === cls.sportTypeId);
        ...
        // --- HELPER: Render the Filter Pills ---
        const renderFilterPills = () => {
            const pillsContainer = container.querySelector('#bookingFilterPills');
            // We only hide if there is NO data. If there is data, we show "All" even if it's the only one, 
            // so users can see the Total Used count.
            ...
            pillsHTML += `<button class="filter-pill px-3 py-1 text-xs font-bold rounded-full transition-all ${allClass}" data-type-id="all">${_('filter_all')} ${allUsageText}</button>`;

            // Type Pills (Only if more than one type exists, otherwise All covers it)
            if (usedCreditTypeIds.size > 1) {
                usedCreditTypeIds.forEach(typeId => {
                    ...
                    let style = '';
                    let className = 'filter-pill px-3 py-1 text-xs font-bold rounded-full transition-all ml-2 ';
                    
                    if (isActive) {
                        // Solid color when active
                        style = `background-color: ${baseColor}; color: white; box-shadow: 0 4px 6px -1px ${baseColor}66;`;
                    } else {
                        // Faded background when inactive
            ...
            pillsContainer.querySelectorAll('.filter-pill').forEach(btn => {
                btn.onclick = () => {
                    ...
                    renderFilterPills(); // Re-render pills to update active state
                    renderBookingList(currentFilterId); // Re-render list
                };
            });
        };

        // ... (getProfileContent Logic remains exactly the same as before) ...
        const getProfileContent = () => {
            const joinDateHtml = `<div><p data-lang-key="label_join_date" class="text-sm text-slate-500"></p><p class="font-bold text-lg text-slate-800">${formatShortDateWithYear(member.joinDate)}</p></div>`;
            ...
            if (member.monthlyPlan) {
                ...
                const planPill = `<span class="text-base font-medium px-3 py-1 rounded-full" style="${pillStyle}">${formatCurrency(member.monthlyPlanAmount)}${_('label_mo')}${tierLabel}</span>`;
                let renewsText = _('label_na');
                if (member.planStartDate) {
                    const day = parseInt(member.planStartDate.split('-')[2]);
                    const suffix = getOrdinalSuffix(day);
                    renewsText = _('label_on_the_day').replace('{day}', day).replace('{suffix}', suffix);
                }
                const nextDue = member.paymentDueDate ? formatShortDateWithYear(member.paymentDueDate) : 'N/A';
                return `${joinDateHtml}<div><p data-lang-key="label_plan" class="text-sm text-slate-500 mb-1"></p>${planPill}</div><div><p data-lang-key="label_renews_every_month" class="text-sm text-slate-500"></p><p class="font-bold text-lg text-slate-800">${renewsText}</p></div><div><p data-lang-key="label_next_payment_due" class="text-sm text-slate-500"></p><p class="font-bold text-lg text-slate-800">${nextDue}</p></div>`;
            } else {
                ...
                if (activeWalletEntries.length === 0) {
                    ...
                    creditsListHtml = `<div class="mb-3"><div class="mb-1"><span class="font-bold text-sm px-2.5 py-1 rounded-full inline-block" style="background-color: ${neutralColor}26; color: ${neutralColor};">0 ${_('label_credits')}</span></div></div>`;
                } else {
                    creditsListHtml = activeWalletEntries.map(([typeId, data]) => {
                        ...
                        return `<div class="mb-3 last:mb-0"><div class="mb-1" title="${name}"><span class="font-bold text-sm px-2.5 py-1 rounded-full inline-block" style="background-color: ${color}26; color: ${color};">${formatCredits(data.balance)} / ${formatCredits(initial)} ${name}</span></div><div class="text-xs text-slate-500 ml-1"><span class="${isExpired ? 'text-red-600 font-bold' : ''}">${statusLabel}: ${expiryText}</span></div></div>`;
                    }).join('');
                }
                return `${joinDateHtml}<div class="mt-4"><p data-lang-key="label_credits_remaining" class="text-sm text-slate-500 mb-2"></p>${creditsListHtml}</div>`;
            }
        };
        ...
        container.innerHTML = `
            <div class="w-full max-w-screen-lg mx-auto grid grid-cols-1 md:grid-cols-3 gap-8">
                ...
                <div class="md:col-span-1 space-y-8">
                    ...
                    <!-- History Cards (Condensed) -->
                    ${member.monthlyPlan ? `
                    <div class="card p-6"><h4 class="text-xl font-bold text-slate-800 mb-4 text-center">${_('header_payment_history')} (${paymentHistoryCount})</h4><div class="space-y-2 text-sm max-h-40 overflow-y-auto">${activePaymentHistory.length>0?activePaymentHistory.sort((a,b)=>new Date(b.date)-new Date(a.date)).map(p=>{const monthUnit=p.monthsPaid===1?_('label_month_singular'):_('label_month_plural');const entryText=_('history_payment_entry').replace('{amount}',formatCurrency(p.amount)).replace('{quantity}',p.monthsPaid).replace('{unit}',monthUnit);const tierId=p.monthlyPlanTierId;const tier=appState.monthlyPlanTiers?appState.monthlyPlanTiers.find(t=>t.id===tierId):null;let tierPill='';if(tier){const name=getMonthlyPlanName(tier);tierPill=`<span class="inline-block text-[10px] font-bold px-2 py-0.5 rounded-full ml-1 align-middle" style="background-color: ${tier.color}26; color: ${tier.color}">${name}</span>`;}let auditMessage='';if(p.lastModifiedBy){const actionKey=(p.date===p.lastModifiedAt?'audit_added_by':'audit_edited_by');auditMessage=`<span class="text-xs text-slate-500 mt-1">${_(actionKey).replace('{name}',p.lastModifiedBy).replace('{date}',formatShortDateWithYear(p.lastModifiedAt))}</span>`;}return `<div class="text-slate-600 bg-slate-50 p-2 rounded-md"><div><strong>${formatShortDateWithYear(p.date)}:</strong> ${entryText} ${tierPill}</div>${auditMessage}</div>`;}).join(''):`<p class="text-sm text-slate-500 text-center">${_('no_payment_history')}</p>`}</div></div>`:
                    `<div class="card p-6"><h4 class="text-xl font-bold text-slate-800 mb-4 text-center">${_('header_purchase_history')} (${purchaseHistoryCount})</h4><div class="space-y-2 text-sm max-h-40 overflow-y-auto">${activePurchaseHistory.length>0?activePurchaseHistory.sort((a,b)=>new Date(b.date)-new Date(a.date)).map(p=>{const creditsUnit=p.credits===1?_('label_credit_single'):_('label_credit_plural');const entryText=_('history_purchase_entry').replace('{amount}',formatCurrency(p.amount)).replace('{quantity}',p.credits).replace('{unit}',creditsUnit);const typeId=p.creditTypeId||'general';const typeDef=appState.creditTypes?appState.creditTypes.find(ct=>ct.id===typeId):null;const typeName=typeDef?getCreditTypeName(typeDef):(typeId==='general'?'General':'Credits');const typeColor=typeDef?.color||'#64748b';const typePill=`<span class="inline-block text-[10px] font-bold px-2 py-0.5 rounded-full ml-1 align-middle" style="background-color: ${typeColor}26; color: ${typeColor}">${typeName}</span>`;let auditMessage='';if(p.lastModifiedBy){const actionKey=(p.date===p.lastModifiedAt?'audit_added_by':'audit_edited_by');auditMessage=`<span class="text-xs text-slate-500 mt-1">${_(actionKey).replace('{name}',p.lastModifiedBy).replace('{date}',formatShortDateWithYear(p.lastModifiedAt))}</span>`;}return `<div class="text-slate-600 bg-slate-50 p-2 rounded-md"><div><strong>${formatShortDateWithYear(p.date)}:</strong> ${entryText} ${typePill}</div>${auditMessage}</div>`;}).join(''):`<p class="text-sm text-slate-500 text-center">${_('no_purchase_history')}</p>`}</div></div>`}
                </div>
                ...
                <div class="md:col-span-2">
                    <div class="card p-6">
                        <div class="flex flex-wrap justify-between items-center mb-4 gap-2">
                            <h3 class="text-2xl font-bold text-slate-800 whitespace-nowrap">
                                ${_('header_my_bookings')} <span id="accountBookingCount"></span>
                            </h3>
                        ...
                        <div id="accountBookingList" class="space-y-3 max-h-[60vh] overflow-y-auto">
                            <!-- Injected via renderBookingList() -->
                        </div>
                    </div>
                </div>
            </div>`;
        ...
        // Initial Render of List and Pills
    ...
    function renderMembersPage(container) {
        ...
        const updateTable = (searchTerm = '') => {
            ...
            tableBody.innerHTML = paginatedUsers.map(member => {
                ...
                // --- Restore Recent History Logic ---
                if (member.monthlyPlan) {
                    const payments = firebaseObjectToArray(member.paymentHistory)
                    ...
                    if (payments.length > 0) {
                        ...
                        // Use localized history string
                        const details = _('history_payment_entry')
                    ...
                    const purchases = firebaseObjectToArray(member.purchaseHistory)
                        .filter(p => p.status !== 'deleted')
                        .sort((a, b) => new Date(b.date) - new Date(a.date));
                    ...
                    if (purchases.length > 0) {
                        ...
                        // Use localized history string
                        const details = _('history_purchase_entry')
                            .replace('{amount}', formatCurrency(p.amount))
                            .replace('{quantity}', p.credits)
                            .replace('{unit}', creditUnit);
                ...
                // Only show tooltip if there is a dot displayed (Critical, Warning, or Notes)
                ...
                <tr class="border-b border-slate-100">
                    ...
                    <td class="p-2 align-middle">${creditsHtml}</td>
                    <td class="p-2 align-middle">${expiryHtml}</td>
        ...
        searchInput.oninput = () => {
            ...
            appState.pagination.members.page = 1; // Reset to page 1 on search
            updateTable(term);
        ...
        clearBtn.onclick = () => {
            searchInput.value = '';
            appState.pagination.members.page = 1; // Reset to page 1 on clear
            updateTable('');
                    ...
                    if (member.monthlyPlan) {
                        ...
                        if (entries.length > 0) {
                            ...
                            expiryString = entries.map(([typeId, data]) => {
                                const typeDef = appState.creditTypes ? appState.creditTypes.find(ct => ct.id === typeId) : null;
                                const name = typeDef ? getCreditTypeName(typeDef) : (data.isLegacy ? 'General' : 'Unknown');
                                return `${name}: ${data.expiryDate || 'N/A'}`;
                            }).join('; ');
                        } else {
                            creditsString = "0";
                        }
    ...
    function openMemberModal(memberToEdit) {
                ...
                <form id="memberForm" class="space-y-4">
                    ...
                    <div class="pt-4 border-t">
                        ...
                        <div class="flex bg-slate-100 p-1 rounded-lg mb-4 select-none">
                            ...
                            <button type="button" id="btnPlanMonthly" class="flex-1 py-2 text-sm font-semibold rounded-md transition-all">${_('label_monthly_plan')}</label>
                        </div>
                        <!-- Hidden checkbox to maintain compatibility with save logic -->
                        <input type="checkbox" id="monthlyPlan" class="hidden">
        ...
        // --- IPHONE PLACEHOLDER FIX ---
        // Helper to toggle between 'text' (for placeholder) and 'date' (for picker) on iOS
        const setupResponsiveDateInput = (input) => {
            ...
            // Check initial state
            refreshType();
            return refreshType; // Return the updater so we can call it when setting value via JS
        };
        ...
        if (showLegacyOption) {
            tierOptionsHTML += `<option value="" data-is-legacy="true">${_('label_legacy_tier')}</option>`;
        }
        ...
        if (memberToEdit.monthlyPlanTierId) {
            ...
            if (showLegacyOption) {
                ...
                monthlyTierSelect.selectedIndex = 0;
            }
        }
        ...
        form.querySelectorAll('.expiry-quick-select-btn').forEach(btn => {
            btn.onclick = () => {
                ...
                if (!isNaN(yearsToAdd)) {
                    ...
                    updateExpiryUI(); // Ensure date picker UI updates
                }
            };
        });
        ...
        form.querySelectorAll('.due-date-quick-select-btn').forEach(btn => {
            btn.onclick = () => {
                ...
                if (!isNaN(monthsToAdd)) {
                    ...
                    updatePaymentDueUI(); // Ensure date picker UI updates
                }
            };
        });
        ...
        btnPlanCredit.onclick = () => {
            monthlyPlanCheckbox.checked = false;
            updatePlanFields();
        };
        
        btnPlanMonthly.onclick = () => {
            monthlyPlanCheckbox.checked = true;
            updatePlanFields();
        };
    ...
    function calculateRevenueForBookings(bookings, revenueMode = 'actual') {
        ...
        for (const memberId in bookingsByMember) {
            ...
            // Legacy Support: If we have 'general' credits, treat them as a fallback for ANY type 
            // (Optional: strict mode would prevent this, but better safe for migration)
            ...
            for (const cls of memberClasses) {
                ...
                if (shouldCalculate) {
                    ...
                    if (member.monthlyPlan) {
                        ...
                        // 2. Get the specific pool, or fallback to general/legacy pool if empty/missing
                        // (This ensures old classes booked with old credits still calculate)
    ...
    function renderAdminPage(container) {
        ...
        container.innerHTML = `
            <div class="space-y-8">
                ...
                <!-- Credit Types Card & Monthly Plan Tiers Card -->
                ${isOwner ? `
    ...
    function renderAdminLists() {
        ...
        const getMoveButtons = (type, id, index, totalLength) => {
            ...
            // Define classes for enabled/disabled states
        ...
        if (sportsList && sportsPaginationContainer && sportsHeader) {
            ...
            // Calculate absolute index for the move buttons based on pagination
                ...
                creditTypesList.innerHTML = credits.map((ct, idx) => {
                    ...
                    <li class="flex justify-between items-center bg-slate-100 p-2 rounded-md">
                        <div class="flex items-center gap-3">
                            ...
                            <span class="text-slate-700 font-semibold">${creditName}</span>
                        </div>
                        <div class="flex items-center gap-2">
                            ${moveBtns}
                ...
                monthlyTiersList.innerHTML = tiers.map((tier, idx) => {
                    ...
                    <li class="flex justify-between items-center bg-slate-100 p-2 rounded-md">
                        <div class="flex items-center gap-3">
                            ...
                            <span class="text-slate-700 font-semibold">${tierName}</span>
                        </div>
                        <div class="flex items-center gap-2">
                            ${moveBtns}
    ...
    function openCreditTypeModal(creditTypeToEdit = null) {
        // Re-use SportType modal styling for consistency
        ...
        if (creditTypeToEdit) {
            ...
            form.querySelector('#creditTypeColor').value = CLS_COLORS[0];
        }
    ...
    function openMonthlyPlanTierModal(tierToEdit = null) {
        ...
        const modal = DOMElements.sportTypeModal; // Reusing the modal container
        const form = modal.querySelector('form');
        ...
        if (tierToEdit) {
            ...
            form.querySelector('#monthlyTierColor').value = CLS_COLORS[0];
        }
    ...
    async function renderIncomePage(container) {
        ...
        const renderIncomeDetails = async () => {
            ...
            // Calculate Revenue for ALL classes in period to get the revenue map
            const memberIdsInPeriod = new Set();
            classesInPeriod.forEach(cls => {
                if (cls.bookedBy) Object.keys(cls.bookedBy).forEach(id => memberIdsInPeriod.add(id));
            });
            ...
            // Use 'projected' mode to show revenue for future bookings in the selected month as well
            const { revenueByClsId } = calculateRevenueForBookings(allMemberBookings, 'projected');

            // 4. Group Data by Date
            const dailyStatsMap = {};
            ...
            classesInPeriod.forEach(cls => {
                ...
                dayStat.revenue += (revenueByClsId.get(cls.id) || 0);
            });

            const dailyDetails = Object.values(dailyStatsMap);
            
            // Calculate Totals for Header
            const totalIncome = dailyDetails.reduce((sum, d) => sum + d.revenue, 0);
            ...
            detailsContainer.innerHTML = `
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                    <div class="bg-slate-100 p-4 rounded-lg"><p class="text-sm text-slate-500">${_('label_total_income')}</p><p class="text-3xl font-bold text-slate-800">${formatCurrency(totalIncome)}</p></div>
                    ...
                    <div class="overflow-x-auto"><table class="w-full text-left">
                        <thead>
                            <tr class="border-b">
                                ...
                                <th class="p-2 sortable cursor-pointer" data-sort-key="attendees">${_('table_header_attendees')}<span class="sort-icon"></span></th>
                                <th class="p-2 text-right sortable cursor-pointer" data-sort-key="revenue">${_('table_header_income')}<span class="sort-icon"></span></th>
                            </tr>
                        </thead>
                        <tbody>
                            ${paginatedDetails.map(d => `
                                <tr class="border-b border-slate-100">
                                    ...
                                    <td class="p-2 text-right font-semibold text-green-600">${formatCurrency(d.revenue)}</td>
                                </tr>`).join('') || `<tr><td colspan="4" class="text-center p-4 text-slate-500">${_('info_no_income_data')}</td></tr>`}
                        </tbody>
                    </table></div>
            ...
            renderPaginationControls(paginationContainer, page, totalPages, dailyDetails.length, itemsPerPage.income, (newPage) => {
                ...
                renderIncomeDetails(); // Re-render with new page
            });
            ...
            // Setup Export Button Handler (Closure over calculated data)
            exportBtn.onclick = () => {
                exportBtn.disabled = true;
                exportBtn.innerHTML = _('status_exporting');
                ...
                const exportData = dailyDetails.map(d => ({
                    ...
                    [_('table_header_income')]: d.revenue.toFixed(2)
                }));

                // Add Summary Rows to Export
                exportData.push({}); // Spacer
                exportData.push({ [_('export_header_date')]: 'TOTAL', [_('table_header_income')]: totalIncome.toFixed(2) });
    ...
    async function openMemberBookingHistoryModal(member) {
        ...
        try {
            ...
            const getPillsHTML = () => {
                ...
                // Type Pills (Only show if > 1 type is used, otherwise "All" is sufficient)
                if (usedCreditTypeIds.size > 1) {
                    usedCreditTypeIds.forEach(typeId => {
                        const typeDef = appState.creditTypes ? appState.creditTypes.find(ct => ct.id === typeId) : null;
                        const typeName = typeDef ? getCreditTypeName(typeDef) : (typeId === 'general' ? 'General' : 'Credits');
            ...
            DOMElements.memberBookingHistoryModal.innerHTML = `
                <div class="bg-white p-8 rounded-2xl shadow-2xl w-full max-w-2xl transform transition-all duration-300 scale-95 opacity-0 modal-content relative open">
                    ...
                    <div class="space-y-3 max-h-[60vh] overflow-y-auto" id="history-content-area"></div>
                </div>
            `;
            ...
            const bindPillEvents = () => {
                ...
                container.querySelectorAll('.modal-filter-pill').forEach(btn => {
                    btn.onclick = () => {
                        currentFilterId = btn.dataset.typeId;
                        renderContent();
                        bindPillEvents(); // Re-render pills to show active state
                    };
                });
            };
    ...
    const initOwnerListeners = () => {
        ...
        const nonUserRefs = {
            ...
            monthlyPlanTiers: database.ref('/monthlyPlanTiers') // <--- NEW
        };
    ...
    const initMemberListeners = () => {
        ...
        const keysToOff = ['tutors', 'sportTypes', 'studioSettings', 'creditTypes', 'monthlyPlanTiers'];
        ...
        // ... (Existing Schedule Status and Class Fetch Logic) ...
        let initialLoadComplete = false;
        let scheduleStatus = {};
        ...
        dataListeners.scheduleStatus = (snapshot) => {
            ...
            if (initialLoadComplete) { initMemberListeners(); }
        };
        ...
        const initialFetchAndRender = async () => {
            try {
                ...
                const publishedClasses = allClasses.filter(cls => { return cls.date < todayIso || scheduleStatus[cls.date] === true; });
        ...
        activeClassesRef.on('child_changed', (snapshot) => {
            ...
            const index = appState.classes.findIndex(c => c.id === updatedCls.id);
            if (index > -1) appState.classes[index] = updatedCls; else appState.classes.push(updatedCls);
            if (timeHasChanged) { _reSortDayColumn(updatedCls.date); } else { const clsElement = document.getElementById(updatedCls.id); if (clsElement) { clsElement.replaceWith(createClsElement(updatedCls)); } }
        });
        activeClassesRef.on('child_added', (snapshot) => {
            if (!initialLoadComplete) return;
            const newCls = { id: snapshot.key, ...snapshot.val() };
    ...
    const setupAuthFormListeners = () => {
        ...
        if (registerPhoneInput) {
            registerPhoneInput.setAttribute('placeholder', ''); // Ensure attribute exists for updateUIText to target
