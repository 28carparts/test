...
${_('whatsapp_closing')}
    ...
    function openClsModal(dateIso, clsToEdit = null) {
        ...
        if (targetBranchId) {
            ...
            targetBranchName = b ? b.name : _('label_legacy_branch');
        }
        ...
        DOMElements.clsModal.innerHTML = `
            <div class="bg-white p-6 md:p-8 rounded-2xl shadow-2xl w-full max-w-md transform transition-all duration-300 scale-95 opacity-0 modal-content relative">
                ...
                <!-- Branch Indicator -->
                ${hasMultipleBranches ? `<div class="text-center mb-6"><span class="bg-slate-100 text-slate-600 text-xs font-bold px-3 py-1 rounded-full border border-slate-200">${_('info_class_branch')}: ${targetBranchName}</span></div>` : '<div class="mb-6"></div>'}
        ...
        // --- Monthly Tier Restriction Logic (Unchanged from Phase 1) ---
    ...
    function renderOwnerSchedule(container) {
        ...
        if (!appState.selectedScheduleBranch && branches.length > 0) {
            // Default to user's home branch or the first one
            appState.selectedScheduleBranch = appState.currentUser.homeBranchId || branches[0].id;
        } else if (branches.length === 0) {
            appState.selectedScheduleBranch = null; // No branches defined
        }
        ...
        if (hasMultipleBranches) {
            if (branches.length <= 3) {
                ...
                const buttonsHtml = branches.map(b => {
                    ...
                    const activeClass = isActive ? 'bg-white shadow-sm text-indigo-600' : 'text-slate-500 hover:text-slate-700 hover:bg-slate-200/50';
                    // Localization Logic
                    const bName = (appState.currentLanguage === 'zh-TW' && b.name_zh) ? b.name_zh : b.name;
                    return `<button class="branch-switch-btn flex-1 py-1.5 px-3 text-sm font-bold rounded-md transition-all ${activeClass}" data-branch-id="${b.id}">${bName}</button>`;
                }).join('');
        ...
        // We add a wrapper for the switcher and the carousel
        container.innerHTML = `
        ...
        if (hasMultipleBranches) {
            if (branches.length <= 3) {
                container.querySelectorAll('.branch-switch-btn').forEach(btn => {
                    btn.onclick = () => {
                        if (appState.selectedScheduleBranch !== btn.dataset.branchId) {
                            appState.selectedScheduleBranch = btn.dataset.branchId;
                            renderOwnerSchedule(container); // Re-render
                        }
                    };
                });
            } else {
        ...
        // 5. Filter Classes based on Selection
        ...
        for (let d = new Date(ownerStartDate); d <= ownerEndDate; d.setUTCDate(d.getUTCDate() + 1)) {
            if (filterDay !== 'all') {
                ...
                if (String(currentDayIndex) !== filterDay) {
                    continue; 
                }
            }
        ...
        if (hasMultipleBranches && appState.selectedScheduleBranch) {
            filteredClasses = appState.classes.filter(c => {
                // Logic:
                // 1. If class has branchId, match it.
                // 2. If class has NO branchId (legacy), assume it belongs to the first branch (index 0).
                const cBranch = c.branchId;
                if (cBranch) {
                    return cBranch === appState.selectedScheduleBranch;
                } else {
                    // Legacy Handling: Show on first branch only
                    return branches[0].id === appState.selectedScheduleBranch;
                }
            });
        }
        // ------------------------------
        ...
        // Pass the filtered list to the renderer
    ...
    function renderMemberSchedulePage(container) {
        ...
        if (!appState.selectedScheduleBranch && branches.length > 0) {
            ...
            appState.selectedScheduleBranch = null;
        }
            ...
            if (branches.length <= 3) {
                container.querySelectorAll('.branch-switch-btn').forEach(btn => {
                    btn.onclick = () => {
                        if (appState.selectedScheduleBranch !== btn.dataset.branchId) {
                            appState.selectedScheduleBranch = btn.dataset.branchId;
                            renderMemberSchedulePage(container);
                        }
                    };
                });
            } else {
        ...
        // B. Calculate End Date based on filtered classes
        ...
        if (type === 'day') {
            ...
            if (classesToCopy.length === 0) {
                ...
                const copyPromises = classesToCopy.map(cls => {
                    // 1. Exclude old payoutDetails and status keys
                    const { id, bookedBy, attendedBy, payoutDetails, ...restOfCls } = cls;
                    
                    // 2. Construct new class object
                    const newCls = {
                        ...restOfCls, // This includes costCreditTypeId automatically
                        date: targetDate,
                        bookedBy: {},
                        attendedBy: {}
                    };

                    // 3. Recalculate payout based on current tutor salary settings
            ...
            const newCls = {
                ...restOfCls, // Includes costCreditTypeId
                date: targetDate,
                bookedBy: {},
                attendedBy: {}
            };
    ...
    function renderCheckInPage(container) {
        ...
        if (hasMultipleBranches) {
            ...
            options += branches.map(b => `<option value="${b.id}" ${savedDeviceBranch === b.id ? 'selected' : ''}>${b.name}</option>`).join('');
            
            deviceLocationSelector = `
                <div class="mb-6 flex justify-center items-center gap-2 bg-slate-50 p-2 rounded-lg border border-slate-200">
                    ...
                    <label class="text-sm font-bold text-slate-600 whitespace-nowrap">${_('label_device_location')}:</label>
                    <select id="deviceBranchSelect" class="form-select py-1 pl-2 pr-8 text-sm font-semibold w-auto bg-transparent border-none focus:ring-0">
                        ${options}
                    </select>
                </div>`;
        }
                            ...
                            ? `<h5 class="text-center font-semibold text-slate-600">${_('header_upcoming_checkins')}</h5>` +
                              todaysUnattendedBookings
                                ...
                                .map(cls => {
                                    ...
                                    // --- Optional: Visual indicator if class is at a different branch ---
                                    const clsBranchId = cls.branchId || (branches.length > 0 ? branches[0].id : null);
                                    const clsBranch = branches.find(b => b.id === clsBranchId);
                                    const branchBadge = (hasMultipleBranches && clsBranch) 
                                        ? `<span class="block text-[10px] uppercase font-bold tracking-wider text-slate-400 mt-0.5">${clsBranch.name}</span>` 
                                        : '';
        ...
        const deviceSelect = container.querySelector('#deviceBranchSelect');
        if (deviceSelect) {
            deviceSelect.onchange = () => {
                localStorage.setItem('studioPulseDeviceBranch', deviceSelect.value);
    ...
    function renderAdminPage(container) {
        ...
        // Grid 1: Sport Types & Tutors (2 columns on Desktop)
        // Grid 2: Credits, Tiers, Locations (3 columns on Large Desktop)
        ...
        container.innerHTML = `
            <div class="space-y-8">
                ...
                <div class="grid ${gridClassesBottom} gap-8 items-start">
                    ...
                    <div class="card p-6 flex flex-col">
                        <div class="flex flex-wrap gap-4 justify-between items-center mb-4">
                            <h3 data-lang-key="header_credit_types" class="text-2xl font-bold text-slate-800"></h3>
                        </div>
                    ...
                    <div class="card p-6 flex flex-col">
                        <div class="flex flex-wrap gap-4 justify-between items-center mb-4">
                            <h3 data-lang-key="header_monthly_tiers" class="text-2xl font-bold text-slate-800"></h3>
                        </div>
    ...
    function renderAdminLists() {
        ...
        // 0. RENDER LOCATIONS (BRANCHES) - BILINGUAL UPDATE
    ...
    function handleBranchFormSubmit(e) {
        ...
        // Basic validation: At least English name is required (or fallback logic)
        if (!branchData.name) {
        ...
        if (id) {
            ...
            // Handle "Soft Migration" implicitly:
            // If this is the FIRST time branches are added, existing users/classes
            // might assume "Default". We don't change old data yet, just add the new branch reference.
    ...
    async function renderClassesPage(container) {
            ...
            const updateClassesTable = () => {
                ...
                tableBody.innerHTML = paginatedClasses.map((cls, index) => {
                    ...
                    // --- FIX: Resolve Branch ID for the row (Fallback to first branch if legacy) ---
            ...
            tableBody.addEventListener('click', (e) => {
                ...
                if (row) {
                    ...
                    // --- FIX: Read the branch ID from the row ---
                    ...
                    if (date) {
                        ...
                        // --- FIX: Update the global state so the schedule renders this branch ---
                        if (targetBranchId) {
    ...
    const initOwnerListeners = () => {
        ...
        Object.entries(nonUserRefs).forEach(([key, ref]) => {
            dataListeners[key] = (snapshot) => {
                ...
                if (key === 'currentUser') {
                    ...
                    // Handles lists: tutors, sportTypes, creditTypes, monthlyPlanTiers
                    ...
                    // Refresh pages that might show plan pills
                    if ((key === 'creditTypes' || key === 'monthlyPlanTiers') && (appState.activePage === 'members' || appState.activePage === 'account')) {
                        renderCurrentPage();
                    }
                }
            };
        ...
        // ... (Rest of the existing initOwnerListeners code for users and classes) ...
        ...
        // ... (Existing class change listeners) ...
        activeClassesRef.on('child_changed', (snapshot) => {
            ...
            if (timeHasChanged) { _reSortDayColumn(updatedCls.date); } else { const clsElement = document.getElementById(updatedCls.id); if (clsElement) { clsElement.replaceWith(createClsElement(updatedCls)); } if (oldCls) { const oldBookedIds = Object.keys(oldCls.bookedBy || {}); const newBookedIds = Object.keys(updatedCls.bookedBy || {}); if (newBookedIds.length > oldBookedIds.length) { const newMemberId = newBookedIds.find(id => !oldBookedIds.includes(id)); if (newMemberId) { const member = appState.users.find(u => u.id === newMemberId); const sportType = appState.sportTypes.find(st => st.id === updatedCls.sportTypeId); if (member && sportType) { showBookingNotification({ memberName: member.name, clsName: sportType.name, clsTime: updatedCls.time, duration: updatedCls.duration }); } } } } }
        });
        activeClassesRef.on('child_added', (snapshot) => {
            if (!initialLoadComplete) return;
            const newCls = { id: snapshot.key, ...snapshot.val() };
            if (!appState.classes.some(c => c.id === newCls.id)) { appState.classes.push(newCls); _reSortDayColumn(newCls.date); }
        });
    ...
    const initMemberListeners = () => {
        ...
        Object.entries(refs).forEach(([key, ref]) => {
            dataListeners[key] = (snapshot) => {
                ...
                if (key === 'studioSettings') {
                    ...
                    if (key === 'branches' && appState.activePage === 'schedule') renderCurrentPage();
                }
            };
            ref.on('value', dataListeners[key], (error) => console.error(`Listener error on /${key}`, error));
        });

        // --- Schedule Status & Class Fetch Logic ---
        let initialLoadComplete = false;
        let scheduleStatus = {};
        ...
        dataListeners.scheduleStatus = (snapshot) => {
            ...
            // --- FIX: RESTORE INSTANT UPDATES ---
            // If the schedule status changes (e.g., a day is published), we must
            // re-run the fetch sequence. This ensures classes that were previously
            // hidden (and thus not in appState.classes) are now retrieved and shown.
        ...
        // --- HELPER: Check if a class is published ---
        const isClassPublished = (cls, statusMap) => {
            const todayIso = getIsoDate(new Date());
            
            // 1. Past classes are always visible
            if (cls.date < todayIso) return true;

            // 2. Check Status
            const dateStatus = statusMap[cls.date];
            
            // A. Legacy Boolean (Old data)
            if (dateStatus === true) {
                // If boolean true, it applies to "Legacy/Main" classes only
                const mainBranchId = appState.branches[0]?.id;
                // It is published if the class has NO branch, OR matches main branch
                return !cls.branchId || cls.branchId === mainBranchId;
            }
            
            // B. New Object Format (Branch Specific)
            if (dateStatus && typeof dateStatus === 'object') {
                const branchId = cls.branchId || appState.branches[0]?.id; // Resolve class branch
                return dateStatus[branchId] === true;
            }
        ...
        const initialFetchAndRender = async () => {
            try {
                // Ensure branches are loaded for the helper
                if (appState.branches.length === 0) {
                    const bSnap = await database.ref('/branches').once('value');
                    appState.branches = firebaseObjectToArray(bSnap.val());
                }
                ...
                // Filter using the new smart helper
                const publishedClasses = allClasses.filter(cls => isClassPublished(cls, scheduleStatus));
                ...
                renderCurrentPage();
                initialLoadComplete = true;
            } catch (error) { console.error("Initial class fetch failed for member:", error); }
        };
        ...
        activeClassesRef.on('child_changed', (snapshot) => {
            ...
            // Use helper to check visibility
            if (!isClassPublished(updatedCls, scheduleStatus)) return;
        ...
        activeClassesRef.on('child_added', (snapshot) => {
            ...
            // Use helper to check visibility
            if (!isClassPublished(newCls, scheduleStatus)) return;
    ...
    const setupAuthFormListeners = () => {
        ...
        registerForm.onsubmit = (e) => {
            ...
            // If the dropdown exists and is visible, value is required
            if (branchSelect && branchSelect.offsetParent !== null) {
            ...
            const formData = {
                ...
                homeBranchId: selectedBranchId // Pass this to the handler
            };
        ...
        // --- UPDATED: Dynamic Branch Injection Logic ---
        showRegisterLink.onclick = async (e) => {
            e.preventDefault();
            loginContainer.classList.add('hidden');
            registerContainer.classList.remove('hidden');

            // Check if we have already injected the branch selector
            if (!document.getElementById('registerBranchContainer')) {
                try {
                    // Fetch branches publicly (Assuming rules allow read, or we handle empty gracefully)
                    ...
                    if (branchesVal) {
                        ...
                        const branches = Object.entries(branchesVal).map(([id, val]) => ({ id, ...val }));
                        branches.sort((a, b) => (a.order || 0) - (b.order || 0));

                        // Only show dropdown if > 1 branch. 
                        // If 1 branch exists, we can default to it silently later, or show it as read-only. 
                        // Requirement: Switchable. We will show if > 0 to be explicit, but strictly required if > 1.
                        if (branches.length > 1) {
                            const container = document.createElement('div');
                            container.id = 'registerBranchContainer';
                            container.className = 'mb-2';
                            
                            const label = document.createElement('label');
                            label.className = 'block text-slate-600 text-sm font-semibold mb-2';
                            label.textContent = _('label_home_branch'); // Will need translation update trigger
                            label.dataset.langKey = 'label_home_branch';

                            const select = document.createElement('select');
                            select.id = 'registerBranchSelect';
                            select.className = 'form-select w-full';
                            select.required = true;
                            
                            let options = `<option value="" disabled selected>${_('placeholder_select_branch')}</option>`;
                            branches.forEach(b => {
                            ...
                            // Insert before the password field (aesthetic choice)
                            const passwordFieldParent = registerPasswordInput.closest('div').parentElement;
                            registerForm.insertBefore(container, passwordFieldParent);
                        }
                    }
                } catch (err) {
                    console.log("Could not load branches for registration (likely permission issue, skipping UI):", err);
                }
            }
        };
                ...
                database.ref('/users/' + user.uid).once('value').then(snapshot => {
                    ...
                    if (userData && (userData.role === 'owner')) {
                        ...
                        database.ref('/branches').once('value').then(bSnap => {
                            if (!bSnap.exists()) {
                                // NO branches found. Create default "Main Studio".
