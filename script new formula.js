                ...
                fList.addEventListener('click', (e) => {
                    ...
                    if (type === 'formula') {
                        if (target.classList.contains('edit-btn')) {
                            ...
                            // Handle Delete
                            showConfirmation(_('confirm_delete_generic_title'), _('confirm_delete_generic_desc').replace('{name}', name), () => {
                                database.ref('/salaryFormulas/' + id).remove();
                            });
                        }
                    }
                });
...
return Math.min(base + (A - threshold) * bonus, cap);`;
    ...
    function addSkillRow(container, skill = null) {
        ...
        let formulaOptions = '';
        if (appState.salaryFormulas && appState.salaryFormulas.length > 0) {
            formulaOptions = appState.salaryFormulas.map(f => 
    ...
    function handleTutorFormSubmit(e) {
        ...
        form.querySelectorAll('.tutor-skill-row').forEach(row => {
            skills.push({
                ...
                salaryFormulaId: row.querySelector('.salary-formula-select').value // Capture this
            });
        });
