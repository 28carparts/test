...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const renderNav = () => {
        ...
        if (isAdmin) {
            ...
            if (isOwner && checkSubscriptionAccess('income')) {
            ...
            if (isOwner && checkSubscriptionAccess('salary')) {
            ...
            if (isOwner && checkSubscriptionAccess('statistics')) {
    ...
    const renderCurrentPage = () => {
        ...
        // If an admin user tries to access a member-only page (Except Check-in which relies on Nav/Modal)
        if ((isOwner || isStaff) && memberOnlyPages.includes(pageIdToRender)) {
            pageIdToRender = 'schedule';
        }

        // --- 2. Subscription-Based Access Control ---
        // If the page is restricted by the plan, redirect to schedule
...
${_('whatsapp_closing')}
    ...
    async function renderAdminPage(container) {
        ...
        let formulaCardHTML = '';
        if (isOwner && checkSubscriptionAccess('admin_formulas')) {
        ...
        try {
            ...
            if (isOwner) setupSearchField('tutorSearchInput', 'clearTutorSearchBtn', 'tutors', 'tutors');
            ...
            if (settingsForm) {
                const defaults = appState.studioSettings.clsDefaults || {};
                // Populate fields even if defaults object is sparse
            ...
            if (btnExportSchedule) {
                btnExportSchedule.onclick = async () => {
                    // (Export logic remains same as provided previously - safe copy/paste)
                    ...
                    try {
                        ...
                        // Helper for date ranges (redefined locally to be safe)
...
return Math.min(base + (A - threshold) * bonus, cap);`;
        ...
        Object.entries(nonUserRefs).forEach(([key, ref]) => {
            dataListeners[key] = (snapshot) => {
                ...
                if (key === 'currentUser') {
                    ...
                    // Sync plan to global state
                    appState.studioPlan = val.subscriptionStatus; 
                    renderNav(); 
                    renderSubscriptionBadge();
                    if (['income','salary','statistics','check-in','filter','account'].includes(appState.activePage)) renderCurrentPage();
    ...
    const initMemberListeners = () => {
        ...
        // --- NEW: Listen for Owner's Subscription Status ---
        // This ensures the "Check-In" button appears/disappears dynamically
        if (ownerPlanQuery) ownerPlanQuery.off(); // Cleanup previous if exists
        ...
        const handleOwnerPlanUpdate = (snapshot) => {
            ...
            // snapshot.val() is an object with keys { "USER_ID": { ...data } }
            if (ownerData) {
                const owner = Object.values(ownerData)[0];
                if (owner) {
                    appState.studioPlan = owner.subscriptionStatus;
                    renderNav(); // Refresh UI to update Check-In button
                    if (appState.activePage === 'check-in') renderCurrentPage();
                }
            }
        };
        // Use 'value' to get the list (of 1) whenever it changes
